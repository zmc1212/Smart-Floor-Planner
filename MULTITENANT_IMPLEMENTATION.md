...
3. **超级管理员显式租户上下文**：
   - 对于超级管理员（`super_admin`/`admin`），在访问业务管理页面（如线索、员工、户型图等）时，必须在前端实现**企业切换器（Enterprise Selector）**。
   - 禁止在这些视图中一次性列出所有企业的数据。
   - 切换企业时，所有相关子数据（如部门、员工列表）必须同步刷新。
4. **避免手动过滤**：不要在新代码中使用`getTenantFilter`
5. **索引设计**：确保所有查询字段都有合适的索引
