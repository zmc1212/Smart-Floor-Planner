...
type WorkItem = { id: string; leadId: string; floorPlanId?: string | null; title: string; subtitle: string; phone?: string | null; status: string; updatedAt?: string; timeRange?: string };
...
export default function StaffWorkbench({ role }: { role: 'designer' | 'measurer' }) {
  ...
  return <Flex vertical gap={20} className="dashboard-stack">
    ...
    {data.role === 'measurer' ? <Alert showIcon type="info" message="正式 BLE 量房入口仍在小程序" description="后台用于查看任务和交接结果；点击任务后请使用小程序正式量房编辑器完成采集。" action={<Button type="link" href="/measurements">查看量房记录</Button>} /> : null}
    <Row gutter={[20, 20]}>
