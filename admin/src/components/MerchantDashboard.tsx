    ...
    <Flex vertical gap={24} className="dashboard-stack">
      ...
      <Row gutter={[24, 24]}>
        <Col xs={24} xl={15}><Flex vertical gap={16}>
          <Card className="admin-panel-card dashboard-workbench-card" size="small" title="协作待办" extra={<DashboardLink href="/promotion-records" label="前往处理" onNavigate={(href) => router.push(href)} />}>
      ...
      {canViewEnterpriseAssets ? <Card className="admin-panel-card dashboard-workbench-card" size="small" title="最近线索流转" extra={<DashboardLink href="/leads" label="查看全部" onNavigate={(href) => router.push(href)} />}>
...
function DashboardLink({ href, label, onNavigate }: { href: string; label: string; onNavigate: (href: string) => void }) { return <Button type="link" size="small" icon={<ArrowRight size={15} />} onClick={() => onNavigate(href)}>{label}</Button>; }
