...
const CATEGORIES = [
  ...
  { id: 'chair', label: '椅凳', icon: Armchair },
...
const FURNITURE_ASSETS: FurnitureAsset[] = [
  createAsset(
    ...
    'sofa',
    '客厅主沙发',
    'a soft L-shaped sectional sofa',
    'cream fabric upholstery, rounded silhouette, warm residential styling',
    'primary_seating',
    'large',
  ...
  createAsset(
    ...
    'light wood and stone mix, clean modern detailing',
    'center_table',
    'medium',
  ...
  createAsset(
    ...
    'chair',
    '点缀单椅',
    'an accent lounge chair',
    'slim profile, cozy upholstery, premium residential styling',
    'accent_seating',
    'medium',
    'from-neutral-200 to-zinc-300',
    Armchair
  ),
  createAsset(
    'wall-bookshelf',
    ...
    'cabinet',
    '展示收纳',
    'a tall open shelving unit',
    'warm built-in wood shelving with subtle display lighting',
    'storage_tall',
    'large',
    'from-stone-500 to-zinc-700',
    Boxes
  ),
  createAsset(
    'glass-wardrobe',
    ...
    'cabinet',
    '高柜整柜',
    'a tall wardrobe cabinet',
    'dark framed glass doors with refined storage proportions',
    'storage_tall',
    'large',
  ...
  createAsset(
    ...
    'bed',
    '卧室主床',
    'a double upholstered bed',
    'soft headboard, warm textile bedding, restful bedroom styling',
    'sleeping',
    'large',
  ...
  createAsset(
    ...
    'minimal recessed magnetic track lights, soft ambient glow',
    'ceiling_light',
    'large',
  ...
  createAsset(
    ...
    'finish',
    '背景材质',
    'wood veneer finish treatment',
    'warm wood slat feature wall and coordinated ceiling finish',
    'surface_finish',
    'large',
...
export default function AiSoftFurnishingPage() {
  ...
  const handleGeneratePreview = async () => {
    ...
    try {
      const res = await fetch('/api/ai/soft-furnishing/preview', {
        method: 'POST',
  ...
  const handleRender = async () => {
    if (!sourceImage || selectedItems.length === 0 || !previewData) {
      alert('请先生成摆位预览');
      return;
    }
    ...
    try {
      const res = await fetch('/api/ai/soft-furnishing/render', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          image: sourceImage,
      ...
      if (data.data?.imageUrl) {
        ...
        router.push(`/ai-studio/floor-plan/${data.data.id}`);
      }
    } catch (error) {
      console.error(error);
      alert('渲染提交失败，请稍后重试');
  ...
  const roomKindLabel =
    previewData?.sceneAnalysis.roomKind === 'bedroom'
      ? '自动识别：卧室空间'
      : previewData?.sceneAnalysis.roomKind === 'living_room'
        ? '自动识别：客厅空间'
        : '等待识别';
        ...
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              上传正视角空房或毛坯现场图，选择家具类型，系统会先自动生成摆位预览，再基于 Tensor 输出结构更稳定的软装效果图。
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-5 xl:grid-cols-[1fr_390px]">
          <section className="rounded-2xl bg-white p-6 shadow-sm">
            <div className="mb-5 text-center">
              <h2 className="text-xl font-bold">自动摆位预览</h2>
              <p className="mt-2 text-sm text-muted-foreground">
                第一步先让系统推断家具布局，确认结构和摆位合理后，再提交最终渲染。
              </p>
            </div>
              ...
              <div className="space-y-4">
                <div className="grid gap-4 xl:grid-cols-2">
                  <div className="overflow-hidden rounded-2xl border bg-slate-50">
                    <div className="flex items-center justify-between border-b bg-white px-4 py-3">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">结构基准图</p>
                      </div>
                  ...
                  <div className="overflow-hidden rounded-2xl border bg-slate-50">
                    <div className="flex items-center justify-between border-b bg-white px-4 py-3">
                      <div>
                        <p className="text-sm font-bold">自动摆位预览</p>
                        <p className="text-xs text-muted-foreground">{roomKindLabel}</p>
                      </div>
                      {previewData && (
                        <Badge className="border-none bg-amber-100 text-amber-700">
                          {previewData.placementPlan.items.length} 个摆位锚点
                        </Badge>
                      )}
                    </div>
                    {previewData ? (
                      <img src={previewData.placementGuideImage} alt="摆位预览" className="mx-auto max-h-[560px] w-full object-contain" />
                    ) : (
                      <div className="flex min-h-[420px] flex-col items-center justify-center gap-3 px-10 text-center text-muted-foreground">
                        <Wand2 size={32} />
                        <div>
                          <p className="font-bold text-foreground">还没有摆位预览</p>
                          <p className="mt-1 text-sm">选好家具类型后，先生成摆位预览，确认 AI 会把家具放在什么区域。</p>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                {generatedImage && (
                  <div className="overflow-hidden rounded-2xl border bg-slate-50">
                    ...
                    <img src={generatedImage} alt="软装效果图" className="mx-auto max-h-[600px] w-full object-contain" />
                  </div>
                )}
              </div>
            )}
          ...
          <aside className="rounded-2xl bg-white p-5 shadow-sm">
            ...
            <div className="mt-5 rounded-2xl border bg-slate-50 p-4">
              <div className="mb-2 text-sm font-bold">当前识别策略</div>
              <p className="text-xs leading-6 text-muted-foreground">
                V1 仅支持单点透视、正视角、空房或近空房现场图。系统会先自动判断更像客厅还是卧室，再按家具类型推断摆位方案。
              </p>
            </div>
            ...
            <div className="mt-7 grid grid-cols-2 gap-3">
              ...
              <Button
                ...
                disabled={isPreviewLoading || !sourceImage || selectedItems.length === 0}
                onClick={handleGeneratePreview}
              >
                {isPreviewLoading ? <Loader2 size={18} className="mr-2 animate-spin" /> : <Wand2 size={18} className="mr-2" />}
                {previewData ? '重新生成预览' : '生成摆位预览'}
              </Button>
            </div>
            ...
            <div className="mt-3">
              <Button
                ...
                disabled={isRendering || !previewData}
