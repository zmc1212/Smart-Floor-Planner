...
export async function GET(request: Request) {
  try {
    ...
    const result = await withMiniProgramPostgresTransaction(context, async (transaction) => new AcquisitionRepository(transaction).listNotifications(parsePostgresId(context.staff!._id, 'staff id'), onlyUnread));
...
export async function POST(request: Request) {
  try {
    ...
    const marked = await withMiniProgramPostgresTransaction(context, (transaction) => new AcquisitionRepository(transaction).markNotificationsRead(ids, parsePostgresId(context.staff!._id, 'staff id')));
