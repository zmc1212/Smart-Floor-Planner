...
export async function GET(request: Request) {
  try {
    ...
    const result = await withMiniProgramPostgresTransaction(context, (transaction) =>
      new ReferrerPortalRepository(transaction).listProgress(
    ...
    return NextResponse.json({ success: false, code: (error as { code?: string }).code, error: error instanceof Error ? error.message : '读取推荐服务进度失败' }, { status });
  }
}
