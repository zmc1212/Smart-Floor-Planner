...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!body.stageKey) return NextResponse.json({ success: false, error: 'Missing stageKey' }, { status: 400 });
    if (body.confirmed === false) {
      const stage = getWorkflowStageDefinition(body.stageKey);
      return NextResponse.json({
        success: true,
        data: { requiresConfirmation: true, message: `Executing ${stage?.name || body.stageKey} consumes enterprise AI credits.` },
      });
    }
    ...
    await submitPostgresCreationGeneration({ enterpriseId: context.enterpriseId, generationId: generation.id.toString() });
    const workflowContext = await getPostgresAiWorkflowContext({ enterpriseId: context.enterpriseId, workflowId: id });
    ...
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : 'Failed to run workflow stage', code: conflict.code, existingGenerationId: conflict.generationId },
