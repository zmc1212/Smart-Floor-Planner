...
export async function GET(request: Request) {
  try {
    ...
    const data = await withMiniProgramPostgresTransaction(context, async (transaction) => {
      ...
      if (context.mode === 'staff' && context.staff?.role === 'designer' && lead?.assignedTo !== BigInt(context.staff._id)) {
        return null;
      }
      if (context.mode === 'staff' && context.staff?.role === 'measurer' && lead?.measurerId !== BigInt(context.staff._id)) {
        return null;
      }
