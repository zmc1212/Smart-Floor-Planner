...
export async function GET(request: Request) {
  try {
    ...
    const staffId = mini?.staff?.role === 'enterprise_admin' && targetStaffId ? parseAppointmentId(targetStaffId, '家装现场顾问') : mini?.staff ? BigInt(mini.staff._id) : targetStaffId ? parseAppointmentId(targetStaffId, '家装现场顾问') : undefined;
...
export async function POST(request: Request) {
  try {
    const mini = await resolveMiniProgramContext(request);
    if (!mini?.enterpriseId || mini.mode !== 'staff' || !mini.staff || !['designer', 'measurer', 'enterprise_admin'].includes(mini.staff.role)) return NextResponse.json({ success: false, error: '仅家装设计顾问、家装现场顾问或企业负责人可设置不可用时间' }, { status: 403 });
    const body = await request.json();
    const staffId = mini.staff.role === 'enterprise_admin' ? parseAppointmentId(body.staffId, '家装现场顾问') : BigInt(mini.staff._id);
...
export async function DELETE(request: Request) {
  try {
    const mini = await resolveMiniProgramContext(request);
    if (!mini?.enterpriseId || mini.mode !== 'staff' || !mini.staff || !['designer', 'measurer', 'enterprise_admin'].includes(mini.staff.role)) return NextResponse.json({ success: false, error: '仅家装设计顾问、家装现场顾问或企业负责人可删除不可用时间' }, { status: 403 });
