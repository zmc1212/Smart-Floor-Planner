...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string; workflowId: string }> }) {
  try {
    ...
    if (miniContext) {
      ...
      const result = await withMiniProgramPostgresTransaction(miniContext, async (transaction) => {
        ...
        return new CustomerProjectRepository(transaction).withdrawScheme({ enterpriseId, leadId, workflowId, withdrawnBy: staffId });
      });
      ...
      return NextResponse.json({ success: true, data: { workflowId: workflowId.toString(), withdrawnCount: result.publications.length } });
    }
