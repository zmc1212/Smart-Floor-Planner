...
export interface AcquisitionTaskOptions {
...
export interface AcquisitionTaskRecord {
  lead: LeadWithRelations;
  commission: AcquisitionCommissionWithRelations | null;
}
...
export class AcquisitionRepository {
  ...
  async listTasks(options: AcquisitionTaskOptions) {
    ...
    if (options.status === 'pending_confirmation') {
      filters.push(isNull(leads.acquiredAt));
    } else if (options.status === 'confirmed') {
      filters.push(isNotNull(leads.acquiredAt));
    }
    const timeColumn = options.status === 'confirmed' ? leads.acquiredAt : leads.createdAt;
    if (options.from) filters.push(gte(timeColumn, options.from));
    if (options.to) filters.push(lt(timeColumn, options.to));
    ...
    const [idRows, totalRows] = await Promise.all([
      this.transaction
        ...
        .from(leads)
        .where(where)
        .orderBy(desc(leads.acquiredAt), desc(leads.createdAt), desc(leads.id))
        .offset((page - 1) * limit)
        .limit(limit),
    ...
    if (!ids.length) {
      return { rows: [] as AcquisitionTaskRecord[], total: Number(totalRows[0]?.value ?? 0) };
    }

    const [leadRows, commissionRows] = await Promise.all([
      new LeadRepository(this.transaction).findByIds(ids),
      this.transaction
        .select()
    ...
    const attachedCommissions = await this.attachCommissions(commissionRows);
    const commissionMap = new Map(attachedCommissions.map((item) => [item.leadId, item]));
    return {
      rows: ids.flatMap((id) => {
        const lead = leadMap.get(id);
        return lead ? [{ lead, commission: commissionMap.get(id) ?? null }] : [];
      }),
      total: Number(totalRows[0]?.value ?? 0),
    };
  }

  async taskSummary(options: Pick<AcquisitionTaskOptions, 'role' | 'staffId'>, month: { start: Date; end: Date }) {
    const visibility = options.role === 'designer'
      ? eq(leads.assignedTo, options.staffId)
      : eq(leads.promoterId, options.staffId);
    const [pendingRows, confirmedRows, monthlyRows] = await Promise.all([
      this.transaction
        .select({ value: count() })
        .from(leads)
    ...
    const commissionSummary = options.role === 'measurer'
      ? await this.summary({ measurerId: options.staffId, status: 'pending_settlement' })
      : {};
    return {
      pendingCount: Number(pendingRows[0]?.value ?? 0),
      confirmedCount: Number(confirmedRows[0]?.value ?? 0),
      confirmedThisMonth: Number(monthlyRows[0]?.value ?? 0),
      pendingSettlementAmount: commissionSummary.pending_settlement?.amount ?? 0,
    };
  }
