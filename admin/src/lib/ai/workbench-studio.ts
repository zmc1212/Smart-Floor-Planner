...
export function workbenchFloorPlanPreviewPath(workflowId: string, roomId?: string, previewVersion?: string) {
  ...
  const trimmedPreviewVersion = typeof previewVersion === 'string' ? previewVersion.trim() : '';
  if (trimmedPreviewVersion) params.set('fp', trimmedPreviewVersion);
...
export function workbenchComposerControlPreviewUrl(workflowId: string, scopeSelection: string, previewVersion?: string) {
  const selection = scopeSelection.trim();
  const roomId = selection && selection !== WORKBENCH_WHOLE_FLOOR_SCOPE_KEY ? selection : '';
  return workbenchFloorPlanPreviewPath(workflowId, roomId, previewVersion);
}
