...
function findOverlappingWall(floor, start, end, options) {
  ...
  return floor.walls.find((wall) => { console.log('Checking wall', wall.id, start, end);
...
function getCursorPlacementTarget(floor, point, maxDistanceMm) {
  ...
  const innerVertexPreferenceRadiusMm = nearestVertex && !nearestVertex.snapLine
    ...
    : 0;
  if (nearestVertex && nearestVertex.distanceMm <= innerVertexPreferenceRadiusMm) {
...
function commitPreviewLength(draft, lengthMm, inputSource) {
  ...
  const isManualInput = inputSource !== 'preview' && inputSource !== 'closure-preview';
  const effectiveClosureToleranceMm = isManualInput
    ? CLOSE_TOLERANCE_MM + (Number(session.thicknessMm) || DEFAULT_THICKNESS_MM) * 2.5
    : CLOSE_TOLERANCE_MM;
  const isClosingCurrentSpace = activeStartNode &&
    (activeWallCountBeforeCommit >= 2 || !!closureProjection || !!outerFaceProjection) &&
    (closureProjection || outerFaceProjection || distanceMm(endPoint, activeStartNode) <= effectiveClosureToleranceMm);
  ...
  if (extendLastWall || shortenLastWall) {
    ...
    if (closureProjection) {
      endNode = getOrCreateSnapNode(floor, closureProjection);
    } else if (isClosingCurrentSpace && !outerFaceProjection && distanceMm(endPoint, activeStartNode) <= effectiveClosureToleranceMm) {
      endNode = activeStartNode;
      endPoint = { xMm: activeStartNode.xMm, yMm: activeStartNode.yMm };
    } else {
      endNode = addNode(floor, endPoint);
    }
  ...
  } else if (activeStartNode && activeWallCount >= 3 && distanceMm(endNode, activeStartNode) <= effectiveClosureToleranceMm) {
    session.state = 'closing';
    session.closeCandidateNodeId = activeStartNode.id;
    session.closeCandidateType = 'start';
  } else {
