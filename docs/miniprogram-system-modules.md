...
- The standalone production artwork mapping is `design-references/...market-v5.png` character treatment -> `miniprogram/images/designer-contact/xiao-k-peeking.png`; the dialog structure, copy, background, and controls remain native WXML/Less.
...
(`packages/business/free-design-service/free-design-service`) now uses the latest
approved `design-references/free-design-service-phone-auth-redesign-v1/free-design-service-phone-auth-v1.png`
for the phone-authorisation state while preserving the implemented claim
contract and the previously restored assignment-pending flow. The official
production lockup remains `/images/home-ip-v1/brand-logo.png` + **家客来** rather
than the mockup-generated logo. The phone-authorisation first viewport leads
with a **双重免费权益** pass for **免费量房 · 免费设计**, then explains the two
truthful uses (survey contact and designer WeChat delivery), the privacy limit,
**允许微信授权手机号**, and **暂不授权**. At normal/tall viewports, the benefit
pass and purpose card share remaining flex height; heights at or below `760px`
restore vertical scrolling. The standalone privacy-gatekeeper artwork is the
built-in-imagegen-produced transparent
`packages/business/assets/referral-service-v1/xiao-k-phone-privacy.png`
(`284x270` after alpha trimming, under `300KB`, F1 stepped body and doorway retained).
The visible character now occupies the approved hero area instead of being reduced by
transparent canvas padding. Five original, consistently stroked green PNG icons map the
approved tape-measure, design-pencil, calendar, chat, and privacy-shield meanings under
`packages/business/assets/referral-service-v1/phone-auth-*.png`; the phone-auth hero keeps the
approved `46rpx` scale while placing `授权手机号 / 领取免费量房 / 免费设计` on separate lines in a
`330rpx` copy column, with Xiao K shifted into its own right-side visual lane so the title and
subtitle remain unobscured. Benefit and section labels use `30rpx`, and supporting copy uses
`24–26rpx` to match the approved readable hierarchy. The page mockup is not sliced. The prior pending state keeps its spatial guide, benefit journey,
and viewport-fill behavior. Route, API, identity, claim-result, and navigation
boundaries are unchanged. Focused referral-service and transparent-asset tests
pass. The refreshed native `390x844` capture remains pending: the verified
DevTools automator accepted fresh compile/reconnect and returned `systemInfo`,
but the phone-authorisation `setData` call still timed out after explicit
recompile and reconnect recovery, so the frozen frame was not retained as QA.
