...
The `packages/ai-workflow/scheme-studio/scheme-studio` collapsed state now
keeps its two mutually exclusive entry modes: a selected template shows its
compact template summary, **编辑**, complete control-image label, and compact
generate action; only a no-template, unconfigured draft shows the full-width,
solid same-hue green-gradient **开始新一轮设计** action. It opens the existing
round-setup chooser; generation APIs, render-mode selection, tenant scope, and
role boundaries are unchanged. The timeline rail and its state nodes use the
same center coordinate at standard and narrow viewports. A processing round
uses one rounded reply tail joined to its card, replacing the detached
rotated-diamond artifact. The fixed Composer dock now uses a pale-green
...
the toolbar; input, toolbar structure, generation, and keyboard-safe behavior
are unchanged. Runtime visual QA remains pending the owner's
`390x844` captures for both completed and processing rounds.
...
| AI design workflow | `pages/ai-design/ai-design`, `packages/ai-workflow/*` | The Design tab and recipe-project remain the creation entry. `scheme-studio` follows the owner-approved five-state composer source `design-references/ai-design/unified-entry-v2/14-studio-manual-expanded-v1.png`: manual mode stays a compact editable prompt/control-image/generate row and expands to 手动创作 with 选择模板 and the full editor. Selecting a template now opens its **完整提示词** editor immediately; the template source/cover metadata and **恢复模板** remain available, but the separate **个性化调整** field, its labelled prompt composition, and its client submission parameter have been removed. The chosen template's `promptContent` (or legacy `internalPrompt`) replaces the prior draft directly; recipe detail responses expose that editable text as `promptContent` while keeping `internalPrompt` private, and templates without an editable prompt are rejected. Timeline, render mode, batch/API, tenant/permission, recipe, publication, retry, and modal contracts are unchanged. Runtime `390×844` visual QA remains pending a user-provided capture. | Implemented/Limited; provider, credits, eligibility, publication visibility and scope remain server controlled. No Admin canvas annotation editor or dark theme; exact approved-state runtime QA awaits a workflow with one completed and one processing batch plus user manual `390×844` composer captures. |
...
- Leads, floor plans, measurements, devices, AI, commissions, promotions, and
  ...
  Starting a new `scheme-studio` round now opens the approved `04-round-setup-v2.png` mode chooser instead of forcing it on page entry. Its content-intrinsic bottom sheet presents the same whole-plan / single-room choice, the truthful formal-plan input basis, an optional selected recipe, and the existing single-room photo requirement; its input action retains the tagged-site-photo picker. Its recipe **更换** action closes the round-setup sheet before opening the template sheet, then restores the same round-setup sheet after either template application or dismissal, so no two masks stack. The selected `renderMode`, input basis, draft, template metadata, batch/API, tenant, and permission contracts remain unchanged. The initial workflow detail and composer bootstrap load in parallel; same-customer workflow chips fill in after the first screen is usable. **设计整屋** sends `renderMode=whole_floor_plan`, keeps the formal floor-plan control image as the structural constraint, and treats optional tagged site photos as camera evidence. **设计单间** requires a tagged site photo, may optionally carry a selected closed-room identity when a formal plan is bound; without a room selection, it submits from the site photo alone, sends `renderMode=single_room_photo`, and omits the floor-plan control image; its nested **仅软装换搭** choice sends `renderMode=soft_furnishing` with the same photo-first input contract. The Mini batch endpoint forwards `renderMode`/`hasStyleReference`/`hasSitePhoto` plus exact `sitePhotoAssetIds` into `preparePostgresCreationBatch`; room identity remains persisted for photo-first modes without composing a floor-plan constraint prompt, and a generation continued as a baseline cannot later masquerade as a required site photo.
