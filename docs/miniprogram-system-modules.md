...
- The source package explicitly excludes development folders, tmp preview
  ...
  Main-package artwork under `images/` is palette-optimized in place. The
...
- Staff workbench: `GET /api/miniprogram/workbench` accepts `period=week|month|year|custom`
  ...
  artwork scene. The three generated flow cutouts and Hero cutout are packaged independently; `GET /api/miniprogram/workbench`
