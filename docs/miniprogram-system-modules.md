...
- Sessions use `/api/auth/miniprogram` and bearer JWT. `GET /api/miniprogram/bootstrap`
  ...
  Platform admins land on the device enroll workbench (`platform-device-workbench`):
...
start without a second tap. In guide mode that reset-cursor state immediately
...
- BLE integration is `miniprogram/utils/bluetooth.js`; protocol semantics come
  ...
  `POST /api/devices/verify-binding` (enterprise ownership). Platform enroll
  uses `scanBLEForEnrollment` (multi-device scan, MAC only, no connect) plus
  `GET/POST /api/miniprogram/devices` (batch assign via `devices[]`).
