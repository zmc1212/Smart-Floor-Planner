...
block on the opening face with a CCW 90° arc; sliding doors and windows are
double rails inside the opening. Dimensions are rotated linear entities from
the shared closed-space plan (`标注线-内墙` inner segments, `标注线` overall
lengths). Closed rooms write four-line labels from the inner-face polygon.
A cyan model-space title block and north-arrow wrap the side-by-side floors.
Company and designer come from the linked customer lead; customer phone and
address are omitted. The app saves the DXF to its
