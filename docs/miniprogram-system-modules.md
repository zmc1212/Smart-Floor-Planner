...
### Current enterprise-owner workbench and operations contract
...
The current approved workbench source is `design-references/enterprise-owner-workbench-density-v2/comp-b-with-operations-reminder.png`.
`pages/index/index` is the enterprise-owner **工作台**: the compact green dispatch Hero is followed by one white **获客与团队** hub containing the tall left activity-code card, stacked right **邀请入驻** / **查看推广人** actions, the contextual **查看预约安排** row, the B+C **经营提醒** strip, and the two blue/purple insight cards. The reminder uses only live workbench rows for `newLeads` and `signedCount` (with their returned units), never the comp's illustrative values, and opens the dedicated **经营** Tab through `switchTab`. The priority tray remains a separate white grouped container with independent alert and CTA geometry. **查看预约安排** still `navigateTo`s the `enterprise.appointments`-protected schedule as a TabBar-free secondary page with 本周/本月/本年 plus custom range chips; a selected custom window's dates sit after the 自定义 chip (`8/1 ~ 8/31`, cross-year keeps years) rather than in the capsule-safe nav subtitle, which stays `自定义 N 单预约`. The current independent production-art mapping is `activity-code-share-v3.png` (592×464, 20,073 bytes), `team-onboarding-v3.png` (480×384, 21,267 bytes), `referrer-roster-v2.png` (180×201, 10,011 bytes), `lead-inbox-v2.png` (240×239, 10,044 bytes), and `priority-alert-v2.png` (90×88, 3,347 bytes), under `miniprogram/images/operations-dashboard/`; all are transparent PNGs below the 300KB package limit and were generated as standalone cutouts, not sliced from the approved composite. `pages/enterprise-operations/enterprise-operations` is the dedicated **经营** Tab and follows the user-supplied 2026-08-28 composition: a capsule-safe pale-mint **本月经营大盘** Hero contains four period controls, the three live period KPIs, and owner-only `contractAmountSum` as **签约金额**. Its newly regenerated independent art mapping is `enterprise-hero-k-v2.png` (384×351, 83,900 bytes), `new-leads-kpi-v2.png` (192×192, 29,400 bytes), `completed-survey-kpi-v2.png` (192×192, 26,700 bytes), `signed-contract-kpi-v2.png` (192×192, 25,500 bytes), `contract-amount-kpi-v2.png` (192×192, 30,100 bytes), and `operations-growth-chart-v2.png` (176×161, 16,100 bytes), all under `miniprogram/images/operations-dashboard/`. These are standalone RGBA PNGs below the 300KB limit, regenerated from the approved visual language rather than cropped from the composite. The Hero artwork was increased to its source-calibrated display scale. The white **经营闭环** is a three-stage flow, followed by two real efficiency cards. `GET /api/miniprogram/workbench?period=` now also returns owner-only `contractAmountTrend`: Shanghai-time daily buckets for week/month/custom ranges up to 62 days and monthly buckets otherwise, with a same-length previous comparable range. The `签约金额趋势（万元）` canvas renders those two real series and uses the empty state only when both return zero. The route remains in native `tabBar.list` so the custom TabBar can `switchTab` to it. The owner TabBar is **工作台 / 经营 / 客户 / 提成 / 我的**; API/data behavior is otherwise unchanged, and the additive amount and series are restricted to enterprise owners. Authenticated `390x844` runtime QA remains pending manual review.
...
Covered: `enterprise-staff`, `enterprise-referrers`, `referrer-progress`, `referrer-earnings`, `staff-earnings`, `enterprise-commissions` (`status`), `commission-records` (`status`), `promotion-records` (`search`), platform `enterprises` / `devices` (device rows only), lead-detail `assignable-staff`, and recipe-project `GET /api/miniprogram/ai/studio/leads`. Not paginated: `customer-projects` ranking index, workbench preview caps, identity-contexts, calendar/day appointments, BLE scan session, the devices enterprise picker, and overdue todos (`GET /api/workbench/todos`, still internally capped at 200).
...
Floor-plan writes keep the formal-v4 400 envelope gate. Drafts run `quick` validation; completed plans run enhanced `full` validation and require at least one closed Space before any database write or preview generation. Invalid topology returns 422 with the first error code/message plus `validation.mode/errors/stats`, and the server never repairs the submitted graph. Full validation rejects proper crossings, unsplit T endpoints, coincident endpoints owned by different node IDs, and positive-length collinear overlaps; it uses integer-millimetre centerlines and the geometry epsilon, never the 350 mm snap tolerance. Manual and BLE remeasurement use the same full immutable transaction, so an invalid edit rolls back graph, rooms, openings, history, and draft rather than auto-splitting walls.
...
- Staff workbench: `GET /api/miniprogram/workbench` accepts `period=week|month|year|custom`
  ...
  For `enterprise_admin`, `pages/index/index` is the **工作台** for acquisition, referral-roster entry,
  live dispatch insights, and the existing **查看预约安排** contextual jump. The new
  `pages/enterprise-operations/enterprise-operations` is the separate **经营** Tab for the same API's
  period-filtered full-store dashboard and business loop. Its approved pale-mint Hero keeps the title, four native
  period controls, three live KPIs, and the owner-only `contractAmountSum` contract amount inside one capsule-safe
  artwork scene. The amount is compacted as separate native number/unit text (`5万元`, `10万元`, `100万元`; large totals move to `亿元`/`万亿元`) instead of rendering the full yuan value. Hero KPI figures use `44rpx`; its number reduces to `36rpx` only for a three-or-more-character compact value, while the following unit remains `26rpx`, so every supported range stays inside the narrow tile. The three generated flow cutouts and Hero cutout are packaged independently; `GET /api/miniprogram/workbench`
  supplies owner-only `contractAmountTrend` as Shanghai-time current/previous daily or monthly contract-amount buckets,
  which the trend card renders without synthetic values. The tall-device optical calibration enlarges the Hero art,
  KPI tiles, flow nodes, efficiency cards, trend canvas, and all operational text through intrinsic content dimensions.
  It is listed in native `tabBar.list`
  so custom TabBar taps `switchTab` rather than failing silently. The owner TabBar is **工作台 / 经营 /
  客户 / 提成 / 我的**: the existing customer route remains, while Appointments is no longer a fixed Tab and
  keeps its `enterprise.appointments` route/capability boundary.
...
- Referrer network: enterprise join-code PNG/JPEGs open the dedicated onboarding
  ...
  Owner referrer roster entries: the operations Hero 已入驻推荐人 row, Mine action 已入驻推荐人,
  and a 查看已入驻推荐人 link on both join-code tabs (not only 推荐人) into owner-only
  `packages/business/enterprise-referrers/enterprise-referrers` (`enterprise.operations`).
  `GET /api/miniprogram/enterprise-referrers` (`query`, `status`) and `POST /api/miniprogram/enterprise-referrers/[id]/disable`
  reuse `ReferrerNetworkRepository` with Mini JWT `requireMiniProgramEnterpriseAdmin` (designer/measurer/referrer/customer 403).
  Disable is idempotent, has no re-enable, and does not change historical leads or commissions; a later join-code scan can create a new `active` membership row.
  Roster cards: disabled/exited rows place the packaged phone glyph on the number line and omit the action footer; active rows keep left **电话联系** and right **停用后续扫码**.
