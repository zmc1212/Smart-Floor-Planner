...
start without a second tap. During that wall-drop wait (`wallSnapPending`), the
canvas still pans and pinch-zooms; only a short tap on a wall or vertex places
the cursor. In guide mode that reset-cursor state immediately
...
- Referrer network: enterprise join-code PNG/JPEGs open the dedicated onboarding
  ...
  lets the error-state QR stage grow so the CTA stays inside the white card.
