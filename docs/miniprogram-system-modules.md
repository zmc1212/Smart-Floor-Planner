...
### Password-login first-change restriction

`packages/business/login/login` and `packages/business/account-security/account-security` now implement the current password-login contract. `/api/auth/miniprogram` returns `requiresPasswordChange` only for password-authenticated flagged staff and signs that restriction into the token. The login page immediately reLaunches account security without normal bootstrap. A restricted token may read the profile, update the account password, or log out; all other Mini Program APIs return `password_change_required`. `PUT /api/miniprogram/account/password` clears `admin_users.must_change_password`; the existing page then clears local session and requires login with the new password. WeChat authorization and phone quick login do not trigger this restriction. Shared password matching reports `invalid_credentials` or, after filtering by password, explicit `ambiguous_identifier`. Existing sessions are not proactively revoked; the latest flag is acquired at token refresh or the next password login. Status: Implemented. Design source remains `design-references/miniprogram-airy-minimalist-v1/30-account-security.jpg`; the layout is unchanged and authenticated `390x844` runtime confirmation is pending.
...
- Referrer network: enterprise join-code PNG/JPEGs open the dedicated onboarding
  ...
  can enable「联系当前家装设计顾问」, and the resolve existing-attribution lead includes
