...
For an isolated orthogonal chain with no openings, shared nodes, or branches, a return-to-start residual inside the existing close tolerance is balanced over same-axis walls by measured-length weight. Preview and confirmation use the same plan, keep segment directions and minimum lengths, and project the adjusted chain before offering closure; any new non-adjacent intersection, overlap, or contact with another wall rejects the plan. Confirmation stores each raw reading and its derived closure correction separately, and later endpoint inset, collinear merge, and wall split operations preserve that traceability.
...
canvas still pans and pinch-zooms, and the projector can apply a page-level view-only `rotationRad` that is never persisted; a wall tap only selects the wall for opening
...
- Referrer network: enterprise join-code PNG/JPEGs open the dedicated onboarding
  ...
  **推广网络** uses `view=network` with enterprise totals, every employee branch
  including zero-referrer employees, and an optional historical-unassigned branch;
  **全部推广人** keeps `query`/`status`/`page`/`limit` pagination. Only owners may call
  ...
  Disabled/exited cards keep the packaged phone glyph on the number line and omit
  the action footer; active owner cards retain **电话联系** and **停用后续扫码**.
