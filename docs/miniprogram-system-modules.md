...
| Referrer network, appointments, and anonymous claim | `packages/business/referrer-workbench/referrer-workbench`, `packages/business/promotion-service-code/promotion-service-code`, `packages/business/free-design-service/free-design-service`, `packages/business/customer-project/customer-project`, `packages/business/appointment-reschedule/appointment-reschedule`, `packages/business/appointment-booking/appointment-booking`, `packages/business/measurer-calendar/measurer-calendar`, `packages/business/measurer-unavailability/measurer-unavailability` | The internal referrer workbench lists active enterprise memberships, selects one, enters its protected service-code route, and leaves it without changing historic attribution; existing anonymous-claim APIs; phase-5 appointment contracts and designer first booking, the customer card, rescheduling, measurer itinerary, and self-service unavailability editor routes are implemented. Phase 6 backend endpoints now provide an owner-only customer-project aggregate, protected published-design images, and designer/enterprise-admin publication or withdrawal; the customer-project page retains its approved appointment card and uses the aggregate only to populate the existing service rows with the actual designer and measurer. First booking and customer rescheduling use fixed full-width CTAs computed from the current window, avoiding native-button compression | Implemented/Limited; customer-project APIs enforce `customer_user_id` ownership and omit withdrawn/deleted designs; formal-plan/published-design UI awaits an approved design. Repository/RLS/concurrency tests pass and post-commit creation, rescheduling, and cancellation attempts deliver to staff and subscribed customers. WeChat delivery depends on external configuration and can be rejected. Every affected phase-5 route has a verified top route and full host-window capture including the native capsule in the actual iPhone 12/13 Pro `390x844` WeChat DevTools simulator |
