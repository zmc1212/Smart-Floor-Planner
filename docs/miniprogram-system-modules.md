...
Identity-switch visual treatment: `packages/business/identity-switch/identity-switch` follows the user-selected role-tile reference `design-references/identity-switch-role-tiles-v1/identity-switch-role-tiles-v1.png`. It uses native layout only: one central Xiao K preview for the selected identity, followed by a horizontally scrollable rail of larger vertical role tiles and the existing confirmation CTA. The redundant top「选择你的角色」label is omitted. Adjacent role previews are deliberately omitted so the selected character remains the only visual focus. Each rail Xiao K is `118rpx` inside a `148rpx × 178rpx` tile, keeping character details legible without relying on a composite stage image. The non-designer role icons have their lowermost light ground residue cleared to transparency while preserving the character pixels. The disabled current-identity CTA uses an opaque mint fill, green border and dark-green text to remain distinguishable from the page background. Tapping a role previews it locally; only a non-current selection exposes the existing native confirmation and signed-token switch. A single valid context shows the preview but no meaningless switch action. The existing `GET /api/miniprogram/identity-contexts` / `POST /api/miniprogram/identity-contexts/switch`, route, and permission boundaries are unchanged. Each available `customer`, `referrer`, `enterprise_admin`, `designer`, `measurer`, `salesperson`, and `platform_admin` context resolves to the locally packaged, role-specific Xiao K icon; unsupported legacy roles retain the customer-guide fallback and do not gain capabilities. No generated role-gallery-stage runtime image is packaged.
...
reticle 12×20 CSS px upper-left of the finger and clamp the aim point to the
canvas; snap, the corner magnifier, and release all use the aim point, not the
finger pad. Dock drag coalesces snap, wall-guide, and lens blit onto one
canvas animation frame so cover-view touchmove does not stall the overlay. Canvas wall-endpoint drags keep a sticky grab delta from
...
- Referrer network: enterprise join-code PNG/JPEGs open the dedicated onboarding
  ...
  so stack-root custom-nav landings can return to the role home. Promotion and
  staff-activity presenters use the approved QR-first credential source
  `design-references/service-code-redesign/free-design-service-qr-focus-v3.jpg`: the protected code
  is the dominant target (`520rpx` at the `390px` baseline and `508rpx` on
  `<=360px` narrow screens), the QR group follows the heading instead of
  centering inside leftover flex height, the promises share one mint credential, and the
  `open-type="share"` CTA says「分享给客户」while keeping the existing claim-route
  token target. The dual join-code page retains its Design 09「一键分享」CTA on an
  ...
  restores design-14 with a complete packaged `thumbs-up-xiao-k.png`, a compact
  three-stage service path, clipboard service card, and in-flow retention note;
  its content is scrollable normal flow rather than a bottom spacer. The stage
  index derives only from existing lead status/service-stage labels. That page
  ...
  can enable「联系当前设计师」, and the resolve existing-attribution lead includes
...
Designers and measurers can maintain their unlocked professional title, career start year, and title preference from `profile-edit` through `GET/PATCH /api/miniprogram/staff/professional-profile`; enterprise-forced visibility disables the employee switch with an explanation, and a locked profile disables all professional edits. The page previews the final customer-visible title/experience/service labels and shows the raw count only to the staff member. Customer project and claim DTOs expose only the resolved `professionalProfile`. The designer contact sheet inserts that proof between identity and the unchanged QR stage: a visible title shares the credential heading with the designer name, while the experience and service labels use licensed semantic icons. Hiding the title removes that heading and restores the name to the relationship pill so identity is never lost. The V4 service archive uses the separate user-approved fixed designer/measurer proof recorded below; no measurer QR capability is introduced.

- `components/designer-contact-sheet` now follows the user-approved `design-references/designer-contact-sheet/designer-contact-sheet-professional-card-v7.png` across `pages/index/index`, `packages/business/free-design-service/free-design-service`, and `packages/business/customer-project/customer-project`.
...
- The standalone production artwork mapping is the retained Xiao K treatment -> `miniprogram/images/designer-contact/xiao-k-peeking.png`; the v7 medal, compass, and customer-heart semantics map from `docs/icon-sources/designer-contact/` to three optimized PNGs under `miniprogram/images/designer-contact/`. The dialog structure, data-driven copy, background, and controls remain native WXML/Less.
- `packages/business/free-design-service/free-design-service` now follows `docs/superpowers/specs/2026-08-25-free-design-service-contact-conversion-design.md`: a newly assigned success result with any usable designer contact opens the shared sheet once; dismissal reveals a compact matched-designer summary, solid-green `查看服务档案`, secondary outlined `查看设计师微信`, and the existing weak service-needs entry. The neutral contact label remains correct whether or not the customer already completed the unobservable friend request. The page no longer downloads or renders a duplicate inline QR. QR-only and WeChat-ID-only contacts share the same dialog; absent contact data remains an honest disabled synchronization state. Routes, APIs, claim idempotency, permissions, and role boundaries are unchanged.
...
archive book. All customer booking controls now say `预约量房` / `重新预约`; assigned staff
show the user-approved fixed proof `金牌设计师 / 7年设计经验 / 已免费服务客户100+` and
`资深测量师 / 7年量房经验 / 已免费服务客户100+`. The first user-supplied tall-device runtime
...
**免费设计顾问 / 解答你的装修问题**, and **免费现场顾问 / 解答现场问题**, followed
...
(`560x473` RGBA PNG, `201267` bytes, F1 stepped body and complete black limbs retained).
...
and icon scale. Pending,
success, and existing-attribution behavior, routes, APIs, identity, claim results,
permissions, and navigation boundaries are unchanged. Focused referral-service and
transparent-asset tests guard the intrinsic-flow contract. Manual runtime visual QA
is accepted under the project rule; no WeChat DevTools capture was used.
...
The `free-design-service` existing state now uses the complete packaged `thumbs-up-xiao-k.png` Xiao K, a compact three-stage service path, and an in-flow retention note. Content uses normal-flow scrolling instead of a bottom spacer; the stage index derives only from existing lead status/service-stage labels, while `GET /api/miniprogram/customer-projects/[leadId]`, contact permissions, and button behavior remain unchanged.
