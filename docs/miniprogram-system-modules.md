...
- Referrer network: enterprise join-code PNG/JPEGs open the dedicated onboarding
  ...
  retention banner; that page hydrates `GET /miniprogram/customer-projects/[leadId]`
...
`packages/business/assets/referral-service-v1/phone-auth-*.png`; required copy uses the
approved `46rpx` hero, `28rpx` section/benefit labels, and `22rpx` helper floor. The page mockup
is not sliced. The prior pending state keeps its spatial guide, benefit journey,
