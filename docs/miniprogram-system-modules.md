...
- Sessions use `/api/auth/miniprogram` and bearer JWT. `GET /api/miniprogram/bootstrap`
  ...
  context. Only `designer`, `measurer`, and `enterprise_admin` staff contexts
  are Mini Program workbench roles. A platform-only `admin` or `super_admin`
  context instead selects the same user's active referrer context, or the
  customer context when no referrer membership exists; it never receives
  enterprise Mini Program permissions. Refresh work is token-versioned, so a
...
`POST /api/miniprogram/codes/resolve` recognizes platform `er_` open-account tokens (including bare 32-char scenes) as `{ kind: 'enterprise_registration', displayName: '家客来企业入驻', valid: true }`. `POST /api/miniprogram/enterprise-registration` requires a Bearer JWT whose authorized phone exactly matches `contactPerson.phone`, validates the active `er_` code, and creates a `pending_approval` / `self_service` enterprise through the same `createSelfServiceEnterpriseApplication` helper as Web `/api/auth/register-enterprise`. Platform review/ops for that application live on Admin `/enterprises` (`POST /api/admin/enterprises/[id]/status`); non-`active` enterprises are not usable as Mini Program staff/referrer workbench contexts. The scan landing is not a sticky home: ready/success/error/recovery expose **去登录** (`reLaunch` to password login after clearing the incidental phone-auth customer session); `ACCOUNT_CONFLICT` after approval is an already-account exit; a signed workbench identity leaves to role landing instead of staying on the form. Status: API and `packages/business/enterprise-register/enterprise-register` Implemented; focused contract tests cover scene/`er_` restore, resolve-before-phone, phone-match submit, and the login/workbench exit. Authenticated `390x844` native-capsule visual QA remains pending. After platform approval the contact phone logs in as `enterprise_admin` with initial password `123456`. Approval SMS and in-app progress lookup remain out of scope.
...
inset + start extension - end inset`. Closed-room Canvas dimension lanes include
wall-thickness ticks, keep recessed L-notch spans on the local face, and sit
...
- Referrer network: enterprise join-code PNG/JPEGs open the dedicated onboarding
  ...
  state (skipping the historical confirm screen). Existing in-flight attribution
...
- Leads, floor plans, measurements, devices, AI, commissions, promotions, and
  ...
  category+search filtering, cover enlarge-before-apply, and a floating
  ...
  **creation entry**; `recipe-project` is the apply-to picker (default
  `whole_floor_plan`, or `single_room` with required `roomId`); `scheme-studio`
  is the scheme archive and continuation. Closed-room options come from the bound
  formal v4 survey graph (`adaptSurveyGraphToRooms` on `GET /api/miniprogram/ai/sources`).
  ...
  `scheme-studio` continuation currently omits `targetScope`/`roomId`, so those
  rounds default to `whole_floor_plan`; the apply-to picker remains on
  `recipe-project` and the Admin workbench. Appointment
