...
- Referrer network: enterprise join-code PNG/JPEGs open the dedicated onboarding
  ...
  staff-activity presenter may still show the enterprise name. Existing in-flight attribution
...
- Leads, floor plans, measurements, devices, AI, commissions, promotions, and
  ...
  category+search filtering, cover enlarge-before-apply (prompt and recommended model only; the cover is not cloned as a reference image), and a floating
  ...
  **creation entry**; `recipe-project` follows Admin workbench selection (customer,
  ...
  `single_room` with required `roomId`); `scheme-studio` is the scheme archive
  and continuation. Closed-room options come from the scheme-bound formal v4
