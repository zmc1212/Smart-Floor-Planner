...
## 2026-09-03 Scheme re-edit reference preview correction

When `scheme-studio` reopens a timeline batch through **重新编辑**, the draft
now preserves the signed preview URLs returned in `batch.referenceAssets` while
retaining the persisted reference ids and site-photo role classification. The
configuration sheet therefore renders carried-over reference/effect images
instead of blank placeholders. Studio routes, APIs, tenant scope, permissions,
and generation payloads are unchanged; focused composer tests cover the
restored URLs.

## 2026-09-03 Scheme finalization feedback correction

In `packages/ai-workflow/scheme-studio/scheme-studio`, finalization is now shown
once at the workflow level instead of once per completed round. Unpublished
workflows show the prerequisite hint, while published workflows expose the
scheme-level action/state. The existing action gives explicit feedback when
called without an active customer-visible publication or after finalization.
Finalization writes `leadLifecycleEvents` (`scheme_finalized` and
`scheme_unfinalized`) and withdrawing the last active publication clears the
finalized pointer. The finalization API, tenant scope, role boundary, and
generation workflow remain compatible; finalized means customer-primary and
does not freeze later generation.

## 2026-09-02 AI Design entry visual calibration

The scheme-studio configuration sheet keeps **保存配置** in a fixed footer visible on the initial viewport; only the configuration content scrolls when it overflows.

`pages/ai-design/ai-design` remains the V2 creation entry at
`design-references/ai-design/unified-entry-v2/01-design-tab-v2.png` with the
existing real project/recipe data, APIs, permissions, and six-item designer
navigation unchanged. Its native WXML/Less composition now calibrates the
project Hero to `480rpx`, uses a `1.32fr/1fr` primary/secondary action ratio,
and source-matched `88rpx` customer avatar plus `34rpx` project title. The
app-layer evidence is `output/playwright/ai-design-v2-fresh.png`; native
capsule runtime QA remains pending a fresh manual `390x844` capture.

## 2026-09-02 Scheme studio timeline alignment and round entry

The `packages/ai-workflow/scheme-studio/scheme-studio` collapsed state now uses
the entire left block as the **本轮配置入口**: it shows the template cover,
**选择模板**, the **设计整屋 · 参考图 1 张** summary, and an **调整** chevron;
the standalone **生成** action remains on the right. The collapsed input,
cost strip, and expand-edit row are removed. Tapping the left block opens the
existing round-configuration sheet, while the expanded state retains full prompt
editing. Generation APIs, render-mode selection, tenant scope, and role
boundaries are unchanged. The timeline rail and its state nodes use the
same center coordinate at standard and narrow viewports. The owner's latest
After generation submission, the Composer dock collapses and its dim mask is
hidden. Timeline cards expose the selected template cover and every persisted
reference asset for the batch, with unified preview behavior.
tall-device capture exposed the processing card's rounded tail as an oversized
hook around the timeline node. The processing bubble now uses a short,
two-layer outlined triangular tail attached to the card edge, with a clear gap
to the node; refreshed manual runtime QA remains pending. The fixed Composer dock now uses a pale-green
gradient surface with a large rounded top edge and translucent white inner
editing surfaces, so it remains visually distinct from the white timeline;
the expanded template editor now labels its existing collapse action **收起**;
the bottom generation action is slightly shorter and explicitly centered within
the toolbar; expanded-state input, toolbar structure, generation, and
keyboard-safe behavior are unchanged. The completed round still awaits the owner's `390x844` capture;
the processing round additionally awaits a refreshed capture after the tail correction.

## 2026-09-02 AI recipe picker survey eligibility correction

The `packages/ai-workflow/recipe-project/recipe-project` picker now treats a
lead as designable when its API payload contains at least one completed formal
v4 floor plan with a closed space, even if the appointment workflow is still
`survey_ready`. The studio-leads API includes the lead's `primaryFloorPlanRecord`
as a fallback for historical links, deduplicates it with linked plans, and keeps
the existing tenant, role, and route boundaries unchanged.

## 2026-09-02 AI recipe-project V2 visual restoration

`packages/ai-workflow/recipe-project/recipe-project` now implements the two
approved V2 compositions in one route: `02-project-picker-v2` before a customer
is confirmed and `03-recipe-bind-v2` after it. Both states expose a capsule-safe
back control; selecting a customer first marks
the card and enables the bottom CTA; the existing survey detour, workflow list,
new-workflow creation, and `scheme-studio` handoff retain their API, tenant, and
role contracts. The picker reuses the selected recipe's existing standalone
preview as its original clear cover (without blur, opacity, scale, or wash) and customer-card fallback; a local translucent heading plate preserves title contrast, so no composite
design frame or new main-package artwork is shipped. Runtime visual QA is pending owner-provided
`390×844` captures with the native WeChat capsule for both states.
...
## 2026-09-02 AI design entry action and icon correction

The approved `unified-entry-v2` Design tab now separates the two recent-project actions: **开始新一轮** opens the existing `scheme-studio` with `startNewRound=1`, which auto-opens the approved round-setup chooser; **继续上次创作** opens the same workflow without that flag and resumes the current composer state. The route, API, permission, and tenant boundaries are unchanged. The Design tab, project picker, and recipe binding surfaces now use the registered transparent PNG icon family under `miniprogram/images/ai-studio-icons-v3/` for navigation, generation, refresh, scheme, add, check, and info actions instead of CSS-drawn glyphs; Hero action glyphs are constrained to `30rpx` so transparent artwork cannot overflow the buttons. Runtime visual QA at the required `390x844` native-capsule viewport remains pending a user-provided capture.

## 2026-09-01 AI design unified-entry restoration

The AI design surface now follows the approved `design-references/ai-design/unified-entry-v1/` five-screen composition: the Design tab starts from a customer project, the project picker is a capsule-safe bottom sheet, recipe binding presents customer and scheme together, and the scheme studio keeps the preparation/mode chooser and unified composer hierarchy. Existing Studio, recipe, tenant, permission, and generation APIs remain unchanged; this is a native WXML/Less restoration using packaged assets, with authenticated `390x844` runtime visual QA pending the user's manual screenshot.
The prototype's exact standalone design cuts are packaged under `miniprogram/images/ai-design/unified-entry-v1/`: `cad-floorplan-archive.jpg` maps to formal-plan/project previews, `scheme-wood-cream-showcase.jpg` maps to in-progress project/results, and `recipe-modern-minimal.jpg` plus `recipe-cream-interior.jpg` map to recipe covers and the selected-recipe summary. The existing Xiao K and whole-house/single-room boards retain their approved packaged paths. No composite screenshot is used as a runtime asset.
The owner's five manual tall-device captures (`67b515…`, `a57211…`, `74d638…`, `5161d6…`, `648d10…`) are recorded as failed visual evidence, not an approval: they exposed the old recipe-detail route, a separate customer-picker page, an extra recipe waterfall, three recent-project rows, and oversized mode/studio content. The primary and history recipe actions now enter `recipe-project` directly; `recipe-detail` is redirect-only compatibility; the binding route keeps recipe, customer, and scheme in one page; the Design tab renders two recent projects and two recipe starters; and the setup/studio geometry is source-calibrated. Fresh manual runtime captures are still required to close visual QA.
...
- The source package explicitly excludes development folders, tmp preview
  ...
  4B snapshot was 4,595 lines / 116 top-level functions; at Phase 4C
  measurement extraction the kernel was 4,319 lines / 110 top-level functions.
  All 64 legacy and 69 facade exports remain compatible. The Phase 4C
  40-module / 158-edge audit verifies 43 mirrors.
  Phase 4B acceptance was `cd miniprogram && npm run test:survey-kernel-phase4b`:
  ...
  persistence and atomic opening rejection. Closure writes were outside Phase 4B;
  Phase 4D now owns them. See the
...
### Navigation measurement view rotation

The absolute azimuth shown on the action (for example `231°`) is a reading and
an input to entry-door-relative direction mapping, never a direct canvas angle.
The relative bearing is hysteresis-snapped to `0°/90°/180°/270°`; only a `20°`
trigger delta changes the selected axis, and the canvas eases between axes over
`420ms` with an inverse visual sign so the plan counter-rotates against phone
movement; it cannot settle at a diagonal angle. The navigation label and live
bearing readout are both `28rpx` so the active state remains legible.
Native modal action taps are stopped at the cover-view boundary and Canvas touch
handlers ignore the visible/closing calibration state, so confirming a location
cannot select a wall direction underneath the dialog.
Navigation mode does not auto-lock a direction from its sensor callback; the
operator must tap a candidate arrow explicitly.
...
Designer overview navigation correction: a card with `serviceStage=design_published` and `leadId` now opens `packages/business/customer-ai-schemes/customer-ai-schemes?leadId=…&mode=staff`, the existing staff published-scheme viewer, before considering stale appointment/rebooking/survey actions. Both the card and its bubbling 查看方案 link use `openItem`. Explicit survey/booking buttons, other roles and dedicated task views keep their existing handlers. Status data, markup, styles, assets, APIs and server permissions are unchanged. Implemented; all 35 focused navigation/workbench tests pass, including six stale-action variants and role/focus regressions; manual runtime click verification remains pending.

The approved task-card alignment correction (owner screenshot `b54ff381e24ac67a48f89493b6401065.jpg` and explicitly approved layout) is Implemented in the designer/measurer overview list in `components/role-workbench`: status badges share the full-width title row and right inset, long customer names ellipsize, badges never shrink or wrap, and fallback actions such as 查看方案 sit at the right of the bottom action row. Card padding remains `24rpx`, title-to-badge gap is `16rpx`, and the action row keeps its `14rpx` top gap and wraps when needed. Existing typography, artwork, status data, click handlers, routes, APIs and permissions are preserved. Verification: 27 of 28 selected tests passed and `git diff --check` passed; the sole failure is the existing AI-design 20rpx whitelist check in unchanged `pages/ai-design/ai-design.less` (four selectors), unrelated to these cards; runtime visual QA remains pending the owner’s manual `390x844` and tall-device screenshots including the native capsule.
...
`App.onLaunch` initializes the official WeChat `wx.getUpdateManager` once and
tracks `checking`/`downloading`/`ready`/`latest`/`failed` states. `onUpdateReady`
shows a restart prompt and calls `applyUpdate` after confirmation. The Mine
account panel exposes **检查当前版本** using the cached lifecycle result;
WeChat does not expose a developer-triggered `check()` method, so the button
cannot force a fresh network check. `getUpdateManager` is guarded for older
clients and unsupported runtimes. No route, API, permission, or tenant data
contract changes.
...
an inner closure guide. Beyond a closed-room corner, the closer physical outer
axis wins over the neighbouring inner rectangle axis. When that endpoint lies
on the returning boundary's outer extension, the suggested and confirmed path
follows that outer axis, then bridges to the topology corner; it does not extend
the last measured wall or offset the return wall from the shared wall's body.
Explicit manual/BLE length corrections retain their existing snap behavior and
do not switch an inner preview to an outer axis. Reference: the user's four
images in `design-references/surveying/closure-outer-alignment-20260904/`
(`01-drag.jpg` and `03-expected-close.jpg` define the expected behavior).
The production wall-union geometry in `tmp/survey-outer-alignment/closed-geometry.png`
matches the continuous junction in image 3. Twenty-five geometry regressions
cover 100/200/400 mm walls, rotations, mirrors, direct/committed closure, unchanged
readings and the original room; the editor regression covers release, explicit
close, and undo/redo. Updated `390x844` and tall-device runtime captures remain
pending the user's manual screenshots. Existing layout, artwork, routes, APIs,
permissions, measurement auditing and version-4 storage are unchanged; saved
rooms are not retroactively reshaped.
Verification: the complete Phase 7 gate passes (1,145 survey/editor tests,
39 Admin consumer tests, 55 H5 tests plus build, 79-file mirror/architecture
checks and performance thresholds). The full Mini Program run passes
1,634/1,648 tests; the remaining 14 failures exactly match the recorded
`surveying-module/legacy-kernel-phase0-baseline.md` list. `git diff --check`
and bilingual route/evidence checks pass.
...
The surveying editor synchronizes visible BLE state from the active session on `onShow`; its connect entry does not start a second discovery while that session exists. App-level silent reconnect notifications also call the editor's `updateBleConnected` hook.
...
Its `190rpx` high compact top action row keeps the `132×88rpx` Navigation
Measurement control fixed at the left without native `cover-view` clipping; its
`28rpx` label uses a `32rpx` line box so native glyph ascenders are not clipped;
Guide, Save, CAD, and Finish flex evenly through the remaining width with `12rpx`
sibling spacing and no wrapping, so every action remains fully visible at
`390x844` and narrower phone widths.
In the relocation dialog, **关闭导航测量** is a full-width `100% × 72rpx`
secondary action with a stable native hit area and pressed state; this prevents
`cover-view` from collapsing it into an unstyled text line.
...
Canvas renderer, dimensions, BLE readings, audit queue, undo/redo, and formal-survey
...
residual `spaceClosed` session to wall-drop, so reopening immediately shows
光标拖动到墙体 instead of requiring a reset tap. The formal canvas cursor and both dock states
...
no longer full-clear the overlay. The `132px` corner magnifier is now a square,
image-only crop with its dashed guides and centre crosshair; it no longer paints
snap-status or X/Y-coordinate captions. The live `L… / ∠…` metric pill shares
that row but shifts toward the canvas centre so neither overlay covers the other.
Canvas wall-endpoint drags keep a sticky grab delta from
...
canvas still pans and pinch-zooms. Its lightweight gesture frame keeps the current green reticle visible at the same transformed plan coordinate with a stable screen-space glyph size, providing a continuous zoom reference. The projector can apply a page-level view-only `rotationRad` that is never persisted. The top action row keeps a visible `132×88rpx` **Navigation Measurement** action in an independent far-left slot (technical component: `survey-canvas-compass`), while Guide / Save / CAD / Finish remain a fixed-`20rpx`-gap group aligned to the far right instead of being distributed across the row. Its native `24rpx` **导航测量** label uses the independently generated transparent `packages/surveying/assets/icons/navigation-measure.png` (128×128, 7,858 bytes), and the UI no longer renders `N`. Tapping it opens entry-door calibration. The operator stands inside the room, keeps the phone level and still with its top edge facing the entry door, and must connect the BLE distance meter before confirmation. Nine circular samples with no more than `6°` spread establish the entry-door's real compass azimuth; the active action displays the live absolute azimuth (for example `231°`) rather than rewriting it as `0°`. `surveyDeviceOrientation.sharedHeadingSensorHub` prefers Compass and falls back to DeviceMotion, while `sharedDeviceMotionHub` validates beta/gamma level when available. Heading subscriptions pause on `onHide`/`onUnload` and resume on `onShow` for an active logical mode. Straight-mode BLE quick input keeps blue dashed cardinal guides visible from the first wall and draws three or four compact, single-layer, translucent-green candidate pointers, excluding the active chain's backtrack. Once a bearing is selected, all other candidates disappear and only a small blue arrow remains on that direction's guide. When a pointer's `28px` touch radius overlaps an opening or wall, the object hit wins so the operator can still select the wall and use **复尺**; a short tap beside the reticle follows the same object-selection path, while a moved gesture still starts wall dragging. Tapping otherwise empty canvas space clears the transient bearing and restores the candidate pointers without moving the cursor or changing walls. The wall is still committed only after a valid BLE distance. Automatic pick waits for privacy authorization, converts the difference between the live absolute azimuth and the captured entry-door azimuth into the room-relative cardinal bearing, applies circular median plus activation/switch hysteresis, and rearms after each wall. Cardinal view changes rotate along the shortest arc over `420ms` with ease-out deceleration; tapping the active action reopens relocation or permits disabling navigation. `bleLockedBearingDeg`, entry-door calibration, and view rotation remain editor-session state and are stripped from local/cloud graph serialization. Dimension labels use world wall angle plus view rotation (`resolveScreenEffectiveAngle`); room cards stay screen-axis aligned; the grid rotates with the plan. Graph geometry, ortho snap, BLE protocol semantics, route/API/permission boundaries, and persisted viewport data are unchanged; a wall tap only selects the wall for opening
...
- Staff workbench: `GET /api/miniprogram/workbench` accepts `period=week|month|year|custom`
  ...
  marking converted removes the lead from that snapshot. All three professional overviews now use three
  compact intrinsic-width Hero summary rows with separate number badges, a `12rpx` title offset, and a
  `12rpx` row gap. Each row sizes from its own label plus formatted value rather than sharing a minimum
  width; the shared `stat.count` formatter compacts growing values to 千/万/亿 units before they can enter
  the mascot lane. The designer Hero uses the complete route-specific
  transparent `xiao-k-designer-workbench-v2.png` cutout, and the **客户线索 / 方案交付** helpers
  are explicit two-line native copy lanes that do not enter the illustration bounds. Period `已签约` remains
...
- Referrer network: enterprise join-code PNG/JPEGs open the dedicated onboarding
  ...
  **推广网络** uses `view=network` for summary-only employee branches (including
  ...
  enterprise-wide `query`/`status`/`page`/`limit` pagination. Any authorized staff
  member may call `POST /api/miniprogram/enterprise-referrers/[id]/disable`, but
  ordinary staff are server-scoped to memberships they invited; disable is
  idempotent, has no re-enable, and does not change historical leads or
  commissions. Their **我的推广人** cards provide **查看推广客户**、**电话联系**
  and active-member **停用后续扫码**. Disabled/exited cards keep the packaged
  phone glyph on the number line; every branch card exposes **查看推广客户**.
  **查看推广客户** writes
...
- Staff floor-plan preview on lead detail and the shared customer list: completed
  ...
  `POST /api/measurements` now uses the same linked-lead collaborator boundary
  for immutable audit writes, while unassigned and cross-enterprise staff remain
  denied and non-staff access remains creator-scoped.
...
- Leads, floor plans, measurements, devices, AI, commissions, promotions, and
  ...
  category+search filtering, cover enlarge-before-apply (prompt and recommended model only; the cover is not cloned as a reference image). User reference images on `recipe-confirm` / `scheme-studio` come from tag-first camera/album capture into the lead gallery or an existing 本户现场图 picker (selected `assetId` is used directly; no second copy). A locked **控制图** thumbnail in the scheme-studio reference row (bound whole-plan snapshot, or `?roomId=` crop matching the batch's first reference; `GET /api/miniprogram/ai/studio/workflows/[id]/floor-plan-preview` forwards that `roomId`), and the approved three-state mobile Composer with a mint-green arc, compact collapsed summary, expanded prompt editor, and reopenable configuration sheet. Its homepage exposes only the high-frequency **设计目标 / 提示词模板 / 参考图** entries; **模型**, aspect ratio, resolution, count, renovation subtype, and other technical choices are grouped under **更多设置**. The sheet can change design goal, template, and references at any time; its content remains scrollable on short viewports while its native scrollbar stays hidden. In whole-house mode the bound formal floor plan is shown as locked **参考图1**, followed by gallery/uploaded references; a blocked single-room generate action opens the sheet so the required site photo can be added. Prompt assist uses a 120-second client/API timeout to accommodate reasoning models; effect-image display URLs (platform `/media-storage` switch `directQiniuDisplayUrls`, default on: aligned Qiniu private download; off: aligned Mini signed API; WeChat-cacheable within the TTL window), and signed floor-plan preview URLs remain unchanged.
...
- The production overlay keeps the existing designer-contact data and actions, but restores a QR-first hierarchy with Xiao K gripping the green architectural header, a warm-white native-text speech bubble to Xiao K's right (`比小红书更方便贴心的 / 家装顾问`), a warm-white unobstructed QR stage, the prominent `长按二维码 识别后添加` instruction, WeChat ID copy, retry, preview, mask-close, and outside close controls. The speech-tail geometry points back to Xiao K and remains outside the contact-action flow. The component reads `getMenuButtonBoundingClientRect().bottom` on attach and every open, then places the card so the bubble clears that native capsule boundary by `16rpx`; the fallback uses current window/status-bar metrics. On a page with a visible custom TabBar it also docks the overlay above `--sfp-custom-tabbar-safe-height` so the outside close control stays fully tappable; callers without a TabBar keep the original bottom inset. On tab pages, the QR square now gives up the actual tab-lane height (including the bottom safe area), with a 240rpx minimum shell; its image, loading and error states share the same inner size. The existing 442rpx/384rpx non-tab sizes, typography, artwork, copy/preview/retry actions, routes, APIs and permissions are unchanged. Regression evidence: user screenshots `6b6de96429eb4eeaff47a92d82dbd55a.jpg` (normal non-tab) and `e3500ab9021fffa3960c9e7ebff11da8.jpg` (Service-tab contact footer clipped). All 28 focused contact, Service-home and archive tests pass. Broader checks expose two existing failures outside this change: the AI-page 20rpx whitelist and an outdated authorization-page copy assertion. Refreshed 390x844 and tall-device runtime QA remains pending manual screenshots. The user-supplied Service-tab screenshot of the clipped close control is the QA evidence for this correction.
...
`packages/business/assets/onboarding-referrer-v7/{xiao-k-promoter-hero-v7,enterprise-building-v7}.png`,
...
### Floor-plan AI constraint prompt (Implemented/Limited)

Formal-floor-plan creation paths prepend the platform-managed constraint prompt
from Admin `/api/platform/ai-prompt-config`; the measured plan remains geometry
control and is kept as the first reference, while a site photo controls camera,
perspective, lens height, field of view, and composition. The measured plan's
top-down presentation is not copied to the output unless the user or template
explicitly requests a top-down view. Unbound photo recipes remain unchanged.
...
### Mini Studio payload performance (Implemented)

The `scheme-studio` workflow list/detail routes load generation and publication summaries instead of large `input`/`output` JSON documents. Provider data URIs are replaced before persistence, preventing historical generation payloads from expanding to megabytes while preserving image delivery through signed asset URLs. The page caches and deduplicates the sibling-workflow lookup for 30 seconds, so task polling no longer downloads the workflow list repeatedly.

### Single-room mode boundaries (Implemented)

Automatic formal-plan binding (Implemented): the shared backend reconciles unbound active workflows on the next Studio workflow list/detail read. An unarchived lead's eligible primary formal plan is preferred; with no primary pointer, only a unique eligible linked plan is selected. Draft/invalid/no-closed-space plans and ambiguous choices remain unbound. The NULL-only write preserves prior bindings, source role/image, timestamps, generation/batch history, publications, and the selected design mode. Mini detail access is checked before reconciliation. After reopening or refreshing `packages/ai-workflow/scheme-studio/scheme-studio`, the existing approved Composer can expose the bound floor-plan reference and allow whole-house selection. No new client release, route, role, or visual design is required for this backend repair. Source: `design-references/ai-design/unified-entry-v2/21-studio-composer-v4-three-states.png`; automated eligibility, Mini serialization and local database reconciliation tests pass; ESLint (0 errors; existing warnings) and production build pass; manual runtime screenshot QA after backend deployment remains pending.

`scheme-studio` keeps both photo-first single-room choices but freezes different server-owned provider prompts in each generation. **Full-space design** preserves the site-photo camera, room envelope, walls, columns, doors, windows, and openings while allowing hard finishes, fixed cabinetry, lighting, furniture, and decor to be redesigned. **Soft-furnishing only** additionally preserves ceilings, floors, hard-finish materials, fixed cabinetry, built-ins, sanitary/kitchen fixtures, and architectural lighting; only movable furniture, loose lamps, curtains, rugs, bedding, artwork, plants, and decor may change. These boundaries override conflicting user/template instructions. Both modes still require a tagged site photo, omit the floor-plan control image, and use the existing `image.free_create` model/credit contract.
...
### Referrer customer and earnings empty states (Implemented)

`packages/business/referrer-progress/referrer-progress` retains the approved customer-file design (`design-references/referrer-customers-v1/referrer-customers-v1.png`). Empty copy now says 客户扫描服务码并授权手机号后，推荐记录会显示在这里。 The 出示服务码 action reads `context.referrerMembershipId` from the current signed bootstrap identity and uses `navigateTo` to open the existing `promotion-service-code?membershipId=…` directly, preserving the customer page stack and returning to the Customer Tab. Missing membership or navigation failure shows feedback instead of silently opening home or selecting another enterprise. Code generation and retry retain the existing presenter and membership APIs.

`packages/business/referrer-earnings/referrer-earnings` retains the Phase-13 design language with the user-approved empty-state adjustment: count summaries remain; the empty card contains only 暂时还没有收益记录 and 推荐客户完成签约后，符合条件的收益会显示在这里。, with no button or reserved button space and intrinsic card height. Routes, APIs, data, and role boundaries are unchanged. Focused navigation regression tests pass; updated `390x844` native-capsule and tall-device visual QA awaits the user's manual screenshots. WeChat DevTools was not automated.
