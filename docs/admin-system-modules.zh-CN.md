...
- 后台品牌锁定组：展开的侧边栏在“小程序同款”`家客来`名称前展示完全相同的本地 `brand-logo.png`；收起侧边栏也使用同一 Logo。对于平台管理员，品牌锁定组与全局企业选择器共用一行 `64px` 高的顶部空间；选择器占用剩余宽度并截断过长企业名称，不再换行到品牌下方。登录页及其页底、移动端紧凑页头和独立 AI 创作工作台均使用同一 `brand-logo.png` 加精确的“家客来”名称，仅替换旧产品品牌文本，不改动量房领域术语。后台运行时资源位于 `admin/public/brand-logo.png`，源自 `miniprogram/images/home-ip-v1/brand-logo.png`。`proxy.ts` 明确放行这一固定公开品牌资源，使 Next 图片优化器无需会话即可读取 PNG；未暴露或变更任何业务路由、API、数据契约或角色权限。
...
- 部署/会话边界：Docker 内网 HTTP 部署设置 `AUTH_COOKIE_SECURE=false`，使 `/api/auth/login` 返回的 HttpOnly `auth_token` Cookie 能被保存；切换到 HTTPS 或任何公网部署前，必须设为 `true`。`deploy.sh` 会将 `.env.production` 传给 Docker Compose；该值缺失时仅默认 `false`。JWT 校验、Cookie 作用域、角色权限和未授权跳转保持不变。
...
- API：`/api/admin/enterprises`、`/activate`、`[id]`、`[id]/logo`、`[id]/ai-key`、`[id]/ai-sync`、`[id]/ai-usage`、`/api/branding/[id]`。
...
- 工作台展示：共享 `/` 工作台已使用 `PageContainer` 和 Ant Design 汇总/列表组件。平台角色只查看既有 API 返回的用户、正式户型和企业总量，不再展示节点、趋势、延迟或健康度等占位数据；所有非平台角色只读取 PostgreSQL/RLS 按角色裁剪的工作台卡片和待办，只有 `enterprise_admin` 额外读取既有租户范围内的线索、正式户型和员工总量。所有受支持角色的工作台汇总标签均使用中文，通用操作反馈会在展示前转换已知及纯英文的 API 错误；企业负责人的默认菜单已补齐 `promotion-records`，与既有报备 API 的角色边界一致；迁移 `0022_enterprise_admin_promotion_records` 会为现有企业负责人系统角色补写缺失的该菜单权限。工作台“前往处理”改为按钮直接导航，不再嵌套在链接内。迁移后用户需重新登录，以刷新 JWT 中的权限声明。响应字段和接口行为不变。
...
- 页面：`/ai-studio/scenarios` 是客户方案 AI 执行工作台，包含“客户方案、快速工具、AI 助手”；旧 `/ai-studio/designer`、`/ai-studio/floor-plan`、`/ai-studio/furnishing`、`/ai-studio/soft-furnishing` 和方案详情 URL 保留相关查询参数后跳入统一工作台。`/ai-studio/create` 是独立全屏自由创作台，后台侧栏以新标签页打开；初始化失败时会退出无限加载态，显示缺少登录或企业上下文的可操作提示，并提供重试入口，创建 API、租户边界和 `ai-scenarios` 权限 key 不变。资源/配置入口继续为 `/inspirations`、`/ai-presets`、`/ai-providers`、`/ai-models`、`/ai-credit-prices`，企业 AI 页继续管理统一点数。场景新建设计向导现使用带原生步骤指示的 Ant Design `Modal`，版本历史现使用 Ant Design `Drawer`；输入、确认、上传、生成轮询及路由/查询参数行为均不变。
...
- 灵感方案后台：`/inspirations` 使用同一套 `PageContainer`、`ProTable`、`ModalForm` 展示模式，支持案例筛选、图片预览、发布、推荐状态和删除。`GET/POST/DELETE /api/inspirations` 现通过 `InspirationRepository` 在租户 RLS PostgreSQL `inspirations` 表中运行；封面和效果图通过共享受管图片上传 `/api/ai/creation/assets` 写入当前默认 Provider。响应会把受管图片转换为小程序签名媒体 URL，因此七牛图片不会要求小程序访问只供后台会话使用的图片路由；既有筛选、十进制字符串 `_id`、数字 `viewCount`、当前菜单权限与案例工作流保持不变。路由要求已认证的企业上下文，状态为 `Implemented`；历史 MongoDB 灵感方案既未导入，也不会混入新的 bigint 列表。
...
- API：`/api/auth/miniprogram`、`/api/miniprogram/home`、`/mine`、小程序 AI 能力/装修配方/来源/方案/媒体/任务/历史接口，以及共享线索、户型、测量、提成、订单、报备接口。
...
- 装修配方 API（2026-08-14）：`GET /api/miniprogram/ai/recipes`、`GET /api/miniprogram/ai/recipes/[id]` 和签名预览路由把活动 PostgreSQL 提示词模板转换为客户端配方目录。公开 DTO 只包含名称、分类、适用空间、输入要求、支持范围、说明和同源预览 URL；内部 `promptContent` 只在 `POST /api/miniprogram/ai/tasks` 创建任务时用于编译供应商输入，并以任务快照保存，不进入客户端响应。配方不存在或下架会在冻结点数前返回 `RECIPE_UNAVAILABLE`。未指定方案时，服务端会自动复用同客户/户型唯一活动方案；多个匹配返回 `WORKFLOW_CONFLICT` 及脱敏方案列表，不返回内部提示词。预览路由验证租户 HMAC，优先读取导入后的存储对象；Windows 开发服务未挂载 Docker `/data/ai-assets` 时会由服务端回源素材 `sourceUrl`，对象存储重定向也由 Admin 转发，随后统一压缩成小程序适用 JPEG 并同源交付。
