...
DXF 同样只读：`admin/src/lib/dxf.ts` 将 graph 适配给 MIT 许可的写入器，墙体为开口切开后的并集内外皮 `LINE`，平开门为洞口面上的门块 `INSERT`，推拉门/窗为洞口内双轨，闭合房间写内皮面积/层高/周长，整图带模型空间图框和指北针，公司/设计师取自关联客户线索；草稿、非正式
