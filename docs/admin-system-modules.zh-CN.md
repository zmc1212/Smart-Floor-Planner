...
| AI 工作室与生成 | `/ai-studio/scenarios`（AI 工作台）、`/ai-studio/create`（创作台）、供应商/预设/点数页面 | PostgreSQL AI Repository 与供应商适配器。商户工作台 `/ai-studio/scenarios` 为线索 → 命名对话 → 多轮对话出图；开对话要求合格已完成正式 v4 户型。`POST /api/ai/workflows/[id]/chat-turn` 写入 `stageKey: conversation` generation，使用量房控制图 PNG 与选中的上一张效果图。`POST /api/leads/[id]/ai-scheme-publications` 把该对话勾选图集作为命名方案发给客户，再次发送会替换同一对话的上一套。小程序设计师单图发布仍走 `/api/leads/[id]/ai-publications`。九阶段 `run-stage` API 保留兼容，不再作为后台工作台界面 | 平台及租户 `ai-scenarios` 权限；Implemented/Limited | 供应商可用性和图片存储依赖外部服务。创作台在挂到 workflow 前仍不绑定线索 |
