...
| 线索与转化 | `/leads`、`/leads/[id]` | 线索生命周期、户型、`ReferralLeadRepository`、`CustomerProjectRepository` 与 `LeadCommissionRepository`；列表/详情 DTO 包含已分配测量员、派生 `serviceStage`/`nextAction`/`nextActionKind`/`canRebook`（测量员匹配后客户下一步为 `book`，设计师仍可代约）、当前有效预约（重约后未结束的 `confirmed` 档优先于过期或已过结束时间的旧行），以及由 `referrerMembershipId -> referrerEnterpriseMemberships -> referrerProfiles` 解析出的推广人（使用资料展示名/电话，不依赖无租户基础用户行）。`/api/appointments` 与 `/api/appointments/availability` 支持授权的后台 Cookie 设计师/企业负责人创建、读取可用时段和后台改期线索预约；`POST /api/appointments/[id]/cancel` 与 `/complete` 现已接受与小程序一致的后台 Cookie 角色边界，线索详情抽屉提供取消、完成、重新预约及改期/补地址操作，过期行完成规则与 Slice 2 一致。`GET/POST /api/leads/[id]/ai-publications` 与 `DELETE /api/leads/[id]/ai-publications/[generationId]` 仍接受后台 Cookie，供小程序单图发布/撤回兼容。负责设计师和企业负责人在 `/ai-studio/scenarios` 通过 `POST /api/leads/[id]/ai-scheme-publications` 发送命名多图方案，并通过 `DELETE /api/leads/[id]/ai-scheme-publications/[workflowId]` 整套撤回；线索详情抽屉展示这些方案套并深链到 AI 工作台。后台预约审计通过 `admin_users.user_id` 映射到 `users.id`；`/leads/[id]` 通过查询参数打开共享线索详情抽屉 | 客户授权或租户/责任人校验；负责设计师仅管理自己线索的已成功 generation，并只能为本人线索创建、改期或取消预约；已派测量员可完成本人预约；企业负责人可管理本企业；撤销签单仅企业管理员且已支付提成会阻止撤销；Implemented/Limited | 客户项目已展示受保护聚合中的真实服务事实、完成正式户型摘要及主动发布方案；后台 ProTable 保留筛选/分页，并将每条线索渲染为响应式 ProCard，分组展示客户、派单、推广人、预约和操作；卡片与小程序共用服务端日期/时段选择合同，改期写入审计并通知相关人员；后台员工主键与用户审计主键已分离处理 |
| 正式户型 | `/floorplans`、`/floorplans/[id]`、`/floorplans/kujiale`；`GET /api/floorplans/[id]/export/dxf` | `FloorPlanRepository` 与只读量房适配器。`/floorplans/[id]` 用小程序 `surveyCanvasRenderer` 的后台镜像（`admin/src/lib/survey-runtime/`）绘制当前楼层，去掉编辑 session，只支持平移/缩放，不写回 graph。已完成的正式 v4 户型在创建/更新后把同一套 canvas 导出为 PNG 快照，存入 `media_assets`（`ownerType: floor_plan_preview`），由 `floor_plans.preview_asset_id` / `preview_render_revision` 指向，不写入 `layoutData`。AI 工作台控制图和 `GET /api/ai/workflows/[id]/floor-plan-preview` 复用该快照；`GET /api/floorplans/[id]/preview` 输出该图。正式查看器通过 `@tarikjabiri/dxf@2.8.9`（MIT）下载分图层、毫米单位、AutoCAD 2007+ DXF，图层为国内 CAD 习惯（`墙`、`门`、`窗`、`尺寸标注`、`空间名称`、`指北针`），并带毫米 DIMSTYLE（`标注线` / `标注线-内墙`）、`_ARCHTICK`、黑体和门扇弧虚线 `ACAD_ISO03W100`；墙体按开口切开后交给 `surveyWallSolidPlan` 并集，导出内外皮 `LINE`（含门垛），不再按墙段输出厚度矩形；平开门以 `DOOR` 块 `INSERT` 到洞口面，门扇为打开 90° 的厚矩形加灰色虚线弧和 50mm 门垛；推拉门仍为洞口内双轨，窗为离开墙皮的四条洞口内玻璃线；尺寸复用同一套闭合尺寸计划，写成旋转 `AcDbRotatedDimension`（内圈分段含墙厚刻度用 `标注线-内墙`，外圈总长用 `标注线`，文字为整毫米，`DIMTAD` 2 / `DIMGAP` 10，角度 0 或 90，L 形凹口就近标注）；闭合房间在质心写四行 MTEXT（名称、内皮面积㎡、层高 m、内皮周长 m，用 `\P` 换行）；整图套青色模型空间通高右侧标题栏和黄色指北针；共享墙、斜墙、门窗、尺寸、闭合空间标注及多楼层横向分区均从权威 v4 graph 映射，不复制布局。端点要求 `completed`、正式 v4 数据和至少一个闭合空间。后台与小程序 DXF 端点把户型名、关联线索的企业名（否则用户型租户）、线索负责设计师和户型完成日期写入图框 | 原有租户、负责设计师或已派测量员权限；Implemented | 2D 画布只读，不是小程序编辑器或 BLE 入口。不存储 CAD 文件，不导出客户电话地址；图框为通高右侧栏，含图纸名「原始户型平面图」、计算比例、完成日期，公司取关联线索企业并置于栏首，设计师取线索负责设计师；酷家乐供应商能力为 Limited |
| 测量与 BLE 设备 | `/measurements`、`/devices` | 测量、设备、绑定、审计 Repository | 平台/企业分配边界；Implemented | 仅支持协议文档定义的测距仪 |
| AI 工作室与生成 | `/ai-studio/scenarios`（AI 工作台）、`/ai-studio/create`（创作台）、供应商/预设/点数页面 | PostgreSQL AI Repository 与供应商适配器。商户工作台 `/ai-studio/scenarios` 与创作台一样新开全屏窗口。左侧选线索和命名对话，右侧用创作台出图条（模型、张数、比例、分辨率、提示词模板、参考图和提示词优化）。开对话要求合格已完成正式 v4 户型；每一轮会自动带入量房 canvas 快照 PNG（与 `/floorplans/[id]` 同一套 `surveyCanvasRenderer`，含墙体、门窗、尺寸和房间标注），并把 `free_create` generation 绑定到该对话。对话标题旁展示同一张已存储快照供设计师对照（可放大、与效果图左右对照，并链到 `/floorplans/[id]`）；`GET /api/ai/workflows/[id]/floor-plan-preview` 输出该图，不发给客户。夜间主题沿用创作台深色，日间用后台绿/白色系，选择记在 `localStorage`。`POST /api/leads/[id]/ai-scheme-publications` 把该对话勾选图集作为命名方案发给客户，再次发送会替换同一对话的上一套。小程序设计师单图发布仍走 `/api/leads/[id]/ai-publications`。九阶段 `run-stage` API 保留兼容，不再作为后台工作台界面 | 平台及租户 `ai-scenarios` 权限；Implemented/Limited | 供应商可用性和图片存储依赖外部服务。创作台在挂到 workflow 前仍不绑定线索 |
...
`surveyGraph`；测量是不可变审计，尺寸和房间摘要是派生读模型。`/floorplans/[id]` 的 2D 平面用小程序 `surveyCanvasRenderer` 的后台镜像绘制，不写回 graph。已完成正式 v4 户型保存时把该 canvas 导出为 PNG 快照（`floor_plans.preview_asset_id`），不写入 `layoutData`。DXF 墙体并集仍使用 `admin/src/lib/surveyWallSolidPlan.js`，与小程序同一套局部凸/凹求交规则。
DXF 同样只读：`admin/src/lib/dxf.ts` 将 graph 适配给 MIT 许可的写入器，墙体为开口切开后的并集内外皮 `LINE`，平开门为打开 90° 厚门扇、灰色虚线弧和门垛的门块 `INSERT`，推拉门为双轨、窗为离开墙皮的四线，闭合房间写四行 MTEXT，整图带通高模型空间标题栏和黄色指北针，公司/设计师取自关联客户线索；草稿、非正式
