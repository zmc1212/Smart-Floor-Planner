...
- 状态：`Implemented`。`/staff` 已使用共享 Ant Design ProComponents 模式（`PageContainer`、`ProTable`、`ModalForm` 与 `Tree`）承载服务端员工搜索/分页、部门筛选以及员工和部门维护；此次仅迁移展示层，不改变 API、租户范围或角色边界。支持企业员工、平台管理员、角色、部门树、状态和地推/设计师/测量员关系管理；为兼容前端，现有 `_id` 响应字段继续使用十进制字符串，RLS 与 route 角色检查共同执行租户边界。
...
- 已冻结的生成记录现可在内部 PostgreSQL 边界原子认领供应商尝试，保存供应商配置、固定远端模型、请求指纹和请求快照并推进为 `processing`；重试复用活动尝试。该边界尚不调用供应商网络，也未切换公开路由。
- 已冻结生成记录也可在内部 PostgreSQL 边界原子释放：完成幂等释放流水、标记生成失败且不再占用冻结点数；成功扣除仍须随供应商结果处理一并迁移。
