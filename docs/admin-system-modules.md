...
| Media storage | `/media-storage`; `npm run db:backup`, `npm run db:restore-drill`, and `npm run db:cleanup:dry-run` | `media_assets`, provider configuration, storage adapters; backup emits a custom PostgreSQL dump and duration, the restore drill uses only `smart_floor_planner_restore_drill` and verifies the current app schema before removing that drill database, and the cleanup dry-run uses a read-only transaction to fingerprint the target, report retain/delete counts, and emit a Qiniu candidate manifest without calling object storage | Platform admin; Implemented/Limited | Bucket cleanup and production data cleanup remain separately approved operations; the dry-run never deletes database rows or objects |
