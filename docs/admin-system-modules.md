...
| Leads and conversion | `/leads`, `/leads/[id]` | Lead, legacy acquisition, lifecycle, floor-plan, `ReferralLeadRepository`, and `CustomerProjectRepository`; the new flow atomically writes customer attribution, referrer membership, designer/measurer assignment and events, releases active attribution on closure, and exposes AI generations to customers only through publication facts | Customer authorization or tenant/assigned-staff checks; an assigned designer may manage succeeded generations only for their own leads, while an enterprise administrator may manage the tenant; Implemented/Limited | The customer-project aggregate and protected publication-image APIs are implemented; the existing customer appointment card now receives actual designer/measurer data from the aggregate, while the Mini Program formal-plan/published-design UI awaits an approved design; legacy acquisition remains until phase 8; leads with contracts or derived records cannot be purged |
