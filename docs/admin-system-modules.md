...
| Mini Program support APIs | Admin diagnostics and shared API handlers; no new phase-3 Mini Program page | Identity/context and dual-code/referrer APIs; `/api/miniprogram/codes/resolve` issues a ten-minute pending source, `/api/miniprogram/referrals/authorize-and-create-lead` atomically links the customer, locks attribution, creates the lead, and assigns staff; service identity can call `/api/internal/lead-assignments/[leadId]/retry` | Promotion resolution is public but anonymous; lead creation requires customer context or direct WeChat phone authorization plus `Idempotency-Key`; internal retry requires an `INTERNAL_SECRET` of at least 32 characters; Implemented/Limited | Phase-3 backend is implemented, while the production three-screen flow and referrer workbench remain phase 4; WeChat authorization/notification depend on external configuration |
