...
| Packages, orders, commissions | `/packages`, `/orders`, `/commissions`; three-role commissions currently expose APIs | Package, order, existing commission, and `LeadCommissionRepository`; `GET/PUT /api/commission-rules`, `GET /api/lead-commissions`, `POST /api/lead-commissions/mark-paid`. Each report record includes customer, referrer membership, enterprise, designer, measurer, and current confirmed appointment | Platform/enterprise boundaries; Implemented/Limited | Three-role rule configuration, report reads, and offline batch paid marking are protected by RLS and enterprise-admin/platform roles; a production Admin page has no approved design yet, and payment settlement remains outside this system |
...
are governed by this inventory and the development plan; their Admin UI still
requires an approved design source.
