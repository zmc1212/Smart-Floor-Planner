...
| Leads and conversion | `/leads`, `/leads/[id]` | Lead lifecycle, floor-plan, `ReferralLeadRepository`, `CustomerProjectRepository`, and `LeadCommissionRepository`; list/detail DTOs include the assigned measurer and latest confirmed measurement appointment. `/api/appointments` accepts Admin Cookie sessions for authorized designers/enterprise admins to create and read lead appointments; `/leads/[id]` opens the shared lead detail drawer through the query handoff | Customer authorization or tenant/assigned-staff checks; an assigned designer may manage succeeded generations and create appointments only for their own leads, while an enterprise administrator may manage the tenant; only an enterprise administrator can revert signing, and paid commissions block reversion; Implemented/Limited | The customer project renders the protected aggregate's real service facts, completed formal-plan summary, and explicitly published designs; publication images remain owner-only and are fetched into app-local temporary files. The Admin ProTable keeps filtering/pagination and renders each lead as a responsive ProCard with customer, assignment, appointment, and action groups; appointment rescheduling/cancellation remains on the existing appointment action surfaces |
...
| Mini Program support APIs | Admin diagnostics and shared API handlers; production anonymous claim routes are in the Mini Program | Identity/context and dual-code/referrer APIs; `GET /api/miniprogram/bootstrap` validates the signed JWT `contextVersion` and active context, returning the current role, valid role groups, enterprise/membership context, landing path, capability allowlist, and server-owned badge summary; invalid contexts return `identity_context_invalid`. `/api/miniprogram/codes/resolve` returns an onboarding code's type and target enterprise display name before phone authorization, or issues a ten-minute pending source for referral codes. `/api/miniprogram/referrer-memberships/[id]/promotion-code/image` returns a protected PNG/JPEG generated by WeChat, and `/api/miniprogram/referrals/authorize-and-create-lead` atomically links the customer, locks attribution, creates the lead, and assigns staff; phase 5 provides appointment APIs; phase 6 provides customer-project aggregation, owner-only published-design images, and designer/enterprise-admin publication or withdrawal; phase 13 adds the owner-only `GET /api/miniprogram/customer-projects` index plus signed-membership `GET /api/miniprogram/referrer-progress` and `GET /api/miniprogram/referrer-earnings`; phase 14 adds `GET /api/miniprogram/workbench` for the signed designer, measurer, or enterprise owner | Promotion resolution and code-image delivery are anonymous to the customer but membership-scoped for the referrer. Customer project endpoints validate `customer_user_id` and never trust client enterprise context; referrer progress and earnings validate both the JWT user and active membership before querying. The workbench derives staff role, enterprise, and staff scope from the signed context: designers receive only assigned leads, measurers only own confirmed appointments and their linked tasks, and enterprise owners only tenant lead/appointment aggregates. Appointment APIs are isolated by customer ownership, responsible designer, assigned measurer, or enterprise owner and use tenant transactions; `/api/miniprogram/notification-template` supplies authenticated identities with the configuration needed for customer subscription consent; Implemented/Limited | The customer project uses the completed-plan summary plus protected published-design images, not an editable graph or measurement editor. Referrer aggregates expose only a masked customer label, service facts, and the referrer's own commission status. Workbench summaries are read-only and use the existing authoritative lead, appointment, and formal-survey entry contracts. Appointment create, reschedule, and cancel attempt staff and authorized-customer delivery after commit; WeChat code generation, authorization, and notifications depend on external configuration |
