...
| Staff and accounts | `/staff`, `/departments`, `/users`; no phase-2 dual-code UI | Staff, department, binding, and admin-user repositories; `/api/enterprise/join-codes`, rotation/disable endpoints, and `/api/miniprogram/onboarding/staff` | Join-code management is limited to `super_admin`, `admin`, and `enterprise_admin`; onboarding tokens are type-isolated and staff can belong to only one enterprise; Implemented | Current active join tokens require a stable production `REFERRER_TOKEN_SECRET` or `JWT_SECRET` of at least 128 bits; production dual-code UI remains planned |
...
| Mini Program support APIs | Admin diagnostics and shared API handlers; no new phase-2 Mini Program page | Authentication/context APIs; `/api/miniprogram/codes/resolve`, `/api/miniprogram/onboarding/{staff,referrer}`, and `/api/miniprogram/referrer-memberships/*`; leads, floor plans, AI, notifications | Phone-authorized users; live identity checks; referrer memberships default to at most three active enterprises and exit invalidates the context version; Implemented/Limited | Token resolution does not yet create customer attribution, leads, or assignments; those and the production UI remain later phases |
