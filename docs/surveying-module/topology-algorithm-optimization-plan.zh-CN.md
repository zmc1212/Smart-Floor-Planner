...
### 下一阶段：低端真机性能验收（开发中）
- 已完成采样报告的预算判定：交互渲染帧 P95 ≤ 33.3ms、单帧 ≤ 50ms、堆增量 ≤ 8MB；脚本会输出逐项检查和 overall 结果。
- 真机 Canvas 桥接耗时仍必须在目标设备采样，Node 纯 JS 结果不能自动通过该项。

## 八、后续工作稳定编号
P0/P1/P2 只表示缺陷优先级，不表示开发顺序；阶段 1–4 表示已规划的实施阶段。阶段 4 收口后，长期项使用以下稳定编号：

| 编号 | 工作项 | 当前状态 |
|---|---|---|
| `S4-A` | 低端真机性能验收：帧率、Canvas 桥接、全量校验、内存、长时间拖动 | `Limited`，采样工具已完成 |
| `S4-B` | 完整 Snap Rounding：整数网格 arrangement-wide 节点化 | `Implemented`，热像素全图节点化及稳定化、门窗保护、审计预算已实施 |
| `S4-C` | 独立墙/半墙：用途字段、编辑交互、完成豁免和校验 | `Placeholder`，需先确认交互设计 |
| `S4-D` | 内洞与嵌套空间：包含关系、内洞面积和消费者语义 | `Placeholder`，当前明确拒绝 |
| `S4-E` | 真机视觉验收：微信运行环境、390×844 和长屏截图 | `Limited`，等待用户截图 |

让 AI 开发时必须指定一个编号，例如：“开发 `S4-A`，只处理低端真机性能验收；先读本计划、formal-surveying.md 和 README.md；实现前检查现有脚本；完成代码、测试、双语文档和验收结果，不修改无关模块。” AI 不应把 P1、P2 当作下一个阶段，也不应一次同时开发多个 S4 工作项。
- S4-A 代码侧已收口：设备报告现在要求 Canvas bridge P95/最大值、堆增量和至少 300 帧持续样本；缺少 bridge 数据时明确返回 incomplete，不会误报通过。
