...
仓库根目录的 `surveying-h5/` 是正式算法的浏览器验证台。它在构建时直接打包生产
`surveying-editor.js`、`surveyWallGraph.js` 和 `surveyCanvasRenderer.js`，浏览器端只
适配小程序平台 API，因此不得复制或分叉拓扑/闭合算法。该验证台用于浏览器自动化、
场景回放和 version-4 JSON 检查，不注册第二个小程序路由，默认不调用生产保存接口。
当前目录按 5 类提供 23 个代表场景，包含用户截图所示的底边对齐、上沿错层、共享竖墙
双房，并以 `2761×3223 mm`、`3082×4120 mm` 两个闭合空间做定向回归；
场景自动适配按固定测距栏的实际占用预留底部安全区；T 型三房按左房、右上房、右下房
沿共墙逐间闭合，避免闭合边界被控件遮挡或隔墙内缩在交点墙面形成三角缺口；
真实 BLE、`cover-view`、微信页面栈和真机 Canvas 仍须在现有开发者工具窗口/真机验证。

## CAD 内核渐进式重构（2026-08-14）

`miniprogram/utils/surveyWallGraph.js` 现为稳定兼容门面。既有编辑器、Canvas、
`surveyLayout.js` 和 H5 引用路径不变；原有导出合同全部保留，只新增只读
`validateSurveyDraft(draft, { mode })`。实现模块位于 `miniprogram/utils/survey/`：
`core` 管理常量/草稿/会话，`geometry` 管理二维纯函数，`domain` 管理墙/门窗/空间，
`topology` 管理临时索引、拆墙描述和 Face shadow，`operations` 管理事务，`snap`
管理吸附迟滞，`read-model` 组合原有墙体/空间读模型，`invariants` 管理校验。
`legacy-kernel.js` 只是当前已验证编辑状态机的兼容宿主，禁止页面、渲染器或 H5 直接引用。

`commitPreviewLength`、`confirmClosure`、`deleteWall`、`snapCursorToWall`、
`remeasureSelectedWall` 和门窗增删改通过一次克隆事务执行；失败不得改变输入草稿。
`quick` 校验在复杂事务后运行；`full` 增加重复墙、未切分交点、自交空间、异常共享墙和
Half-Edge Face 对照。编辑器仅在状态为 `completed` 的云端保存前执行 `full`；页面加载、
拖动帧和 Canvas 渲染禁止执行 `full` 或 Face。Face 输出和开放墙链 dangle 仅作诊断，
不得回写 `spaces[]` 或 version-4 JSON。

底部光标拖放通过统一 SnapEngine 使用 `16px` 获取、`26px` 释放迟滞；屏幕容差按 viewport
换算为毫米，结果仍取整为毫米。锁定候选保存在页面实例临时字段，松手/取消清空，不进入
草稿、云端或测量审计。本次没有修改 WXML、WXSS、素材或视觉层级，现有正式量房设计来源
和设计还原台账状态不变。

验收证据：既有量房定向测试 `74/74`，新增 CAD 测试 `20/20`，H5 `29/29` 且构建通过；
小程序全量 `356/359`，3 项失败均为实施前已知的非量房基线。2,000 次 200 墙提交微基准
测得事务门面相对兼容内核为 `+16.6%`；500 墙 quick/index 测试耗时低于 `250ms` 上限。
现有微信开发者工具窗口没有兼容自动化端点，因此真机首帧和矩形、多房间、T/十字、
拆墙门窗、撤销的正式路由人工复核仍是发布前检查项。
...
- 并集顶点的亚像素归并必须以当前墙组首个输入顶点为局部原点，不得直接对 Canvas 绝对坐标取整；连接节点判定必须沿用同一低于可见像素的端点容差。绝对坐标取整或连接判定容差不一致，会使纯平移改变 `0.01px` 网格相位，导致同一闭合房间或 T 型墙在手势帧与松手后正式帧之间裂解为不同的 `rings`。局部计算完成后再还原到 Canvas 坐标，使墙体并集对任意平移保持严格不变。回归覆盖等厚/不等厚外墙 T、斜 T、双房外墙 T、共享墙 T、十字、删除分支后恢复等七类场景及三组亚像素/大幅平移。
...
- T 型/十字节点的测量内缩、选中表现与实体几何必须分层：内缩影响红色测量边、尺寸读数和门窗沿墙坐标。同源切分后的被贯穿源墙段，其墙体实体、选择命中和实体并集继续使用原始拓扑节点，保证源墙连续；从该节点新拉出的分支墙及预览若存在端点内缩，其实体和命中范围必须从有效端点开始或结束，使分支墙停在源墙远侧墙面，不能把浅色分支实体画进原墙。选中墙体时，`selectionPolygon` 必须按同一拓扑节点处非共线相邻墙的实际 `bodyPolygon` 占用范围裁切，并使用垂直端帽；不能直接复用测量内缩或实体斜接点。这样普通转角不出现斜切，T 型、十字和房间分割两侧也只把相交墙体本身保留为中性墙色。禁止为了留出交点而在被贯穿源墙中制造空洞。斜墙连接点在实体并集中允许按不可见的亚像素容差归并，避免浮点法线产生开环；删除分支后必须根据节点剩余墙体重算同源切分段内缩，不能保留已删除墙造成的扣减。系统矩阵必须覆盖开放/闭合、外墙/共墙、直墙/斜墙、内外边、旋转、非等厚、门窗、删除、十字、保存恢复及固定种子回放。
