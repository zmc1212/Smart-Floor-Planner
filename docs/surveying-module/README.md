...
- 引导模式在每台本地客户端默认开启；顶部固定的“引导”操作以绿色状态点表示开启，点击可开关。显式关闭会持久化为本机偏好，重新开启时不会重播固定教程，而是从当前正式墙图、光标、BLE、选中对象和面板状态继续。
- 同一时刻只显示一个不遮罩画布的状态跟随式小K引导：它依次跟随首墙方向、待确认长度、独立墙链首墙的内外测量边、续墙、可闭合、空间闭合后的继续量房、下一空间光标吸附、门窗/墙体编辑和完成提交。小K以本地透明左指、右指、下指素材（`images/surveying-guide-k-left.png`、`-right.png`、`-down.png`）作为测量搭档，白底浅绿边框对话框仅显示一句行动提示；绿色虚线、箭头和目标波纹全部由真实 Canvas/控件坐标生成，并自动避开顶部安全区、右侧工具栏、底部控制坞和当前墙体。面板、数字键盘、构件编辑器或测角面板打开时由面板自身说明接管。
