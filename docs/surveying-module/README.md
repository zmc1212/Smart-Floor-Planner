...
- 同一未闭合墙链连续向前拖出同向共线墙段时，若墙体模式、墙厚和测量侧兼容，则延长末墙并保留原 wall ID；真实转向、属性变化、独立墙链、已连接分支或闭合边界仍保留独立墙。该规范化不改变 v4 数据合同、API、角色权限或测量审计队列。
- 闭合候选只提供提示，用户仍可从端点继续拖墙；只有明确执行闭合动作才生成闭合空间。
- 直线模式的独立阶梯墙链闭合时，候选虚线与正式补边都使用两段正交路径：优先沿末墙方向延伸到与起点同轴的拐点，再转向回到起点。两段补边必须分别通过相交和重叠校验；不允许以单条斜线切回起点。斜线模式和共享墙闭合继续使用其既有路径规则。
...
- 空间未闭合时，当前墙链只显示内测边尺寸并以红线高亮；预览墙使用顶部实时读数。光标放到画布空白处创建独立墙链后，该墙链第一面墙确认时可选择内/外测量边；小 K 对话框会随对应真实目标重新求解位置，内/外墙切换控件和红线继续指示实际测量边，后续墙保持锁定。光标吸附到已有墙体或顶点后属于连接既有边界的续画，不显示该提示，也不能重新切换首墙测量侧。
