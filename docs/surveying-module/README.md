...
- 同一时刻只显示一个不遮罩画布的绿色锚定气泡：它依次跟随首墙方向、待确认长度、独立墙链首墙的内外测量边、续墙、可闭合、空间闭合后的继续量房、下一空间光标吸附、以及门窗/墙体编辑。面板打开时由面板自身说明接管。首次拉墙可显示本地打包的透明测距小 K（`images/surveying-guide-k.png`）；墙线、测量边、闭合路径、方向和吸附反馈均由真实 Canvas/控件动态呈现。
