...
状态：**P0 四项缺陷已实施**，P0 验证记录见 [P0 当前合同](./topology-p0.zh-CN.md)。P1 七项缺陷已实施，见 [P1 当前合同](./topology-p1.zh-CN.md)；P2 并发保存已实施；阶段 2 交点、拆墙与成房事务已实施；本文上方“已发现的缺陷”保留为实施前审查记录。
...
**Implemented**：完整整数毫米 Snap Rounding 在同一事务内收集全图交点/端点热像素、经过像素中心、稳定化弯折片段、保留 1mm 拆段、迁移门窗与原始读数、同步 Face/Space 并完整校验。替代 P0 的 `UNSUPPORTED_INTERSECTION_PRECISION` 限制；物理墙重叠、门窗冲突、超预算修正和内洞/嵌套仍原子拒绝。路由、API、权限、v4 外壳和界面不变。算法、双端验收及当前边界见 [S4-B 合同](./snap-rounding.zh-CN.md) / [English](./snap-rounding.md)。
