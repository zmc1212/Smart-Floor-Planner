...
- 封闭外墙中段的 T 型分支保持同一拓扑节点和实体墙，但内外侧光标决定首段红色实时测量线所显示的墙边：内边起步显示分支墙内边，外边起步显示分支墙外边，两条红线相差一个墙厚。外边起步仅锁定首段；首段转角后，后续红线、橙色预览线和光标必须按当前工作内边续接，不能沿用首段外边偏移而横向跳一个墙厚。任何共享边闭合链都必须保留确认前的实体侧；包括“向外量墙、最后橙线吸附既有房间内边”的路径，确认闭合不得将墙体翻到红线/橙线另一侧或再外推一个墙厚。最后光标命中既有墙的可见外边时，必须保留该外边工作坐标并用短桥接连接拓扑角点，不能暗中投影回中心线；墙角和共享内墙续接沿用边界闭合规则。
