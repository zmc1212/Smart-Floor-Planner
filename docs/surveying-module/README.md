...
- 画布平移和缩放必须按顶部不透明导航、右侧工具轨和底部控制坞之间的可见工作区约束结构边界；当完整墙体外壳能够放入该工作区时，任何手势都不得把闭合边拖到不透明控件后方而造成视觉开口。大于工作区的户型仍可平移查看，但不得把同一轴两侧的墙体同时拖离工作区而露出无意义空白。该约束只修正 viewport 偏移，不修改闭合拓扑、毫米坐标、正式保存、API、角色或测量审计。
...
- 正式 Canvas 存在活动光标时，蓝色整屏十字辅助线只能锚定该光标的拓扑吸附点；共享墙闭合产生 `measurementEndInsetMm` 时，有效测量终点仍负责红色测量边和读数，但不得再绘制第二组蓝色十字。这样墙厚内缩不会在放大后表现为两组光标分离，且不改变正式墙长、闭合拓扑、API、角色或测量审计。
