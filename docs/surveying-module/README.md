...
- 小程序 AI 设计入口只传递可选 `floorPlanId`/`roomId`；服务端通过 `adaptSurveyGraphToRooms` 生成尺寸、层高和开口提示词上下文。携带客户上下文的结果可关联 `AiWorkflow`，现场照片和 AI 结果仍保存在独立媒体/生成记录中，且仅凭户型生成的图片只作为概念效果。
...
- 空间闭合或光标复位后，底部光标可以拖到已有顶点、墙体位置或画布空白处；画布、独立轻量 Canvas 和底部拖放控件的光标均使用一致的细线十字与方框，不重复绘制正式墙图。可闭合提示作为画布内锚定气泡显示，并在墙体和固定工具区之外自动选择位置。
...
- 空间明确闭合后，从闭合几何及该房间的墙链顺序推导每面墙的外侧和转角，不依赖已存墙侧；尺寸只绘制在全户型外边界，绝不进入任何闭合房间或共享内墙。外边界门墙使用“墙段-门宽-墙段”链式尺寸，窗墙保留整墙尺寸。
