...
- 空间未闭合时，当前墙链只显示内测边尺寸并以红线高亮；预览墙使用顶部实时读数。每次拖动光标创建墙链后，该墙链第一面墙确认时都可选择内/外测量边；提示气泡会随对应墙/光标平移并在边界限位，内/外墙提示使用短箭头指示，后续墙保持锁定。
- 空间明确闭合后，从所有闭合空间的墙链重建建筑边界：每个空间同时校验首墙正向/反向闭合，所有边按几何交点拆分并抵消重合反向边，因此共享墙即使使用不同 ID 或不同分段也不得进入尺寸计划；闭环后只保留最外层正向环，封闭内部孔洞不得标注。外边界生成无渲染依赖的 `DimensionPlan`：门墙使用“墙段-门宽-墙段”细分尺寸，连续共线外边界使用一组分段尺寸，并且每段连续边界最多一条总尺寸；尺寸采用从外墙面起笔的细延伸线、小箭头和文字断线，不得穿过墙体，也不得为同一整墙重复生成内/外两个数值。
