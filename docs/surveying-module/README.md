...
- 闭合候选只提供提示，用户仍可从端点继续拖墙；只有明确执行闭合动作才生成闭合空间。独立直线墙链已确认三面墙后，用户把第四面墙的光标真实拖入首顶点吸附范围时属于明确闭合动作：预览端点必须正交吸附到首顶点，松手立即提交第四面墙并自动闭合；仅由正交投影进入容差而手指未命中首顶点时仍只显示“合”候选，避免误闭合。
...
- 正式 Canvas 存在活动光标时，蓝色整屏十字辅助线只能锚定该光标唯一的已提交显示点：内边吸附使用拓扑节点，外边顶点吸附使用放大镜命中的可见斜接外角；墙图锚点始终保留在源墙中心线拓扑节点。共享墙闭合产生 `measurementEndInsetMm` 时，有效测量终点仍负责红色测量边和读数，但不得再绘制第二组蓝色十字。这样松手不会把外边光标跳回内角，墙厚内缩也不会在放大后表现为两组光标分离，且不改变正式墙长、闭合拓扑、API、角色或测量审计。
...
- 空间未闭合时，当前墙链只显示内测边尺寸并以红线高亮；预览墙使用顶部实时读数。光标放到画布空白处创建独立墙链后，该墙链第一面墙确认时可选择内/外测量边；小 K 对话框会随对应真实目标重新求解位置，内/外墙切换控件和红线继续指示实际测量边，后续墙保持锁定。光标吸附到未闭合墙体或顶点后仍继承既有边界，不显示该提示，也不能重新切换首墙测量侧。从已闭合房间边角续测属于例外：首段以与新墙垂直的既有闭合墙外表面作为真实测量起点，手输/BLE 长度、红线、预览墙和光标终点均以此起算；新墙实体所在侧自动匹配与首段共线的既有闭合边界外法线，因此相同方向从内边角或外边角起画都与相邻墙面连续，内/外吸附仅保留不同的真实起算点。预览及首墙确认后均显示“当前测量位置”切换按钮，切换不改变起算面或长度；按钮的双向矢量箭头沿墙体法线绘制，竖墙为左右、横墙为上下、斜墙随法线旋转。
