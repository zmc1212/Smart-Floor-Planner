...
最终目标覆盖参考视频中可见的完整工作台能力：连续绘墙、墙侧与墙厚、画布控制、保存与恢复、多楼层、2D/3D/漫游、朝向、参照和导出入口。本地操作录屏进一步补充了墙体编辑、门窗/构件尺寸输入、3D 构件预览和材质选择等后续需求证据；这些能力作为后续阶段需求来源，不进入第一墙体原型。
...
| 第一原型 | 仅支持一个闭合空间的墙体测量体验；可保存并关联带 `surveying_prototype` 标记的服务端草稿，后台可只读查看墙图，但不产出正式户型数据，不实现门窗、导出或 3D。 |
...
| [本地操作视频分析](./reference-local-operation-video.md) | `Validated` | 本地录屏中的墙体编辑、门窗/构件、3D 构件和材质操作证据。 |
...
- 该重构只允许写入带 `surveying_prototype` 标记的原型草稿，不产出正式户型数据，不触碰旧版 `miniprogram/pages/editor/editor.*`。
...
- 使用按线索隔离的本地原型草稿键保存体验过程，并在用户点击“保存草稿”时同步服务端原型草稿，便于反复现场验证。

### 原型期间禁止

- 把新版交互继续塞入旧 `editor.js` 的 `guidedEdgeIndex` / `pendingDirection` 流程。
- 将原型墙图写入正式兼容 floor plan、覆盖旧草稿或改变旧版默认入口。
- 在门窗/构件交互合同未完成前，将旧门窗弹层硬接到新版墙图中。
...
7. 退出后保留当前线索的原型本地数据；点击“保存草稿”会更新带 `surveying_prototype` 标记的服务端草稿并关联线索，后台可只读查看，不更新正式兼容户型记录。
