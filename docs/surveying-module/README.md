...
- 本批将当前正确的合法量房效果作为兼容基线：不改吸附/闭合容差、闭合推断、多房共享墙、
  ...
  外墙外皮”和“确认首段后续拖穿墙”的定向纠正外，原本正确的合法操作，其
  `nodes / walls / openings / spaces / session` 除时间字段外保持原值；前一项墙面继承纠正
  也不改变 `nodes / walls / openings / session`，后一项只收紧原本错误的穿墙预览与落点。
...
- 一面物理墙只存一份；共享墙厚度不重复计入空间净边界。连续量墙后点闭合时，
  ...
  不接受直接点击墙体或顶点吸附，点击墙体仅用于选中墙体后放置门窗；光标只能通过底部控件拖动到画布后放置（准星瞄准指尖左上 24×40 CSS px）；画布拉墙使用粘滞抓取，不套用该 Dock 偏移；拖动手势不会锁死视口。短按闭合房间内域则可直接
