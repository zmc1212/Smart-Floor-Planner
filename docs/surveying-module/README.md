...
- 外边 T 后续墙段将可见工作面相对拓扑锚点的有符号起点修正保存为 `measurementStartInsetMm`（沿测量方向前移）或 `measurementStartExtensionMm`（反向延伸）。预览、手工/BLE 确认、Canvas 和尺寸消费者统一按“拓扑长度 - 起点内缩 + 起点延伸 - 终点内缩”计算；红线长度、尺寸标注和输入读数不得相差一个墙厚。
