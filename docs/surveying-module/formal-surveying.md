...
墙体提交时，同一未闭合墙链连续向前、同向共线且模式、墙厚、测量侧兼容的拖动必须延长末墙，保留原 wall ID 和端点节点 ID，不得把一次直线延长持久化为多段墙。当前未闭合墙链的末墙可沿同轴反向回拖以缩短，并保留相同的 wall ID 和末端节点 ID；已闭合空间、共享末端、分支节点或带门窗的墙体不得使用该便捷修正。矩形补全时，第三面末墙即使已先落图为半墙，后续延长或反向缩短仍必须向首墙的正交轴线吸附；删除该未闭合末墙后，剩余墙链必须保留同一吸附参考。真实转向、属性变化、独立墙链、分支连接或闭合边界仍必须创建独立墙。该墙图规范化不改变毫米单位、正式保存结构、API、角色边界或 BLE/手动测量审计行为。
