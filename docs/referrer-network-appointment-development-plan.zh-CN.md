...
当前切片已实现带逐项真实入口的 `/referrer-network-operations`，并新增 `/appointment-settings`，明确区分系统自动默认值与管理员已确认策略。同时补齐此前仅有接口的客户端缺口：预约详情/内部动作、AI 结果发布/撤回及身份切换。普通双码列表继续不返回活动令牌，受租户授权并写入审计的图片接口保持私有、禁止缓存。认证 Chrome 已在 `http://localhost:3006` 完成核验；小程序已截取预约详情缺少上下文的应用层状态，登录态岗位动作及原生宿主胶囊证据仍待补。

### 13.2 第 11-15 阶段：五角色完整小程序体验（Planned）

当前服务端已经能够创建并切换 `customer/staff/referrer` 上下文，但客户端仍以旧的“员工/非员工”二分法组织首页、线索、量房、设计和“我的”。推荐人首次入驻后可以直接进入推广工作台，冷启动或重新登录却会回到通用首页；静态 TabBar 还会向不相关角色暴露线索、量房或 AI 入口。身份切换藏在设置页，多企业推荐人成员关系又在身份列表和推广工作台重复表达。这些属于第 11-15 阶段必须消除的产品缺口，现有路由不能作为闭环已经完成的证据。
...
| 10. 后台运营与全流程验收工作台 | `In progress` | `/referrer-network-operations` 已实现双码操作、派单资格和可执行真实状态清单；`/appointment-settings` 展示并确认企业预约策略。小程序深层业务路由与身份接口已经存在，但冷启动、通用首页和静态导航尚未形成五角色闭环；认证后台视觉核验已在 `http://localhost:3006` 完成，新增小程序路由的登录态岗位动作和原生宿主胶囊证据仍待补。 |
