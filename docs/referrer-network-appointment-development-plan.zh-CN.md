...
| 14. 设计师、测量员与企业负责人闭环 | `In progress` | `GET /api/miniprogram/workbench` 现于租户事务中从签名员工上下文推导角色、企业和员工范围：设计师只取得本人已派线索，测量员只取得本人已确认预约及关联量房任务，企业负责人只取得当前租户线索/已确认预约聚合。既有静态 Tab 路由现按能力呈现“工作台/客户/设计/我的”、“日程/任务/量房/我的”或“经营/客户/预约/我的”；测量员量房继续以已派上下文进入唯一正式编辑器。聚焦导航测试已通过，登录态 `390x844` 原生胶囊核验与真实多角色数据验收待完成。 |
