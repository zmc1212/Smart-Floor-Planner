...
| `/ai-studio/scenarios` — merchant AI workbench | Existing Ant Design 5 workbench: three columns for lead search, named conversations, and the image-chat thread, plus the shared quota bar and send-to-customer confirmation Modal. The previous stage wizard, quick-tools iframe, and assistant Tab are no longer mounted. Sidebar label is AI 工作台 | Route remains `/ai-studio/scenarios` under tenant `ai-scenarios`. Conversations require an eligible completed formal v4 floor plan. Chat turns use `POST /api/ai/workflows/[id]/chat-turn`; scheme albums use `POST /api/leads/[id]/ai-scheme-publications` and replace the prior album for the same workflow. Mini Program single-image publish APIs stay. Creation studio `/ai-studio/create` is unchanged | Focused repository, conversation, and Mini Program customer-project tests pass. Admin ESLint on the workbench files reports no errors. `npm run build` passes | Reopen only for a workbench workflow/permission change or a reproducible layout defect |
