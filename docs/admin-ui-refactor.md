...
| --- | --- | --- | --- | --- |
| `/leads` | 2026-08-11: added Active/Archived segmented views, single and up-to-100 batch archive preview, retained-asset counts, per-row partial-result reasons, read-only archived detail, restore, and manager-only purge preview with exact customer-name confirmation. The former daily-list delete action is now Archive. Added bottom spacing beneath the archive-note field so its character counter cannot overlap the modal actions. `closed` and existing customer/acquisition states remain unchanged; tenant/row access is extended by live `leads.archive_manage`, while purge stays non-delegatable. | Hold | Migration applied locally; 57 lifecycle/PostgreSQL tests, targeted ESLint, and the production build passed. Authenticated desktop and `390x844` QA at `http://localhost:3005/leads` covered Active/Archived switching and single/batch preview without mutating data. The local tenant had no archived rows, so populated restore/purge dialogs were not visually exercised; their API and blocker contracts are test-covered. The note-field spacing correction passes targeted ESLint; visual confirmation remains pending until an archive preview can be opened without changing business data. | A lifecycle/API/permission contract change, a populated restore/purge visual defect, a recurrence of the archive-note counter overlap, or the user explicitly names `/leads`. |
...
| `/ai-studio/create` | 2026-08-12: retained the existing Roomi-style dark canvas and added the requested multi-reference-image treatment. All existing assets permitted by the selected model are shown as numbered, angled stack cards; hover or keyboard focus expands the cards in sequence, exposes per-image removal, and moves the add slot to the next position. Selecting a card opens a multi-image preview with thumbnail switching, previous/next navigation, zoom, rotation, fullscreen, and download. The UI preserves the existing tenant-scoped asset upload and task/batch DTOs, route, `ai-scenarios` permission, task data, and generation behavior. | Hold | Targeted ESLint passes. Authenticated Chrome desktop QA at `http://localhost:3005/ai-studio/create` confirmed the reloaded empty-upload state and its accessible upload action. The single-image and multi-image upload, expansion, removal, and preview flow still requires visual QA with tenant-owned reference assets; no test assets were uploaded during this UI check. | A reproducible visual defect, the user names `/ai-studio/create`, or its workflow contract changes. |
