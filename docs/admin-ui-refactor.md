...
| --- | --- | --- | --- | --- |
| `/leads` | 2026-08-08: updated the existing paginated ProTable, overview labels, and detail-drawer status rail to the canonical five-step lead workflow: New lead, Acquired, Measuring, Design proposal, and Signed. The drawer adds next-action copy; historical values remain normalized in place. Routes, tenant/API boundaries, role permissions, table paging, and floor-plan/follow-up actions are unchanged. | Hold | Targeted ESLint and `git diff --check` passed. Authenticated visual/interaction QA is still pending. | A change to the lead workflow, permission, or accepted design direction, or a reproducible authenticated visual defect. |
