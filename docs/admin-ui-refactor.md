...
| --- | --- | --- | --- | --- |
| `/leads` | 2026-08-10: implemented the approved dual-track acquisition design by changing filters and the detail rail to the four-stage customer workflow: New lead, Measuring, Design proposal, and Signed. An independent acquisition filter, table column, and detail facts replace Acquired as a business stage. Routes, tenant scope, role permissions, paging, and floor-plan/follow-up actions are unchanged; the API only adds acquisition-fact filtering/DTO fields and rejects direct writes of historical `acquired`. | Hold | Targeted ESLint and Mini Program contract tests passed. Authenticated visual/interaction QA at `http://localhost:3005/leads` remains pending. | A change to customer workflow, acquisition permission, or the approved dual-track direction, or a reproducible authenticated visual defect. |
...
| `/floorplans` | 2026-08-07: aligned the read-only v4 detail SVG's real-wall-thickness opening cutouts and CAD door/window glyphs with the Mini Program canvas, including hinged, double, and sliding doors plus sliding windows. List, filters, route, API, v4 data contract, DXF, tenant scope, and `floorplans` permission remain unchanged. | Hold | Targeted ESLint and diff check passed. Authenticated desktop screenshot QA passed on `/floorplans/150` with a hinged door and three-rail window; no console errors. | A supplied visual mismatch, workflow/data-contract change, or explicit user request for `/floorplans`. |
