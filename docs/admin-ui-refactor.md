...
| `/ai-studio/create` | 2026-08-14: preserved the approved Roomi-style execution layout while moving the latest round's edit/regenerate-or-retry/delete action row from an absolutely positioned conversation-viewport sibling into the latest round card. The controls now scroll with that round; their handlers, retry/new-round semantics, task deletion behavior, route/API contracts, billing, `ai-scenarios` permission, and role boundary are unchanged. | Hold | Targeted ESLint and `git diff --check` pass. Authenticated Chrome QA at the user-reported `http://192.168.10.111:3006/ai-studio/create` verified a four-round task: the action row is inside the latest round article and scroll container, moved out of view when the container scrolled from `935` to `315`, reopened the existing prompt editor, rendered without a framework error overlay, and produced no console warning/error. Existing multi-reference and failed/partial retry visual-QA limitations remain; no upload, generation, retry, deletion, or other production mutation was triggered. | A task-round action layout or failed/partial-state visual defect, another change to retry/billing semantics, or the user explicitly names `/ai-studio/create`. |
