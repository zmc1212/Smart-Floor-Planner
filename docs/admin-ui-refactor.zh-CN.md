...
| `/referrer-network-operations` — 推荐网络运营与验收工作台 | 沿用既有 Ant Design/Admin Pro 方向：双码操作、派单资格表，以及每行都提供真实处理入口的状态验收清单 | 路由与 `referrer-network-operations` 权限继续限当前租户的 `super_admin`、`admin`、`enterprise_admin`。预约默认值在管理员保存前保持“待处理”；工作台不创建测试数据，也不绕过客户授权 | 聚焦 ESLint 与 `npm run build` 通过。认证 Chrome 已在 `http://localhost:3006` 确认七项清单、逐项真实入口及当前真实就绪状态 | 仅在出现清单/双码工作流缺陷，或工作流/权限合同变化时重开 |
