...
| 路由 | 视觉范围 | 边界 | 核验 | 未决项 |
| --- | --- | --- | --- | --- |
| `/staff` — 员工名册与专业背书 | 沿用 Ant Design/ProTable 名册，新增企业专业背书 ModalForm 与设计师/测量员行级背书 ModalForm。企业表单提供岗位默认头衔、经验、服务门槛、统一内容覆盖、三态头衔显示和客户侧实时预览；员工表单提供管理员专属头衔、从业年份、显示偏好、资料锁定、后台真实人数、受门槛控制的准确人数开关和最终预览 | `GET/PUT /api/professional-profile-settings`、`GET/PATCH /api/staff/[id]/professional-profile` 均按租户隔离且只允许企业负责人/平台管理员。既有建档、编辑、删除、派单容量、微信资料和菜单边界不变；客户 DTO 不返回真实服务人数 | 聚焦 ESLint 与专业背书规则测试通过；未要求后台认证 Chrome 视觉核验 | 仅在观察到密集弹窗/响应式缺陷、专业背书规则变化或权限/API 边界变化时重开 |
