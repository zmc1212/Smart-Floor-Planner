# 后台 UI 当前约定
| `/ai-presets` — 平台 AI 预设与正式户型约束 | 沿用浅色 Ant Design/Admin Pro 设置页：预设表格上方增加通栏 `admin-panel-card`，说明注入范围，用带字符计数的多行文本框维护英文内置约束，展示当前保存的是默认/自定义文案与未保存草稿状态，加载失败可重试，并通过共享操作反馈提供「保存」与「载入默认文案草稿」；原预设 ProTable 的筛选和编辑操作保持在下方 | 路由与 `ai-presets` 权限仍仅平台 `admin`/`super_admin`。`GET/PATCH /api/platform/ai-prompt-config` 读写 `platform_configs.ai_prompt_config`；保存只影响之后新建且绑定正式户型的生成快照。既有 `/api/ai/presets`、预设编辑、租户 AI 权限与未绑户型自由创作不变 | AI 聚焦测试、ESLint 与生产构建通过；`http://localhost:3006/ai-presets` 登录态 Chrome 视觉核验待补 | 仅在提示词配置合同/权限变化，或编辑器/表格出现可复现响应式缺陷时重开 |
...
| 路由 | 视觉范围 | 边界 | 核验 | 未决项 |
| --- | --- | --- | --- | --- |
| `/staff` — 员工名册、手机号登录与专业背书 | 沿用 Ant Design/ProTable 名册与背书表单，将 11 位手机号作为可见的主登录账号，展示内部 username 与「首次登录待改密」状态。常规新建不再填 username/password，明示初始密码 `123456`，行操作新增带确认与共享反馈的重置入口 | `POST /api/staff` 生成 `staff_e{enterpriseId}_{phone}` 并标记待改密；`POST /api/staff/[id]/reset-password` 按租户隔离、禁止重置自己，企业负责人仅可重置普通员工。背书、派单容量、微信资料、菜单、删除和客户 DTO 边界不变 | 聚焦凭据/迁移测试与 ESLint 通过；生产构建通过。`http://localhost:3006/staff` 登录态视觉核验待补 | 仅在观察到密集操作/响应式缺陷、凭据/背书合同变化或权限/API 边界变化时重开 |
| `/lead-pool` — 设计师抢单池与负责人监控 | 沿用 Ant Design/Admin Pro：摘要卡、按服务端时间展示的开放窗口 ProTable、脱敏线索事实、设计师抢单反馈，以及仅负责人的人工指派弹窗。复用后台共用页框与操作反馈，不引入平行控件体系 | 复用 `leads` 菜单权限。设计师只能看当前签名企业的池与抢单动作；企业负责人和平台角色可看监控表并走既有人工指派。领取前 DTO 始终脱敏，租户只由服务端会话解析 | 聚焦 ESLint 与 assignment-racing/referral PostgreSQL 测试通过。认证 Chrome 已在 `http://localhost:3006/lead-pool` 确认空态监控与接口响应；侧栏当前项已修正为浅绿背景及清晰可读的绿色文字/图标 | 仅在有数据表格响应式缺陷、池 DTO/隐私变化，或抢单/人工指派工作流变化时重开 |
| `/assignment-settings` — 赛马规则与绩效 | 沿用 Ant Design/Admin Pro 设置单与绩效看板：单一规则版本表单、当前版本摘要、高/普通分组表、容量使用与自动分流实际比例。比例和容量使用有边界的数值控件，保存走共享成功/失败反馈 | 复用运营工作台父权限。仅企业负责人与平台管理角色可读写设置/绩效；保存创建新版本，不改写开放窗口快照。员工个人容量覆盖仍在 `/staff` | 聚焦 ESLint、确定性 70/30 与容量测试通过。认证 Chrome 已在 `http://localhost:3006/assignment-settings` 确认规则表单、绩效表与保存后生成版本 2 | 仅在规则合同变化、密集表格布局缺陷或新增绩效解释需求时重开 |
...
| `/referrers` — 企业推荐人名册与推广网络 | 沿用既有 Ant Design/Admin Pro 方向：企业负责人与已选企业的平台管理员获得「推广网络 / 全部推广人」切换；设计师/测量员看到「我的推广人」只读名册。网络以可展开员工分支展示直接邀请的推广人；平铺 ProTable 保留姓名、手机号、加入时间、活动推广码、成员状态、姓名/手机号搜索和（负责人/平台）停用后续扫码 | 平铺名册复用 `GET /api/enterprise/referrer-memberships`；`view=network` 对已选企业的 `super_admin`、`admin` 与 `enterprise_admin` 开放；`designer`/`measurer` 通过 `referrers` 权限访问且 API 仅返回本人邀请成员 | 聚焦名册合同测试覆盖推广网络路由、员工只读名册和权限标记。认证 Chrome 核验待补 | 仅在成员合同变化或出现可复现网络/名册缺陷时重开 |
...
| `/leads` 与 `/leads/[id]` — 线索操作与预约交接 | 既有 Ant Design ProTable 转化卡片与详情抽屉在 `status=closed`（含 `terminationType=referrer_withdrawn`）时隐藏开始方案、预约、补充资料、跟进、方案发布和重试派单；详情/归档保留，企业负责人仍可「重新激活」。派单审计 `eventType`/`errorCode` 与抢单窗口 `resolutionReason` 使用中文标签。设计师/测量员「分配/更换」仍由 DTO `assignmentActions` 驱动（关闭线索本身为 false）。导出工具栏、浅色 16px 节奏、20px 头底内边距和 `/leads/[id]` 抽屉交接保持既有还原 | `PUT /api/leads/[id]` 仍拒绝改 `assignedTo`。租户范围、归档/关闭禁止、导出/安全密码、设计师仅操作本人线索的限制不变。员工覆盖改派、角色矩阵、`assignmentActions`、线索范围花名册、已确认预约改写（`staff_reassigned`，测量员时段冲突 409 且排除本条）仍属工作流合同 | 聚焦 `lead-lifecycle`、`lead-assignment-feedback` 与 `lead-assign-staff` 合同测试；撤回线索的登录态 Chrome 视觉核验待补 | 仅在出现有数据卡片布局缺陷、响应式裁切、导出/安全密码合同变化，或预约/员工改派/关闭线索工作流合同再次变化时重开 |
...
| `/enterprise-registration-codes` — 平台小程序开户码 | 沿用既有 Ant Design/Admin Pro 方向：单张生效码卡片提供换新/停用，并在卡片内直接展示当前有效合成开户海报供查看/下载（`variant=poster`，不换新、不限时隐藏），保留下载裸码（`variant=raw`），同页可配置海报二维码叠加位置，可选复制 Web `/register` 邀请链接，并展示近期审计（动作/结果经共享 `code-audit-labels` 显示中文）。令牌不出现在后台页或列表 DTO；图片仍为当前页会话内的私有 no-store Blob | 仅平台 `super_admin`/`admin`；复用 `enterprises` 菜单权限与 `GET/POST /api/admin/enterprise-registration-codes`（及 `rotate` / `disable` / `image`，`requireEnterprise: false`），模板配置走 `GET/PATCH /api/platform/enterprise-registration-code-template-config`。商户 `ej_` 入驻码仍在 `/join-codes` | 聚焦仓储、自助开户、海报合成、后台二维码查看与中文审计标签合同测试通过。认证 Chrome 核验待补 | 仅在开户码合同变化或出现可复现二维码/审计缺陷时重开 |
...
| `/llm-settings` — 平台 LLM 大模型配置 | 将用户提供的 Toonflow 设置页截图和现有实现映射到本后台明亮、绿色主导的 Ant Design/Admin Pro 体系：左侧宽配置单承载 Chat 覆盖启停、服务预设、Base URL、可编辑模型目录与加密凭据，右侧持续展示服务状态。页面级提示与状态栏明确显示当前生效来源（`LLM 大模型配置 · 严格覆盖` 或 `AI 供应商默认路由`）；切换开关后会提示路由变更尚未保存。保存、测试、拉取模型统一使用共享操作反馈，窄屏上下堆叠。当前输入精确等于已拉取模型时保留完整目录，修改为搜索词后才过滤，并防止短前缀候选覆盖较长 ID | 独立 `llm-settings` 菜单与既有 LLM API 仍仅限平台 `super_admin`/`admin`。`platform_configs.llm_config` 保持独立配置源；启用时只严格改道 `chat.general`，Vision 与图片能力继续走 AI 供应商。隐藏的系统托管供应商投影复用既有适配器和调用审计合同，不通过普通 AI 供应商管理暴露。API Key 明文只在写入/测试请求中出现，落库加密且永不回传。覆盖 Chat 请求为推理模型预留最长 90 秒，小程序提示词优化在客户端/API 边界最长等待 120 秒 | 10 条 LLM 聚焦合同测试、范围 ESLint、运行时投影核验、数据库迁移与数据库检查通过。生产编译成功，但全仓类型检查/构建仍被本路由之外已有的企业开户码海报类型错误阻断；`http://localhost:3006/llm-settings` 改造后的登录态视觉核验待补 | 仅在确认响应式/表单状态缺陷、上游 OpenAI 兼容协议差异，或需要增加故障回退/视觉覆盖策略时重开 |
...
| `/login` 与 `/change-password` — 公开登录与首次强制改密 | 保留品牌优先的 Ant Design 登录构图。凭据标签改为面向员工和管理员的「手机号 / 登录账号」，无效的找回动作改为联系企业负责人说明；待改密会话进入同风格独立改密壳 | `POST /api/auth/login` 共用 username/手机号凭据匹配，返回 `requiresPasswordChange` 与 `nextPath`。受限 Cookie 仅能访问 auth-me/password/logout 和 `/change-password`；`PUT /api/auth/password` 成功后清除标记、刷新 Cookie 并进入 `/`。公开登录壳仍不挂账户设置 | 聚焦凭据/会话测试与范围 ESLint 通过；生产构建通过。Playwright 已在 `http://localhost:3006` 确认新登录文案、未登录 `/change-password` 重定向、待改密表单、`/staff` 重定向及 `GET /api/staff` 返回 403 `password_change_required` | 仅在出现凭据/改密表单缺陷，或认证/会话合同再次变化时重开 |
...
`/login`、`/floorplans/kujiale`、AI Studio 簇（`create` / `scenarios` / 工具页及 creation/workbench/ChatInterface 共享组件）、`FloorPlanViewer` 与后台 `Sidebar` 壳层现已使用 Ant Design 5 控件。暗色工作室壳通过 `ConfigProvider` + `studio-antd-theme` 保留夜间主题。根布局挂载客户端 `AntdReact19Patch`（`@ant-design/v5-patch-for-react-19`），使 Ant Design 5 波纹效果与静态方法可在 React 19 下工作。根布局 `AntdRegistry` 开启 css-in-js `layer`，并声明 `@layer theme, base, antd, components, utilities`，使 Tailwind 工具类能盖过 Ant Design 全局 `a { color: colorLink }`（侧栏未选中项保持灰字，仅当前路由用主题绿）。未分层的 `a.ant-btn*` / `a .ant-btn*` 规则补回链接按钮应有的文字对比度。共享 `notify`、`ConfirmDialogProvider`/`useConfirmDialog`（在 `AdminAntdProvider` 的 `ConfigProvider` + `App` 下使用 antd `App.useApp().modal.confirm`）、`AccountSettingsProvider`/`useAccountSettings`（侧栏头像账户菜单与登录/安全密码弹窗）和 `ImageUploadField` 迁至 `admin/src/components/admin/*`。原 `@/components/ui` 平行控件库及无引用死代码（`UserDashboard`、`DepartmentTree`、`EnterprisePageHeader`、`enterprise-utils`）已删除。无路由/API/权限或模块清单行为变化。

可见行为变化时同步更新该路由的唯一当前记录和中英文文件。只有实现细节变化、
且不影响路由/API/权限/视觉时，在交接中说明即可，不新增台账记录。

AI 工作台当前出图行为：`/ai-studio/scenarios` 提供明确的「整屋户型」/「单间现场」
模式选择。整屋模式上传正式户型控制图并保留作用域选择；单间现场模式不上传控制图，
将现场图作为镜头权威；该模式下「客户现场图」可多选当前线索已持久化的
