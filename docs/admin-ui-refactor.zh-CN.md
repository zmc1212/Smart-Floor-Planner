...
| --- | --- | --- | --- | --- |
| `/leads` | 2026-08-10：详情抽屉现只读展示线索创建时绑定的设计师，替换原通用“当前负责人”下拉。`PUT /api/leads/[id]` 拒绝 `assignedTo`，线索不能在员工管理以外重新指派；在 `/staff` 换绑测量员设计师仍只影响后续新线索。客户主流程、获客事实、租户范围、角色权限、表格分页及户型/跟进操作保持不变。 | Hold | 定向 ESLint、生产构建与 `git diff --check` 已通过；认证后的 `http://localhost:3006/leads` 已确认表头更新为“绑定设计师”，当前环境没有线索数据，详情态视觉 QA 仍待完成。 | 创建时设计师归属、获客权限或客户流程变化，或出现可复现的认证后视觉缺陷。 |
| `/staff` | 2026-08-10：设计师二维码字段已迁移至共享 `ImageUploadField`，格式/大小说明仅由共享控件渲染一次，并提供单图卡片缩略图、放大预览、替换和移除，同时保留既有 multipart 上传请求。部门树概览、路由、既有 API 契约、企业范围媒体归属和员工管理权限边界均未改变。 | Hold | 共享组件及受影响的员工、企业、灵感方案文件定向 ESLint 已通过；认证后的桌面/移动截图 QA（含真实图片上传及预览）仍待完成。 | 出现可复现的认证后上传或视觉缺陷、用户明确指定 `/staff`，或其工作流契约变化。 |
...
| `/ai-providers`、`/ai-models`、`/ai-presets`、`/ai-credit-prices` | 2026-08-07：在 AI 供应商菜单/表格中增加经确认的单条和按选中行批量删除。批量删除限制为 1-100 个 ID，并分别返回已删除、受审计保护和不存在的项目；停用仍走 PATCH 状态变更。路由、平台 `super_admin`/`admin` 权限、供应商数据模型及无关目录工作流均未改变。 | Hold | 定向 ESLint 与 `git diff --check` 通过；此前生产构建在这次增量批量删除前已通过。未认证浏览器导航正确跳转至 `/login`，认证后的视觉和交互 QA 仍待完成。 | 供应商/目录工作流、权限、已确认设计变化，或可复现的认证后视觉缺陷。 |
...
| `/media-storage`、`/enterprises`、`/promotion-records`、`/workflow-logs`、`/enterprise-orders`、`/packages`、`/commissions`、`/roles`、`/users`、`/admins`、`/` | 2026-08-10：企业编辑器的 Logo 字段已改用共享单图 `ImageUploadField`，提供本地校验、缩略图、放大预览、替换和移除。既有 Base64 持久化、路由、API 和平台权限均未改变；其余列出的展示层迁移保持上文记录。 | Hold | 共享组件和企业编辑器的定向 ESLint 已通过；认证后的企业编辑器上传/预览 QA 仍待完成。 | 用户明确指定路由、存在可复现视觉缺陷，或其工作流契约变化。 |
...
| `/ai-studio/create` | 2026-08-07：修复已生成任务的执行态布局，使用共享 `lg` 断点承载 `1024px` 桌面编排，避免移动端 `sm` 的高度/顶部规则覆盖底部锚定；居中的摘要/结果条和底部提示词/参数面板在常见浏览器缩放下保持 Roomi 风格画布。任务列表项改为非 button 交互容器，避免菜单按钮嵌套导致无效 HTML。自由创作会把每个已持久化 batch 渲染为任务内独立轮次，保留每轮提示词/参考图上下文，选中任务继续追加 batch，并在打开或提交后滚动到最新轮次。桌面端对话区现在会在摘要和底部提示词面板之间自适应放大，隐藏原生纵向/横向滚动条但保留鼠标滚轮与触控板滚动，操作按钮固定在可滚动内容底部之外。已完成结果 DTO 仍直接使用已持久化资产 URL。路由、`ai-scenarios` 权限、任务数据和生成行为均未改变。 | Hold | 定向 ESLint 与认证 Chrome 桌面 QA 已通过（`1920x919`）：两个已有 batch 已渲染为独立轮次，最新轮次自动定位保持有效，任务列表 DOM 无嵌套 button；后续布局 QA 已确认结果区在底部面板上方扩展、滚动条外观隐藏且滚轮仍可滚动。移动端 QA 待完成。 | 出现可复现视觉缺陷、用户明确指定 `/ai-studio/create`，或其工作流契约变化。 |
