...
| --- | --- | --- | --- | --- |
| `/leads` | 2026-08-10：按已批准的获客协作双轨方案，将详情状态轨道和筛选统一为“新线索、量房中、方案设计、已签约”四阶段客户主流程；新增独立获客状态筛选、列表列和详情事实，不再把“已获客”作为业务阶段。路由、租户边界、角色权限、表格分页及户型/跟进操作保持不变；API 只增加获客事实筛选/DTO，并拒绝直接写入历史 `acquired` 状态。 | Hold | 定向 ESLint 与小程序契约测试已通过；认证后的 `http://localhost:3005/leads` 视觉/交互 QA 仍待完成。 | 客户流程、获客权限或已批准双轨方向变化，或出现可复现的认证后视觉缺陷。 |
