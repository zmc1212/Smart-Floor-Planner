...
| `/ai-studio/scenarios` — 商户 AI 工作台 | 沿用既有 Ant Design 5：左线索搜索、中命名对话、右对话出图线程，加上共用点数条和发送给客户确认 Modal。原阶段向导、快速工具 iframe 和助手 Tab 不再挂载。侧栏文案为「AI 工作台」 | 路由仍为 `/ai-studio/scenarios`，权限为租户 `ai-scenarios`。开对话要求合格已完成正式 v4 户型。对话出图走 `POST /api/ai/workflows/[id]/chat-turn`；方案图集走 `POST /api/leads/[id]/ai-scheme-publications`，再次发送会替换同一对话上一套。小程序单图发布 API 保留。创作台 `/ai-studio/create` 不变 | 聚焦 repository、对话出图与小程序客户项目测试通过。工作台文件 ESLint 无 error。`npm run build` 通过 | 仅在工作流/权限合同变化或出现可复现布局缺陷时重开 |
