...
| `/leads` 与 `/leads/[id]` — 线索操作与预约交接 | 既有 Ant Design ProTable 保留筛选、分页、加载态和批量选择，通过 `tableViewRender` 将每条线索渲染为响应式「转化进度」ProCard：客户头部集中展示阶段、联系方式、中文来源、面积/风格和主方案操作；中段用真实工作流 `Steps` 与下一步行动表达转化进度；有效预约独立呈现，符合条件的已确认预约保留卡内改期；下段以推广人/设计师/测量员/派单状态四列展示协作归属，待派单展示具体失败原因与补齐指引（设计师缺微信号/二维码时可跳转 `/staff`）；底部以弱化操作栏承载归档、重试派单等既有操作；重试仍失败时用警告反馈同一原因。卡片列表置于浅色底面，外层留 16px、卡片之间留 16px；头部/底部明确保留 20px 内边距，客户身份与操作组保持 12px 节奏；分页器有独立边界收尾栏，不再贴住列表容器。`/leads/[id]` 仍通过路由交接进入共享抽屉，保持单一详情面；创建/改预约弹窗继续沿用小程序的日期分段和服务端时段选择模式 | 线索列表/详情 DTO、`GET /api/appointments/availability`、创建/改期端点、租户范围、归档/签约权限及设计师仅操作本人线索的限制均不变；卡片仅重排既有信息并澄清派单重试反馈 | 聚焦 `lead-assignment-feedback` 单测覆盖待派单文案、指引与员工管理跳转门槛；本次为文案/反馈澄清，未重跑布局间距视觉核验 | 仅在出现有数据卡片布局缺陷、响应式裁切或预约/推广人权限与工作流合同变化时重开 |
...
| `/workflow-logs` — 通知送达台账 | 沿用 Ant Design/Admin Pro ProTable：状态附带通道（站内 / 微信订阅）、类型、接收人姓名+用户名+电话+角色、中文发送原因、结果说明、企业/报备上下文与发送时间。手工扫描仅跑预约过期；旧报备催办类型标为已停发历史行 | 路由与 `GET /api/workflow-notification-logs` 仍限 `super_admin`/`admin`/`enterprise_admin`；列表 DTO 含用户名/电话；模板配置卡仍仅平台；报备创建/更新与提醒 cron 不再派发旧工作流通知 | 接线测试断言旧报备停发与 cron 仅预约过期；认证 Chrome 核验待补 | 仅在通知合同变化或出现可复现台账布局缺陷时重开 |
