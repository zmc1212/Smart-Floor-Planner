...
| --- | --- | --- | --- | --- |
| `/leads` | 2026-08-08：将既有分页 ProTable、概览标签和详情抽屉状态轨道统一为“新线索、已获客、量房中、方案设计、已签约”五步主流程；详情增加下一步提示，历史状态仍在原位归一化展示。路由、租户/API 边界、角色权限、表格分页及户型/跟进操作均未改变。 | Hold | 定向 ESLint 与 `git diff --check` 已通过；认证后的视觉/交互 QA 仍待完成。 | 线索流程、权限或已确认设计方向变化，或出现可复现的认证后视觉缺陷。 |
