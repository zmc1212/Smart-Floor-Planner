...
| --- | --- | --- | --- | --- |
| `/leads` | 2026-08-05：在既有分页 ProTable 与详情抽屉上方增加共享概览条，只从现有响应派生当前页漏斗数量。 | Hold | 定向 ESLint 和编译后 CSS 检查已通过；CSS 网格修正后的认证桌面截图仍待完成。 | 剩余截图 QA 发现缺陷、用户明确指定 `/leads`，或其工作流契约变化。 |
| `/staff` | 2026-08-05：在部门树旁增加共享概览条；当前页岗位数和已加载部门数保持既有 API 行为。 | Hold | 定向 ESLint 已通过；认证桌面/移动截图 QA 仍待完成。 | 剩余截图 QA 发现缺陷、用户明确指定 `/staff`，或其工作流契约变化。 |
