...
| `/ai-studio/create` | 2026-08-12：保留既有 Roomi 风格深色画布，并补齐所需的多参考图交互。已选模型允许的全部素材均以带编号、倾斜的叠放卡片呈现；悬浮或键盘聚焦时卡片依序展开，显示逐图移除入口并把添加位移至末位。点击卡片会打开支持缩略图切换、上一张/下一张、缩放、旋转、全屏和下载的多图预览。界面仍使用既有租户范围内的素材上传和任务/batch DTO；路由、`ai-scenarios` 权限、任务数据和生成行为均未改变。 | Hold | 定向 ESLint 已通过。认证 Chrome 在 `http://localhost:3005/ai-studio/create` 的桌面 QA 已确认重载后的空上传状态和可访问的上传操作；单图及多图上传、展开、移除和预览流程仍需使用租户素材进行视觉 QA，本次界面检查未上传测试素材。 | 出现可复现视觉缺陷、用户明确指定 `/ai-studio/create`，或其工作流契约变化。 |
