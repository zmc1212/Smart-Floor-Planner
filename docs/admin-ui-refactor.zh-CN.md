...
| `/ai-studio/create` | 2026-08-13：保留已批准的 Roomi 风格执行态布局及当前多参考图处理，包括带编号的倾斜堆叠、键盘/悬停展开、逐图移除、叠压式圆形添加按钮、隐藏滚动条的提示词输入框、持久化轮次缩略图与多图预览。同时将失败恢复与创作迭代分开：输入未改动的失败轮显示“重试本轮”，部分失败显示“重试失败项”并保留已成功图片；修改提示词、参考图、模型、比例/分辨率、输出数量或模板后，动作切换为新建一轮。排队/生成中禁止重复提交，重试图块显示重试进度/次数，不改变既有卡片几何与视觉层级。新增租户范围的 batch 重试路由，保留原轮次并开启新计费尝试；`ai-scenarios` 权限和角色边界不变。 | Hold | 定向 ESLint、完整 65 项 PostgreSQL 测试和生产构建已通过。仍需在认证后的 `http://localhost:3005/ai-studio/create` 使用租户范围的多参考图素材及失败/部分失败任务，补充参考图交互、两种恢复文案及返回轮询状态的视觉 QA；验证期间未触发上传、生成或生产任务重试。 | 多参考图或失败/部分失败状态出现视觉缺陷、重试/计费语义再次变更，或用户明确指定 `/ai-studio/create`。 |
