...
| `/ai-studio/scenarios` — 商户 AI 工作台 | 全屏独立窗口（无后台侧栏）：左侧沿用当前客户/方案对话列表，右侧用创作台出图条（模型/张数/比例/分辨率、提示词模板、参考图、轮次宫格、发送给客户）。对话标题旁展示绑定量房控制图供设计师对照（可放大、与效果图左右对照、打开正式户型），控制图含门窗 CAD 符号和「门」「窗」标注。夜间主题对齐创作台，日间用后台 `#16a34a` / `#f6f8f6`，选择会记住 | 路由仍为 `/ai-studio/scenarios`，权限为租户 `ai-scenarios`，侧栏新开标签。开对话要求合格已完成正式 v4 户型。出图走 `POST /api/ai/creation/tasks/[id]/batches` 并带 `workflowId`，自动注入户型控制图并绑定该对话。`GET /api/ai/workflows/[id]/floor-plan-preview` 只给设计师看同一张控制图。方案图集仍走 `POST /api/leads/[id]/ai-scheme-publications`。小程序单图发布 API 保留。创作台 `/ai-studio/create` 不变 | 新增工作台主题/参考图槽位/户型预览路径单测，以及 PostgreSQL 户型对照图与「无合格户型则拒绝绑定出图」检查。工作台文件 ESLint 与 `npm run test:ai` 覆盖新 helper。两种主题的登录态视觉核验待补 | 仅在工作流/权限合同变化或出现可复现布局缺陷时重开 |
