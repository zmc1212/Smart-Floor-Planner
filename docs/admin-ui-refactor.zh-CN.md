...
| `/ai-studio/scenarios` — 商户 AI 工作台 | 全屏独立窗口（无后台侧栏）：左侧沿用当前客户/方案对话列表，右侧用创作台出图条（模型/张数/比例/分辨率、提示词模板、参考图、轮次宫格、发送给客户）。对话标题旁展示绑定户型的 canvas 快照供设计师对照（可放大、与效果图左右对照、打开正式户型），效果与 `/floorplans/[id]` 的墙体、门窗、尺寸和房间标注一致。只看效果图时使用受限预览视口，并复用创作台已有的放大、缩小、1:1、顺时针旋转、全屏和下载快捷操作。夜间主题对齐创作台，新建/发送弹窗的取消按钮复用既有深色次级控件令牌，确保文字可读；日间用后台 `#16a34a` / `#f6f8f6`，选择会记住。共享工作台/创作控件（`workbench-workspace`、`creation-workspace`、提示词模板库、图片编辑器）已改用 Ant Design `Button` / `Modal` / `Input` / `Select` / `Dropdown` 并通过主题 className 适配；`notify` 仍为共享操作反馈。常驻深色的提示词模板库与图片编辑器弹窗自带 `studioDarkAntdTheme`，从日间主题打开时 Ant Design 控件仍然清晰可读。全屏工作室路径由 `AdminAntdProvider` 包裹。出图条提示词文本域在对话框上半区撑满可用高度；「发送给客户」禁用态改为深灰底+灰字（日间浅绿底），避免紫底白字对比不足；顶栏去掉独立「返回管理后台」图标入口，仍可通过品牌 Logo 回首页；加载失败态保留「返回管理后台」文字按钮。已发布效果图角标改为实体绿底白字「已发送」 | 路由仍为 `/ai-studio/scenarios`，权限为租户 `ai-scenarios`，侧栏新开标签。开对话要求合格已完成正式 v4 户型。出图走 `POST /api/ai/creation/tasks/[id]/batches` 并带 `workflowId`，自动注入已存储的户型 canvas 快照并绑定该对话。`GET /api/ai/workflows/[id]/floor-plan-preview` 只给设计师看同一张快照。方案图集仍走 `POST /api/leads/[id]/ai-scheme-publications`。小程序单图发布 API 保留 | 提示词区高度、「发送给客户」禁用态对比度、顶栏去回后台图标、效果图「已发送」实体角标已按上述调整；聚焦 Admin ESLint 与双主题登录态视觉核验待执行 | 仅在工作流/权限合同变化或出现可复现布局缺陷时重开 |
...
| `/enterprise-registration-codes` — 平台小程序开户码 | 沿用既有 Ant Design/Admin Pro 方向：单张生效码卡片提供换新/停用/查看/下载二维码，可选复制 Web `/register` 邀请链接，并展示近期审计。令牌不出现在后台页或列表 DTO；二维码仍为私有 no-store Blob | 仅平台 `super_admin`/`admin`；复用 `enterprises` 菜单权限与 `GET/POST /api/admin/enterprise-registration-codes`（及 `rotate` / `disable` / `image`，`requireEnterprise: false`）。商户 `ej_` 入驻码仍在 `/join-codes` | 聚焦仓储与自助开户单测通过。认证 Chrome 核验待补 | 仅在开户码合同变化或出现可复现二维码/审计缺陷时重开 |
