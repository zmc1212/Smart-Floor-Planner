...
| --- | --- | --- | --- | --- |
| `/leads` | 2026-08-11：新增“在用线索 / 已归档”分段视图、单条及最多 100 条批量归档预检、保留资产数量、逐条部分失败原因、归档只读详情、恢复，以及仅管理角色可用的删除预检和客户名称确认；日常列表原“删除”已替换为“归档”。为归档备注字段补充底部间距，避免字数统计与弹窗操作按钮重叠。`closed` 与既有客户/获客状态不变；租户和行级边界叠加实时 `leads.archive_manage`，永久删除不可下放。 | Hold | 本地迁移已应用；57 项生命周期/PostgreSQL 测试、定向 ESLint 和生产构建通过。已在认证后的 `http://localhost:3005/leads` 完成桌面与 `390x844` 的在用/归档切换、单条和批量预检 QA，未修改业务数据。本地租户没有已归档记录，因此未目测有数据时的恢复/删除弹窗；其 API 与阻止合同已有测试覆盖。本次备注间距修复已通过定向 ESLint；待可在不修改业务数据的情况下打开归档预检弹窗后，补充视觉确认。 | 生命周期/API/权限合同变化、有数据时恢复/删除界面出现可复现缺陷、归档备注字数统计重叠复发，或用户明确指定 `/leads`。 |
