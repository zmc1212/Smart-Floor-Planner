...
| --- | --- | --- | --- | --- | --- |
| 1 | `workflow_todo` | `装修待办提醒` | 跟进、量房、设计、关闭，以及家装设计顾问/家装现场顾问签单提成入账等通用任务 | `48Jvq7OjOKwRhsnh8fyvtsjxAamLOakaNtiKcO11rOc` | `thing4` 项目名称；`thing11` 负责人；`phrase12` 当前状态；`thing2` 待办事项；`thing5` 备注 |
| 2 | `lead_assignment` | `客户指派成功通知` | 线索分配给家装现场顾问或家装设计顾问 | `wItuS0LdggzpMWdSOIr6FBSKeRbOKUzqXVCqJDmLpmA` | `thing1` 客户姓名；`phrase2` 客户状态；`thing3` 备注；`time4` 时间 |
...
| 9 | `lead_claim_available` | 由运营选择的可抢线索模板 | 新抢单窗口向当时符合条件的家装设计顾问开放 | 未配置 | 可选语义槽。启用微信投递前须配置真实模板 ID 与准确字段白名单；代码不得杜撰 ID 或字段。 |
...
| 派单成功通知家装设计顾问/家装现场顾问 | `lead_assignment` | `Implemented` |
| 抢单窗口向符合条件且明确授权的家装设计顾问开放 | `lead_claim_available` | `Limited`：站内通知已实现；微信仅在可选模板已配置、全局开关开启且家装设计顾问明确授权时尽力发送。 |
| 待派单催促企业负责人 | `workflow_todo` | `Implemented` |
| 已确认的独立量房预约（创建/改期/取消/过期） | `measurement_appointment` | `Limited`：事件路径已接入，字段契约仍须严格遵守。 |
| 正式量房完成后通知家装设计顾问 | `workflow_todo` | `Implemented` |
...
| `POST /api/leads/[id]/convert` 成功且存在 `role=referrer` 提成快照行 | `signing_commission` | `Implemented`：仅微信尽力发给推荐人受益人 openid；家装设计顾问/家装现场顾问不是该模板收件人。 |
...
| 同一签单提交成功后通知企业负责人 | `lead_converted` | `Implemented`：写 `staff_notifications` 并尽力发微信；家装设计顾问/家装现场顾问/客户不收。 |
...
| 模板 / 投递路径 | 收件人 | 深链 |
| --- | --- | --- |
| `lead_assignment` | 家装设计顾问 / 家装现场顾问 | `/packages/business/lead-detail/lead-detail?id={leadId}` |
| `lead_claim_available` | 家装设计顾问 | `/packages/business/lead-claim-pool/lead-claim-pool` |
| `new_lead` | 企业负责人 | 同上客户详情 |
| `workflow_todo`（待派单 / 量房完成） | 企业负责人 / 家装设计顾问 | 同上客户详情 |
| `workflow_todo`（员工签单提成） | 家装设计顾问 / 家装现场顾问 | `/packages/business/staff-earnings/staff-earnings` |
| `measurement_appointment`（员工） | 家装设计顾问 / 家装现场顾问 | 同上客户详情 |
...
- 手机号命中既有线索时继续复用原数据，不重复生成企业负责人或家装设计顾问通知。
...
- [ ] 平台开关重新启用后，持续按生产预约事件路径核验 `measurement_appointment` 投递。
