...
- 推荐人线索、后台手工录入、测量员活动码线索和企业负责人活动码线索进入抢单/赛马链路；设计师本人活动码仍直接归出示设计师。测量员仍在新线索事务中预分配；负责人出示人只记 promoter，不绑成家装设计顾问或现场顾问。
...
推广人创建的线索可进入 `closed` + `terminationType=referrer_withdrawn` 终止态。事务会释放客户归属锁和开放抢单窗口并保留派单历史；10 分钟内撤回也不会重新打开旧抢单窗口。
