...
获客工作台必须把归档线索作为历史资产处理，而不是在用任务：线索查询默认只返回在用线索，归档线索从获客任务列表和客户选择器隐藏，新增获客确认或其他写入统一返回 `409 LEAD_ARCHIVED`。已有待结算和已结算提成仍可读取和继续结算。手机号去重命中归档档案时返回 `409 ARCHIVED_LEAD_EXISTS`。归档/恢复权限按企业角色默认和员工覆盖实时解析，并继续叠加设计师/测量员行级边界；永久删除仅管理角色可用，绝不强制级联受保护资产。
