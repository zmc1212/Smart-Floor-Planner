...
| --- | --- | --- | :---: |
| `pages/index/index` | `design-references/miniprogram-airy-minimalist-v1/01-customer-workbench.jpg`, `04-designer-workbench.jpg`, `05-measurer-workbench.jpg` | 已签名客户「服务」首屏现消费共享 `serviceStage`/`nextActionKind`：测量员匹配后，主卡片下一步为「预约上门」，进入 `appointment-booking?mode=customer`；同一线索的设计师/测量员代约入口仍保留。角色工作台继续沿用极简透气版壳层。客户首次预约态的登录态 `390x844` 原生胶囊截图待补 | 是 |
...
| `packages/business/free-design-service/free-design-service` | `design-references/miniprogram-airy-minimalist-v1/10-free-design-confirm.jpg`、`11-free-design-phone-auth.jpg`、`12-free-design-success.jpg`、`13-free-design-assignment-pending.jpg` | 确认领取、手机号授权、已分配设计师成功态和待匹配态已按极简透气版领取链路还原，仍走既有解析/授权合同。活动轨确认页可展示服务企业名称。若该微信客户已有未关闭归属，resolve/授权返回 `existingAttribution`，页面复用待匹配档案布局和查看服务档案入口，不再展示新领取成功页，也不泄露新扫码企业。还原态 `390x844` 宿主截图待补 | 是 |
...
| `packages/business/referrer-workbench/referrer-workbench` | `design-references/miniprogram-airy-minimalist-v1/06-promoter-workbench.jpg` | 保持设计稿构图并提升可读性：业务主文字为 `24–28rpx`，说明文字为 `20–22rpx`。企业胶囊恢复为设计稿横向选择区中的单行自然宽度，不再按序号写死宽度，长企业名、后续企业和“加入企业”入口彼此独立。“加入企业”调用原生扫码，仅接受带码参数的既有入驻页路径后进入服务端校验的入驻流程。推广工作台与角色导航回归测试通过；当前 DevTools 窗口没有可验证的 automator endpoint，登录态 `390x844` 宿主截图待补 | 是 |
...
| `packages/business/customer-project/customer-project` | `design-references/miniprogram-airy-minimalist-v1/02-customer-project-archive.jpg` | 已还原的服务档案继续使用绿色时间线 Hero、双人员卡、正式量房户型档案和已发布方案网格。测量员匹配后，档案页用既有 section-card 语言展示可见的首次预约/重新预约卡，进入 `appointment-booking?mode=customer`；已确认预约展示时间/地址，窗口内可改期。聚焦模板/样式测试通过；首次预约态登录态 `390x844` 宿主截图待补 | 是 |
...
| `packages/business/appointment-booking/appointment-booking` | `design-references/referrer-network-appointment-v1/phase-5-designer-appointment-booking-v1.png` | 客户本人与负责设计师共用此时段选择页：填写地址并选择服务端真实可用时段后创建首次预约，并发第二次创建返回 `appointment_already_exists`。预约协调小 K 仍为 `assets/appointment-booking-v1/schedule-guide.png`（126KB）。设计师态先前 `390x844` 整窗截图仍有效；客户态截图待补 | 是 |
