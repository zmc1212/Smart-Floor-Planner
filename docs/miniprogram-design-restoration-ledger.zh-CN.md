...
| --- | --- | --- | :---: |
| `pages/index/index` | `docs/miniprogram-role-shell-design-v1.zh-CN.md` | 客户继续沿用已批准首页设计源；当前签名设计师、测量员和企业负责人改为呈现只读取服务端范围数据的角色工作台，并复用批准的独立小 K 资产 `miniprogram/images/page-ip-v3/mine.png`。聚焦导航测试通过；各角色登录态 `390x844` 准确路由与包含原生胶囊的宿主整窗截图待补 | 否 |
| `pages/leads-management/leads-management` | `docs/miniprogram-role-shell-design-v1.zh-CN.md` | 既有批准的设计师/企业负责人客户入口保留；当前签名测量员只呈现角色工作台中的本人已确认任务。聚焦导航测试通过；登录态 `390x844` 原生胶囊宿主截图待补 | 否 |
...
| `packages/business/referrer-workbench/referrer-workbench` | `design-references/referrer-network-appointment-v1/phase-5-referrer-workbench-v1.png`；用户明确授权第 13 阶段按现有小程序绿色规范扩展 | 活动企业选择会先交换签名成员关系上下文并刷新会话；受保护服务码、退出确认、历史归属提示与账号操作保持原设计。当前企业卡下新增原生“服务进度”和“我的收益”入口，只进入服务端当前签名成员关系的只读脱敏聚合，不改变推广主操作。独立服务码引导小 K PNG 仍为 `assets/referrer-workbench-v1/service-code-guide.png`（114KB）；新入口待真实登录态 `390x844` 宿主胶囊截图核验 | 否 |
| `packages/business/referrer-progress/referrer-progress` | 用户明确授权第 13 阶段按 `miniprogram/DESIGN.md`、`design-tokens.json`、`app.wxss` 及现有绿色工作台语言直接开发 | 当前成员关系的企业名称、脱敏客户标识、服务阶段与更新时间；加载、重试与无数据状态均不暴露手机号、精确地址、户型 graph、内部预约原因或设计文件。聚焦小程序测试通过，待真实登录态 `390x844` 路由与宿主胶囊核验 | 否 |
| `packages/business/referrer-earnings/referrer-earnings` | 用户明确授权第 13 阶段按 `miniprogram/DESIGN.md`、`design-tokens.json`、`app.wxss` 及现有绿色工作台语言直接开发 | 当前成员关系下仅展示本人的待支付/已支付汇总及单条金额、支付状态；加载、重试与无数据状态已实现。聚焦小程序测试通过，待真实登录态 `390x844` 路由与宿主胶囊核验 | 否 |
| `packages/business/customer-projects/customer-projects` | 用户明确授权第 13 阶段按 `miniprogram/DESIGN.md`、`design-tokens.json`、`app.wxss` 及现有绿色工作台语言直接开发 | 客户“项目”入口只列出当前 JWT 客户本人未归档项目的企业、阶段摘要和更新时间，并进入既有已批准的项目服务册；加载、重试与无数据状态已实现。聚焦小程序测试通过，待真实登录态 `390x844` 路由与宿主胶囊核验 | 否 |
| `packages/business/customer-project/customer-project` | `design-references/referrer-network-appointment-v1/phase-6-customer-project-v1.png` | 已按批准的第 6 阶段服务册设计实现：展示仅客户本人读取的真实服务事实、正式户型摘要和主动发布方案。受保护方案图片先以认证字节写入小程序本地临时文件再预览；页面没有客户量房编辑或 graph 编辑入口。第 13 阶段索引只使用其既有入口，不改变页面结构；小程序合同测试覆盖数据绑定和本地图片边界；真实微信开发者工具已在 iPhone 12/13 Pro `390x844` 确认精确顶层路由，并保存应用层截图及含原生胶囊的整窗截图 | 是 |
