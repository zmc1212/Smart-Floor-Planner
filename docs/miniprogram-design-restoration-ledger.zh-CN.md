...
| `packages/business/customer-project/customer-project` | `design-references/miniprogram-airy-minimalist-v1/02-customer-project-archive.jpg` | 绿色服务时间线 Hero、双人员卡、正式量房户型档案和已发布方案网格已重新与路由样式选择器对应，消除了模板与 LESS 脱节导致的原始文本流错乱。自定义导航继续为原生胶囊预留安全区。聚焦模板/样式回归测试通过；登录态 `390x844` 原生宿主截图待补 | 是 |
