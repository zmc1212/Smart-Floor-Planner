...
| `pages/leads-management/leads-management` | `design-references/all-pages-ip-v1/02-leads-management.png` | 新鲜编译后，真实仅 JWT 设计师会话在 `390x844` 加载实际线索空态；开发者工具整窗截图确认批准的标题、工作区和底部导航均与原生胶囊保持安全距离。待存在已派线索后再核验有数据的预约入口 | 是 |
...
| `packages/business/referrer-workbench/referrer-workbench` | `design-references/referrer-network-appointment-v1/phase-5-referrer-workbench-v1.png` | Antigravity 生成的推荐人内部工作台设计已按活动企业选择、受保护服务码入口、退出确认与历史归属提示落地；本次沿用已批准的账号设置行样式，在底部增加始终可见的“退出当前账号”，仅当服务端身份列表包含多个不同身份类型时显示“切换身份”，多个推荐企业仍在工作台内选择，不改变推广主操作；独立的服务码引导小 K 透明 PNG 打包为 `assets/referrer-workbench-v1/service-code-guide.png`（114KB）；真实登录态推荐人已在 `390x844` 完成登录完成与重新编译后冷启动核验，应用层与包含原生胶囊的整窗截图保存在 `tmp/qa/referrer-network-appointment/` | 是 |
