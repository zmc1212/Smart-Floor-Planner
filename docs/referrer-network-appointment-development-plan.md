...
- `packages/business/appointment-reschedule/appointment-reschedule`: server-calculated customer availability plus customer reschedule or reason-required internal reschedule.
...
The dual-code, staff/referrer onboarding, assignment eligibility, and end-to-end business contracts exist server-side, but operations staff have no visual Admin entry that avoids API calls. Phase 10 adds a dedicated Admin route (the final pathname is decided during implementation) for `super_admin`, `admin`, and `enterprise_admin` to perform the following inside the existing tenant boundary:
