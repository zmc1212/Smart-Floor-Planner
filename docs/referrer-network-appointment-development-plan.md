...
Status: `Approved design / Phase 7 in progress`
...
| Commissions | `GET/PUT /api/commission-rules`, `GET /api/lead-commissions`, `POST /api/lead-commissions/mark-paid`; implemented in the Phase 7 server slice. Rules are tenant-scoped and can only be read/updated with optimistic versions by enterprise/platform administrators; report and payout APIs only return or mutate records in that enterprise. |
...
| 7. Conversion and commissions | `In progress` | Three-rule repository, atomic conversion snapshots, three unique commissions, paid/void constraints, RLS report reads, and batch payout API are implemented; production Admin report/rule pages await an approved design source. |
