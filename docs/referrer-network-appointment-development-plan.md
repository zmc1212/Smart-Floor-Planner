...
| Existing-service Xiao K (design 14 crop) | `miniprogram/packages/business/assets/referral-service-v1/xiao-k-existing-service.png` |
| Onboarding welcome Xiao K (design 16 crop) | `miniprogram/packages/business/assets/referral-service-v1/xiao-k-onboarding-welcome.png` |
| Onboarding recovery Xiao K (design 17 crop) | `miniprogram/packages/business/assets/referral-service-v1/xiao-k-onboarding-recovery.png` |
...
Enterprise dual-code onboarding
  ...
  -> the customer books the visit, or the designer books it after WeChat coordination (one active appointment; the later create is rejected)
...
| `lead_commissions` | Unique lead + role; beneficiary, rule snapshot, contract amount, payable amount, immutable original payable/beneficiary snapshot, last-adjust audit (`adjusted_at`/`adjusted_by`/`adjust_reason`), `payable/paid/voided`, finance audit. |
...
Phase 2 implements this section's server contract. Enterprise administrators can list, rotate, disable, and generate private WeChat Mini Program PNGs for the enterprise staff code and their own referrer code; active enterprise designers, measurers, and salespeople can do the same only for their own referrer code. Personal referrer codes are scoped by `enterprise_join_codes.inviter_staff_id`, while the staff code remains enterprise-scoped. A Mini Program user with an authorized phone can scan the code into the dedicated onboarding route, join one enterprise as staff or join the referrer network up to the default three-enterprise limit, then list or exit memberships and retrieve the current promotion token. Tokens are server-key-derived, opaque 192-bit values; PostgreSQL stores only their SHA-256 hashes and no plaintext enterprise or inviter identifier is encoded.
...
- The staff code is enterprise-scoped and can be managed only by the enterprise owner. A referrer code is personal to the signed owner/designer/measurer/salesperson; listing, image generation, rotation, and disablement are forced to that staff scope and never accept a caller-supplied staff ID.
- Rotation creates a new version and disables only the previous version in the same enterprise/personal scope transactionally. Old codes resolve as `code_rotated`.
- A staff scan plus phone authorization and `designer/measurer` selection takes effect immediately.
- A referrer scan plus authorization creates an enterprise-owned active membership immediately and copies the personal code's inviter to `referrer_enterprise_memberships.invited_by_staff_id` plus a display-name snapshot. The first successful inviter wins: scanning another employee's code for an already-active same-enterprise membership is idempotent and never reassigns ownership. There is no manual reassignment endpoint in this slice.
- Historical memberships are backfilled to the owner only when the enterprise has exactly one `enterprise_admin`; ambiguous enterprises stay in the explicit unassigned branch. Deleting an inviter may null the staff reference, while the snapshot preserves the historical branch label.
- The configured membership limit returns `membership_limit_reached`.
...
- `GET /api/miniprogram/enterprise-referrers` is guarded by `referrer.network`. A designer, measurer, or salesperson is forced to `scope=own` and receives only memberships invited by the signed staff member. An enterprise owner receives `scope=enterprise`: `view=network` returns every business employee branch, including zero-referrer branches and an optional historical-unassigned branch, plus enterprise totals; the flat view keeps `query`/`status` pagination across all enterprise memberships.
- Only the enterprise owner may call `POST /api/miniprogram/enterprise-referrers/[id]/disable`; ordinary staff views are read-only. Disablement is idempotent and there is no re-enable operation.
...
1. The owning customer or assigned designer selects a server-calculated available slot; the first successful create occupies the active appointment and the other party receives `appointment_already_exists`.
...
- The customer project aggregates designer card, latest appointment, formal floor-plan summary, and named published scheme albums.
- Public claim pages remain anonymous; the authenticated project and appointment may show the service enterprise.

Phase 6 is implemented through the owner-only customer-project aggregate: `CustomerProjectRepository` aggregates the enterprise, designer contact card, current appointment, completed formal v4 floor-plan summary, and active publications grouped as `publishedSchemes`. Reads are enforced by `customer_user_id`, never by phone number or client-supplied enterprise context. The Admin workbench performs merge publishing from a lead conversation (`POST /api/leads/[id]/ai-scheme-publications`): selected images within the same `workflowId` are merged/updated into the existing active customer-visible publications, so customers see incremental updates within the same scheme; unselected already-confirmed images remain visible until explicitly withdrawn/deleted. Mini Program designer publish remains per succeeded generation. Withdrawal retains the generation but immediately removes it from the customer aggregate and protected image endpoint. The approved customer-project screen renders the service rail, actual designer/measurer and appointment data, a completed-formal-plan summary, and nested scheme grids whose preview queue stays inside that scheme. Ungrouped Mini Program singles appear as 其他效果图. It retrieves protected publication images as authenticated bytes into app-local temporary files before previewing them; the formal-plan card remains a summary because the customer API deliberately does not expose an editable graph or a customer editing entry.
...
- Referrer Mini Program earnings (`listEarnings`) follow the current `beneficiaryUserId` and `payableAmount`; after a beneficiary change the previous user no longer sees the row and the new eligible referrer does.

Phase 7 now implements the server-side signing and commission-ledger slice. For referral-network leads, `LeadCommissionRepository` locks the three enterprise rules and beneficiaries inside the signing transaction, calculates fixed or percentage values in integer decimal units, and snapshots three unique `payable` records with matching original payable/beneficiary values. Percentage rules require a contract amount. Payable rows may later be adjusted for amount and/or eligible tenant beneficiary with last-adjust audit; `markPaid` still accepts only `payable` and books the current amount. Conversion reversion locks commission rows, rejects any paid record, and marks the remaining payable rows `voided` with the supplied reason. Each report record includes customer, referrer membership, enterprise, designer, measurer, current confirmed-appointment context, and original/adjust audit fields. The approved `/lead-commissions` workbench lets enterprise/platform administrators maintain the three rules, filter the real ledger by status, role, and created-date range, inspect the joined context, adjust payable rows, and confirm an offline batch mark-paid action. It is separate from the legacy acquisition-commission page. Legacy acquisition leads that lack the referrer/preassigned-measurer beneficiaries retain their existing conversion behavior until phase 8 removes that flow.
...
| Dual-code management | `GET /api/enterprise/join-codes`, `POST /api/enterprise/join-codes/[type]/rotate`, `POST /api/enterprise/join-codes/[type]/disable`, `POST /api/enterprise/join-codes/[type]/image`; implemented in phase 2/10. The legacy Admin endpoints explicitly use the unassigned enterprise-level referrer scope (`inviterStaffId = null`) for listing, readiness, rotation, disablement, and image generation; personal employee codes are managed by the Mini Program routes. The image endpoint is tenant-authorized, audited, private/no-store, preserves WeChat PNG/JPEG media, and always generates a `develop` Mini Program code through `getwxacodeunlimit`. |
...
| Customer project and design publication | `GET /api/miniprogram/customer-projects/[leadId]`, `GET /api/miniprogram/customer-projects/[leadId]/published-generations/[generationId]/image`, `POST /api/leads/[id]/ai-publications`, `DELETE /api/leads/[id]/ai-publications/[generationId]`, `POST /api/leads/[id]/ai-scheme-publications`, `DELETE /api/leads/[id]/ai-scheme-publications/[workflowId]`. Customer reads remain owner-only. Admin designers perform merge publishing from a lead-bound conversation (`POST /api/leads/[id]/ai-scheme-publications`): selected images within the same `workflowId` are merged/updated into the existing active customer-visible publications, so customers see incremental updates within the same scheme; unselected already-confirmed images remain visible until explicitly withdrawn/deleted. Mini Program designer publish remains per succeeded generation. The customer DTO groups active publications into `publishedSchemes` (workflow albums plus an 其他效果图 bucket for ungrouped singles). Withdrawn or deleted generations never appear in the customer aggregate or image endpoint. |
...
| Commissions | `GET/PUT /api/commission-rules`, `GET /api/lead-commissions?status=&role=&fromDate=&toDate=&source=` (list DTO includes original/adjust audit fields), `GET /api/lead-commissions/beneficiaries?role=`, `PATCH /api/lead-commissions/[id]` (payable-only amount and/or beneficiary adjustment with optional reason), `POST /api/lead-commissions/mark-paid`; Phase 7 server slice plus manual payable adjustment. Rules are tenant-scoped and can only be read/updated with optimistic versions by enterprise/platform administrators; report, adjust, and payout APIs only return or mutate records in that enterprise. |
...
- `packages/business/customer-project/customer-project`: the approved Phase 6 customer project folio renders the real appointment, designer/measurer, completed formal-plan summary, and named published scheme albums that reuse the existing design grid; the protected design-image endpoint is read into an app-local file before customer preview. It intentionally has no customer measurement-editor entry or editable floor-plan viewer.
...
- `packages/business/appointment-booking/appointment-booking`: after a measurer is assigned, the owning customer enters from Service home or the project folio, and the assigned designer can still enter from a lead without a confirmed appointment. Both provide the service address, select a server-calculated real slot, and create the first appointment; the first successful create occupies the active row. Appointment detail remains the follow-up entry when the address needs completion.
...
- `packages/business/onboarding/onboarding`: staff/referrer join-code landing route. It resolves the code type before phone authorization, collects the authorization, selects only `designer` or `measurer` for staff, and for a referrer requires a real display name after phone authorization before calling the existing onboarding endpoint and switching to the returned context.
...
The Phase-7 merchant route `/lead-commissions` implements the established Admin UI direction with Ant Design/Admin Pro rule cards (`Card`, `Segmented`, `InputNumber`, `Switch`), `ProTable` status/role/source/date ledger filters, real joined report columns, a confirmation-protected batch paid action, a lead-parent expandable ledger with nested role rows (default expanded), plus an Adjust modal on payable rows (amount change and/or beneficiary; beneficiary and reason optional), and `Statistic` amount summaries. It adds the dedicated `lead-commissions` navigation and permission boundary without replacing `/acquisition-commissions`. Other functional Admin work follows the bilingual Admin UI refactor contract and the existing Ant Design/Admin Pro route patterns.

### 13.1 Phase 10: Admin operations and end-to-end acceptance workbench (In progress)

The dual-code, staff/referrer onboarding, assignment eligibility, and end-to-end business contracts exist server-side, but operations staff have no visual Admin entry that avoids API calls. Phase 10 provides `/referrer-network-operations`, `/join-codes`, `/referrers`, `/appointment-settings`, and a role-rendered Admin home workbench so management roles can run acceptance and designers/measurers can process their own tasks inside the existing tenant boundary:

1. Select the current enterprise on `/join-codes`; view staff/referrer-code status, version, expiry, and creation/disable audit; rotate, disable, display, or download scannable onboarding QR codes after confirmation. Tokens never leave the server for an Admin page, ordinary logs, or public pages.
2. Check designer/measurer onboarding, account status, `assignmentPaused`, designer WeChat ID/QR completeness, and current assignment eligibility on `/staff`. The operations hub only shows eligibility counts and links. It reuses existing staff-profile and assignment-retry contracts; it adds no manual acceptance or cross-enterprise assignment.
3. Provide testers with a complete-workflow readiness checklist on `/referrer-network-operations`: referrer onboarding, promotion service code, eligible designer/measurer, appointment settings, three-role commission rules, and external WeChat capability. `/referrers` is the membership roster (name, phone, disable-scan). The checklist shows only real state and navigation entries; it does not fabricate customers, leads, appointments, surveying, AI, or conversion data.
...
Referrer selects an enterprise and presents its service code
  ...
  -> the customer books the visit, or the designer books it after contacting the customer
...
| 2. Dual codes and referrer network | `Completed` | Rotation/disable audit, single-enterprise staff, default three-enterprise membership limit/leave, and reproducible opaque promotion tokens are implemented. Referrer join codes now carry a server-enforced personal inviter scope for enterprise owners/designers/measurers/salespeople; memberships remain enterprise-owned while recording the first successful inviter and name snapshot. Staff can read only their own invitees, owners can read the enterprise network or flat roster, and disablement remains owner-only. Historical rows backfill only for enterprises with exactly one owner; ambiguous rows remain unassigned, and no reassignment API is exposed. Repository database contract tests pass. |
...
| 6. Project, surveying, and publication | `Completed` | Project aggregation, owner-only reads, completed formal-plan summary, and protected preview are implemented. Admin workbench conversations publish named multi-image albums; Mini Program designer publish remains per succeeded generation. The customer folio groups those albums (plus 其他效果图 for ungrouped singles) and has no measurement-editor or graph-editing path. Existing Phase-6 visual language remains the source; grouped-album native-capsule recapture is pending. |
| 7. Conversion and commissions | `Completed` | Three-rule repository, atomic conversion snapshots, three unique commissions, payable-only amount/beneficiary adjust with audit, paid/void constraints, RLS report reads, batch payout API, and the approved `/lead-commissions` rule/report/adjust workbench are implemented. Focused PostgreSQL commission and adjust testing, the production build, and authenticated `localhost:3006` visual verification of the approved desktop workbench are complete. |
...
| 10. Admin operations and end-to-end acceptance workbench | `In progress` | `/referrer-network-operations` is a readiness hub; `/join-codes` owns dual-code operations and audit; `/referrers` lists membership name/phone and disable-scan; `/appointment-settings` is linked from the referral-network sidebar group and confirms enterprise policy. The Admin lead detail drawer now cancels/completes/rebooks appointments and publishes/withdraws AI schemes through the same Cookie role boundaries as the Mini Program; `GET /api/miniprogram/bootstrap` now returns role-scoped Tab badges and `暂时无法读取` when counts fail. Authenticated Admin QA is complete at `http://localhost:3006`; a JWT-only designer lead-list empty state and a real signed referrer login/cold-launch workbench, both with native-host-capsule captures, now pass at `390x844`, while authenticated appointment and publication actions, badge chrome, and five-role homes still need real assigned data for final QA. |
...
| 12. Role information architecture and design approval | `In progress` | The user approved the current Mini Program visual language. `docs/miniprogram-role-shell-design-v1.*` defines role targets, allowlists, recovery states, and safe areas; production now includes bootstrap-driven navigation for current executable routes, an identity-recovery page that exposes no invalid-tenant data, and server-owned Tab badges from role-scoped todo counts (failures show `暂时无法读取`). Route ledgers await `390x844` verification. |
| 13. Customer and referrer loops | `In progress` | `GET /api/miniprogram/customer-projects` now exposes an owned, unarchived customer-project index from the current JWT. The project folio groups published images as named scheme albums (`publishedSchemes`), with Mini Program singles without a workflow under 其他效果图. Selecting another enterprise in the referrer workbench exchanges the signed `referrerMembershipId` context before refreshing the session; service code and `GET /api/miniprogram/referrer-progress` share that membership boundary, while `GET /api/miniprogram/referrer-earnings` authorizes the same membership then lists rows by current commission `beneficiaryUserId`/`payableAmount`. The aggregates return only masked customer labels, service stage/update facts, and the referrer's payable, paid, or voided earnings. Customer Service badges count first-booking/reschedule/rebook work; referrer Progress/Earnings badges count open milestones and payable commissions. Customer/referrer negative authorization tests and focused Mini Program tests pass; grouped-album native-capsule QA remains pending. |
| 14. Designer, measurer, and enterprise-owner loops | `In progress` | `GET /api/miniprogram/workbench` now derives the signed staff role, enterprise, and staff scope in the tenant transaction. Enterprise-owner Operations returns pending-assignment failures, expired unrebooked work, and staffing gaps; the Appointments tab opens `enterprise-appointments` (including expired rows) instead of occupying `pages/ai-design`. Measurers land in the role workbench; its calendar entry opens `measurer-calendar`, and expired or completed appointments leave the confirmed list. Completed formal v4 surveys also leave the pending workbench/`待量房任务` count instead of bouncing back as unscheduled tasks. Customer Service home consumes the shared `serviceStage`/`nextActionKind` (first booking after measurer assignment; designers can still book on the customer's behalf). Designers can self-serve WeChat ID and QR on `profile-edit`. Bootstrap badges count designer follow-up plus expired work, measurer workbench today/tasks, and owner exceptions including expired unrebooked rows. Admin adds `GET /api/workbench/staff`. The `referrer.network` capability now gives enterprise staff a personal referrer-code/own-roster entry and gives the owner a grouped network plus the flat enterprise roster; these states extend the existing join-code and roster design sources without new artwork. Focused navigation and badge tests pass. Authenticated Admin role QA, user-supplied `390x844` native-capsule QA for the changed referrer states, and real multi-role data acceptance remain pending. |
...
- Surveying/AI: formal v4 only; customer sees explicitly published named scheme albums only.
