...
| Customer project and design publication | `GET /api/miniprogram/customer-projects/[leadId]`, `GET /api/miniprogram/customer-projects/[leadId]/published-generations/[generationId]/image`, `POST /api/leads/[id]/ai-publications`, `DELETE /api/leads/[id]/ai-publications/[generationId]`, `POST /api/leads/[id]/ai-scheme-publications`, `DELETE /api/leads/[id]/ai-scheme-publications/[workflowId]`. Customer reads remain owner-only. Admin designers send a named selected-image album from a lead-bound conversation; Mini Program designer publish remains per succeeded generation. The customer DTO groups active publications into `publishedSchemes` (workflow albums plus an 其他效果图 bucket for ungrouped singles). Withdrawn or deleted generations never appear in the customer aggregate or image endpoint. |
...
| Measurer | `Workbench / Tasks / Survey / Mine` | Start in the role workbench, read assigned appointments, maintain own unavailability, enter the single formal survey editor from an assigned task, and complete handoff | Design publication, conversion, referrer earnings, enterprise rules, or unassigned customer data |
