...
Status: `Approved design / Phase 8 completed`
...
| 9. Cleanup rehearsal and production release | `In progress` | A read-only dry-run now reports per-table retain/delete counts, platform-admin/configuration and migration fingerprints, a Qiniu candidate manifest, and optional JSON/Markdown audit files; backup and dedicated restore-drill scripts report their actual duration and validate the current migration schema's 68 tables, 47 RLS tables, and 88 policies. The development-environment backup/restore rehearsal passed; production target-fingerprint confirmation, human manifest review, and separate production approval remain required. |
