...
Status: `Approved design / Phase 6 in progress`
...
The first Phase 6 backend slice is implemented: `CustomerProjectRepository` aggregates the owning customer, enterprise, designer contact card, current appointment, completed formal v4 floor-plan summary, and active publications into one read-only project. Reads are enforced by `customer_user_id`, never by phone number or client-supplied enterprise context. The assigned designer or enterprise administrator can only publish/withdraw a succeeded AI generation belonging to the lead; withdrawal retains the generation but immediately removes it from the customer aggregate and protected image endpoint. The customer-project runtime screen retains the approved Phase 5 appointment-card layout but now reads that aggregate and shows the server-returned designer and measurer in its existing service rows; its formal-plan and published-design UI must still wait for an approved design source.
...
- `packages/business/customer-project/customer-project`: the current customer appointment card and rescheduling entry now read the Phase 6 customer-project aggregate and show the actual designer/measurer in their existing service rows; the formal-floor-plan and publication areas remain Phase 6 work.
...
| 6. Project, surveying, and publication | `In progress` | Project aggregation API, publication/withdrawal fact, and customer read boundary now have Repository/RLS integration coverage. The existing approved appointment card reads the aggregate and shows actual designer/measurer data. The approved customer-project formal-plan/published-design UI, measurement entry, and their visual QA remain. |
