...
`packages/ai-workflow/scheme-studio/scheme-studio` 的收起态现保持两种完全排他的入口：
已选模板时只显示紧凑的模板摘要、**编辑**、完整的控制图文字与快捷生成；仅当无模板且
尚未选择设计方式时，才显示撑满内容宽度的同色相绿色渐变实底 **开始新一轮设计** 按钮。
点击仍打开既有的轮次准备方式选择。生成 API、设计方式选择、租户范围与角色边界均未改变。
时间线竖轨和状态节点在标准与窄屏均共用同一圆心坐标；创作中轮次改为单个圆角对话尾巴与
卡片相接，不再出现脱离卡片的旋转菱形伪影。完成轮与创作中轮的 `390x844` 用户真机截图
仍待补充。固定 Composer 底部操作栏现使用浅绿色渐变、大圆角上沿和半透明白色内层编辑区，
与白色时间线页面形成清晰分层；输入、工具栏、生成及键盘安全区行为保持不变。
展开模板编辑器中的既有折叠动作现标为 **收起**，不会关闭方案工作台。
底部生成按钮略微降低高度，并明确在工具栏内容区居中；输入、工具栏结构、生成及键盘安全区行为保持不变。
...
| AI 设计工作流 | `pages/ai-design/ai-design`、`packages/ai-workflow/*` | 设计 Tab 与 recipe-project 继续作为创作入口。`scheme-studio` 现按负责人批准的五状态 Composer 稿 `design-references/ai-design/unified-entry-v2/14-studio-manual-expanded-v1.png` 还原：手动模式保留紧凑可编辑提示词/控制图/生成行，展开后显示「手动创作」「选择模板」和完整编辑器。选中模板后立即进入**完整提示词**编辑器，保留模板来源/封面信息及「恢复模板」；已移除独立的「个性化调整」输入、其带标签的提示词拼接和客户端提交参数。模板 `promptContent`（兼容旧 `internalPrompt`）直接替换旧草稿；配方详情接口现在将可编辑正文统一返回为 `promptContent` 并隐藏 `internalPrompt`，没有可编辑提示词的模板不会被应用。时间线、设计方式、批次/API、租户/权限、配方、发布、重试和弹窗合同保持不变。`390×844` 运行态视觉 QA 仍待用户提供截图。 | Implemented/Limited；供应商、点数、资格、发布可见性和范围仍由服务端控制；无后台画布标注精修、无深色主题；需在有完成批次和进行中批次的真实工作流补做批准状态 QA，并由用户手动提供 `390×844` Composer 截图。 |
...
`scheme-studio` 每次新一轮从 `04-round-setup-v2.png` 的已批准方式选择弹层进入，而非进页即强制选择。内容固有高度的原生底部弹层保留整屋/单间的既有 `renderMode` 合同、真实正式户型输入依据、可选已选配方及单间必须有现场图的约束；可用的输入依据动作继续打开带标签现场图选择。配方「更换」会先关闭本弹层、再打开模板 sheet；使用模板或取消模板选择后，均回到同一份新一轮弹层，避免两层遮罩叠加。已选 `renderMode`、输入依据、草稿、模板元数据、路由、Studio 批次 API、权限、数据合同和父级时间线/Composer 均未改变；更新后的 `390×844` 原生弹层截图仍待用户手动核验。
