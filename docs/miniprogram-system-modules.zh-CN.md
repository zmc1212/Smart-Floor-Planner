...
- 源码包通过 `project.config.json` 的 `packOptions.ignore` 显式排除开发目录和未被运行时引用的历史设计图；主包仅保留当前运行资源，并在微信 2MB 源码包上限下预留余量。主包 `images/` 插画已原地调色板压缩，未使用的历史 Hero/遗留图与本地设计 token 文件也进入 ignore。`packages/business` 分包单独遵守 2MB 源码上限：运行时插画使用调色板压缩 PNG、排除未引用的推荐/客户历史图，小 K 形象复用主包 `images/airy-v1`，不再在 business 分包重复打包。角色引导页及其生成插画统一放入独立 `packages/guides` 分包，运行时插画使用调色板压缩透明 PNG，使该分包保持在独立 2MB 源码上限以内，并由聚焦体积测试约束；每张生成 PNG 不超过 300KB。
...
单一 `pages/index/index` 路由中的企业负责人经营部分，当前设计源为
...
不清的「闭合率」。V3 业务插画均通过 ImageGen 独立生成透明 PNG，打入
`images/operations-dashboard/enterprise-guide.png`（17,943 字节）、`lead-inbox.png`、`staff-load.png`、
`staff-onboarding.png`（7,996 字节）、`scheme-delivery-rate.png`（13,966 字节）和
`signing-rate.png`（11,725 字节）；均小于 `300KB`。复合设计稿禁止裁切为运行时资源。按设计源校准的
...
`design-references/enterprise-owner-activity-code-entry-v3/staff-onboarding-white-card-v2.png`，并打包为
`images/operations-dashboard/staff-onboarding.png`；人员负荷小 K 图盒调整为 `120rpx × 104rpx`，三张阶段卡
...
- 身份/上下文：`/api/auth/miniprogram`、`/api/miniprogram/bootstrap`、`/api/miniprogram/identity-contexts`、
  ...
  错误角色回退。
- 服务码出示：`promotion-service-code` 保留既有二维码优先服务凭证；`staff-activity-code` 与
  `enterprise-join-codes` 改用已批准的 `design-references/enterprise-code-presenters-fullscreen-v1/`：
  ...
  活动码按钮仍为「分享给客户」，生效入驻码仍以「一键分享」进入带当前 `ej_` 令牌的入驻页。
...
`GET /api/miniprogram/referrer-progress` 返回不含个人信息的 `recordCode`、建档时间、终止类型、撤销资格、阻止原因和 10 分钟撤回截止时间。推广人可在当前企业成员关系下，携带 `Idempotency-Key` 调用 `POST /api/miniprogram/referrer-progress/withdraw` 或 `/withdraw/undo`。撤销记录保留为 `closed` 且 `terminationType=referrer_withdrawn` 的只读线索；员工工作台收到站内确认提醒，客户服务档案显示“服务已终止”并禁用所有服务 CTA。
