...
- 已实现：企业共享 AI 点数与业务动作价格展示；首页以“参考图复刻、空间换风格、户型生成、软装深化”四个现场任务组织入口，并提供“先选客户正式户型，再选完整户型或单个闭合房间”的两步选择器，选择后的 `leadId`、`floorPlanId`、`targetScope` 和单房间可选 `roomId` 由四种任务共同继承，只有户型生成强制要求先关联。完整户型一次生成一张带家具的全屋俯视概念图，单房间一次生成一张室内视角概念图；不批量逐房间生成。首页紧凑方案卡会自动带入唯一活动方案，多个方案时要求用户选择，并保留“新建备选方案”；创建页、结果页和历史记录显示客户/方案归属、设计范围及“已同步方案/独立任务”。参考图复刻使用双图，整体换风格和软装深化使用空间图加后台风格预设。页面支持相册/拍摄上传、异步供应商任务状态、失败重试/点数释放、可拖动分割线的原图结果对比、预览、保存、分享、历史复用和删除。详情读取会对生成中的任务强制查询上游状态；首页存在可见生成中任务时每 5 秒刷新最近成果，历史接口每次返回前最多同步 4 条可见生成中任务，直至成功或失败。
...
- 正式户型：入口传递 `floorPlanId`、显式 `targetScope: whole_floor_plan | single_room`，仅单房间携带 `roomId`。后台通过正式墙图读适配层派生尺寸、层高和开口摘要，不修改 `FloorPlan.layoutData`；完整户型额外派生 1024px 墙体/门窗控制图并作为独立 `MediaAsset` 通过图片编辑生成，单房间通过量房摘要调用图片生成。来源选择器只返回当前企业角色可访问的正式户型和闭合房间（设计师按分配线索/归属户型、量房员按归属户型、企业负责人按企业范围）；仅凭户型数据生成的是概念效果，不是施工级或像素级还原。
