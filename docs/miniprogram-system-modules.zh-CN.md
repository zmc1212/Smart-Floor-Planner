...
- 已实现绘制/编辑：覆盖完整墙厚和开口宽度的 CAD 风格门窗符号、未闭合内测边红线、房间填充仅接受首墙正向或反向能够完整闭合的墙链、单侧墙体与连接节点补面先做全局实体合并再统一填充和描边（连接节点、L/T 型接入和重合分段不再出现内部端帽、斜缝或独立方框），闭合户型使用工程图式外轮廓 `DimensionPlan`（空间边界先做几何拆分和重合边抵消，不同 ID/不同分段的共享墙与封闭内部孔洞都不标注；每条连续共线外边界仅有一组门洞细分、一组分段尺寸和一条总尺寸；细延伸线从外墙面起笔，并使用小箭头和文字断线，不再有重复整墙尺寸）、画布、拖拽层和底部拖放控件统一的细线十字/方框光标，以及会避开墙体和固定控件、统一使用绿色底面的画布锚定操作提示；构件规格、构件 BLE 测距、翻转/模型面板，以及选中门窗的 Three.js 预览。
