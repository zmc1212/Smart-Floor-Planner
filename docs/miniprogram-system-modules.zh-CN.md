...
`pages/index/index` 是负责人**工作台**：紧凑的绿色调度 Hero 下方只有一个白色「获客与团队」操作中枢，其中依次包含较高的活动码主卡、右侧上下堆叠的「邀请入驻」和「查看推广人」、「查看预约安排」上下文行、B+C 细窄「经营提醒」及蓝/紫两张调度洞察卡。「经营提醒」只读取工作台真实返回的 `newLeads` 与 `signedCount` 及其单位，绝不展示效果图中的示例数值；点击通过 `switchTab` 进入独立**经营** Tab。异常区保持为中枢外单独的白色分组容器，告警图标与处理按钮独立排布。「查看预约安排」仍以 `navigateTo` 进入受 `enterprise.appointments` 保护、无底部 TabBar 的预约调度二级页，并提供本周/本月/本年与自定义周期；自定义选中区间显示在「自定义」芯片右侧（同年为 `8/1 ~ 8/31`，跨年保留年份），顶栏副标题只保留「自定义 N 单预约」。当前独立生成、未从效果图切片的生产插画映射为 `activity-code-share-v3.png`（592×464，20,073 字节）、`team-onboarding-v3.png`（480×384，21,267 字节）、`referrer-roster-v2.png`（180×201，10,011 字节）、`lead-inbox-v2.png`（240×239，10,044 字节）和 `priority-alert-v2.png`（90×88，3,347 字节），均位于 `miniprogram/images/operations-dashboard/`，且为带透明通道、小于 300KB 的 PNG。`pages/enterprise-operations/enterprise-operations` 是独立**经营** Tab，并按用户 2026-08-28 提供的效果图重组：胶囊安全的浅薄荷「本月经营大盘」Hero 内含四个周期控件、三项真实周期 KPI 以及企业负责人专属的 `contractAmountSum`「签约金额」（金额数字与左侧计数卡同为 `44rpx`，`26rpx` 只用于单位）。当前重新生成的独立切图为 `enterprise-hero-k-v2.png`（384×351，83,900 字节）、`new-leads-kpi-v2.png`（192×192，29,400 字节）、`completed-survey-kpi-v2.png`（192×192，26,700 字节）、`signed-contract-kpi-v2.png`（192×192，25,500 字节）、`contract-amount-kpi-v2.png`（192×192，30,100 字节）及 `operations-growth-chart-v2.png`（176×161，16,100 字节），均位于 `miniprogram/images/operations-dashboard/`。这些都是从已确认视觉语言重新生成的独立 RGBA PNG，并非从复合效果图裁切，且均低于 300KB；Hero 插画已按效果图校准后放大显示。下方白色「经营闭环」为三段流程，随后为两张真实效率卡；`GET /api/miniprogram/workbench?period=` 现同时返回仅负责人可见的 `contractAmountTrend`：本周/本月/不超过 62 日的自定义周期按上海时区逐日聚合，其他周期按月聚合，并带同长度上个可比周期。小程序「签约金额趋势（万元）」canvas 只绘制这两组真实曲线，只有两者都为 0 时才显示空态。该路由继续写入原生 `tabBar.list`，自定义 Tab 可 `switchTab` 进入。TabBar 为**工作台 / 经营 / 客户 / 提成 / 我的**；除只面向企业负责人的增量金额字段与趋势序列外，API、数据和权限边界不变。认证态 `390x844` 运行截图待用户复核。
...
覆盖：`enterprise-staff`、`enterprise-referrers`、`referrer-progress`、`referrer-earnings`、`staff-earnings`、`enterprise-commissions`（`status`）、`commission-records`（`status`）、`promotion-records`（`search`）、平台 `enterprises` / `devices`（仅已登记设备行）、线索详情 `assignable-staff`，以及配方选客户 `GET /api/miniprogram/ai/studio/leads`。不分页：`customer-projects` 首页排序全集、工作台预览上限、身份上下文、日历/当日预约、蓝牙扫描会话、设备页企业选择器，以及已超时待办（`GET /api/workbench/todos`，内部仍截断 200 条）。
...
- 服务码出示：`promotion-service-code` 按用户指定与活动码出示页对齐元素尺度，采用同一内容固有高度的薄荷海报（导航 `36rpx`、标题/副标题 `64/30rpx`、二维码框/图 `478/450rpx`、小 K `260rpx`、权益 `92/26/22rpx`），仍不展示企业名，并复用 `code-presenter-v3` 扫码小 K 与权益切图；`staff-activity-code` 与
  ...
  活动码按钮仍为「分享给客户」，生效入驻码仍以「一键分享」进入带当前 `ej_` 令牌的入驻页。企业负责人推荐人名册入口：经营台 Hero 在「邀请入驻」下方的「已入驻推荐人」整行、「我的」快捷「已入驻推荐人」，以及入驻码页两个 Tab 的文字链「查看已入驻推荐人」，进入负责人专属 `packages/business/enterprise-referrers/enterprise-referrers`（能力 `enterprise.operations`）。`GET /api/miniprogram/enterprise-referrers`（`query`、`status`）与 `POST /api/miniprogram/enterprise-referrers/[id]/disable` 复用 `ReferrerNetworkRepository` 与 Mini JWT `requireMiniProgramEnterpriseAdmin`（设计师/测量员/推荐人/客户 403）。停用幂等、无重新启用、不改历史线索和提成；被停用人可再扫入驻码产生一条新的 `active` 成员行。名册卡：已停用/已退出把电话图标贴在号码行、不单独开操作底栏；活动卡保留左「电话联系」、右「停用后续扫码」。
- 员工工作台：`GET /api/miniprogram/workbench` 支持 `period=week|month|year|custom`，自定义另传上海日历
  ...
  对 `enterprise_admin`，`pages/index/index` 现为**工作台**，承接获客、推广人名单入口、实时调度洞察及既有「查看预约安排」上下文跳转；
  `pages/enterprise-operations/enterprise-operations` 为独立的**经营** Tab，展示同一 API 返回的周期筛选全店经营大盘与经营闭环；批准的浅薄荷 Hero 将标题、四个原生周期控件、三项真实 KPI 与负责人专属的 `contractAmountSum` 签约金额收进同一胶囊安全插画场景。「签约金额」拆为原生数值/单位并按量级紧凑显示（`5万元`、`10万元`、`100万元`，更大金额切换为`亿元`/`万亿元`），不再直接输出完整元金额。Hero KPI 数字为 `44rpx`；仅当紧凑数值达到三个及以上字符时降为 `36rpx`，后续单位仍为 `26rpx`，确保各种金额均在窄卡内完整显示。Hero 与经营闭环的生成插画均独立打包；同一 `GET /api/miniprogram/workbench` 还返回负责人专属的 `contractAmountTrend`，按上海时区给出当前/上个可比周期的逐日或逐月签约金额，趋势卡仅绘制这些真实值。高屏光学校准通过内容固有尺寸放大 Hero 插画、KPI 卡、闭环节点、效率卡、趋势 canvas 与全部经营文字。该页已写入原生 `tabBar.list`，自定义 Tab 点击走 `switchTab` 而不是静默失败。负责人 TabBar 为**工作台 / 经营 / 客户 / 提成 / 我的**：既有客户路由保留，预约不再占固定 Tab，但原 `enterprise.appointments` 路由与能力边界不变。
...
`images/onboarding-referrer-v7/{xiao-k-promoter-hero-v7,enterprise-building-v7}.png`，均小于
...
客户、推广人以及家装设计顾问、家装现场顾问、企业负责人的“我的”页只保留账号、身份与安全能力，不再渲染旧员工户型列表、“新建量房”或“开始量房”。此外，`designer`、`measurer`、`enterprise_admin` 进入“我的”时不会显示统计卡、“我的工作台”“我的待办”以及“AI 设计空间”相关模块。推荐人 TabBar 现在直接提供“推广/客户/收益/我的”四个合同入口。家装设计顾问和家装现场顾问 TabBar 同样提供“收益”入口（`staff-earnings`），只读本人线索提成的待发放/已发放笔数与状态（不下发、不展示金额）。企业负责人 TabBar 提供“提成”入口（`enterprise-commissions`），展示本企业待支付/已支付/已作废金额汇总；客户完整付款批次中，待支付**状态**与可点击的「确认线下付款」**操作**明确分离，原生二次确认写「确认付款」，成功行写「已完成线下付款」。非零金额行复用 `LeadCommissionRepository.markPaid`，小程序不移植后台通用的改金额或改受益人能力；零金额仅适用下述有审计的快速记账例外。家装设计顾问、企业负责人和客户打开线索详情时不显示正式量房编辑、新增或删除动作，只有家装现场顾问可从本人已指派任务进入唯一正式量房编辑器。线索列表的“新增客户”入口仅对企业负责人显示，提交后写入 `manual_entry` 并自动派家装设计顾问/家装现场顾问；预约创建、预约详情和改期深链按客户、家装设计顾问、家装现场顾问、企业负责人能力分别放行。共享 `openSurveyingEditor` 还会对已签名身份执行二次能力校验，防止其他页面或旧深链绕过角色导航。

### 零金额提成快速记账

`POST /api/miniprogram/enterprise-commissions/record-zero-payment` 仅允许已签名 `enterprise_admin` 对一笔当前 `payableAmount` 为 `0.00` 的 `payable` 提成调用。它校验实际付款金额大于 0 且最多两位小数，以「小程序线下付款补录」写入既有金额调整审计，再在同一 Mini Program PostgreSQL 事务中将该笔标为 `paid`。对小程序调用者，`POST /api/miniprogram/enterprise-commissions/mark-paid` 会拒绝零金额行，因此含零金额行的批次不显示批量确认，非零行仍使用标准确认；小程序不能调整其他金额或任何受益人。
