...
- The canvas top bar is invariant: white surface, the same safe-area height, left 40px back target, title format `云栖花园 · 正式量房`, one compact green-dot saved indicator, then the text actions `引导`, `保存`, and `完成` aligned at the same right positions. `引导` is a persistent local toggle whose own green dot indicates enabled state; when enabled, it resolves a single contextual hint from the real wall graph and editor state rather than replaying a paged tutorial. Its green anchored bubble must not mask the canvas or fixed controls, and it must avoid the top navigation, right rail, bottom dock, and current wall. Disabling it removes only teaching bubbles/highlights, never close, measurement-side, snap, BLE, error, or completion feedback. A component-editor state may replace only the title with `构件编辑`; its height, icon placement, saved indicator, and action positions remain unchanged.
