...
test('scheme-studio route wires composer, send modal, and studio APIs', () => {
  ...
  assert.match(less, /border-bottom-left-radius: 26rpx/);
  ...
  assert.match(composerWxml, /timeline-start-round/);
  ...
  assert.match(composerWxml, /创作设置/);
...
test('scheme-studio confirm dialogs keep native action buttons inside the sheet', () => {
  ...
  assert.match(actionsRule, /grid-template-columns:\s*1fr 1fr/);
