...
test('enterprise owner workbench prioritizes acquisition and team actions without a sparse action area', () => {
  ...
  assert.match(styles, /\.enterprise-hero-card \.hero-top-row\s*\{[\s\S]*min-height:\s*48rpx/);
  assert.match(styles, /\.enterprise-hero-card \.stats-pills-row\s*\{[\s\S]*margin-top:\s*12rpx[\s\S]*flex-direction:\s*column[\s\S]*align-items:\s*flex-start[\s\S]*gap:\s*12rpx[\s\S]*padding-right:\s*236rpx/);
  assert.match(styles, /\.enterprise-hero-card \.stat-pill\s*\{[\s\S]*flex:\s*0 1 auto[\s\S]*align-self:\s*flex-start[\s\S]*width:\s*auto[\s\S]*min-width:\s*0[\s\S]*max-width:\s*100%[\s\S]*justify-content:\s*flex-start/);
  assert.match(styles, /\.enterprise-hero-card \.stat-pill-value\s*\{[\s\S]*min-width:\s*34rpx[\s\S]*background:\s*#dff7ea[\s\S]*font-variant-numeric:\s*tabular-nums/);
  assert.match(template, /class="stat-pill-value">\{\{stat\.count\(item\.value\)\}\}<\/text>/);
  assert.match(template, /role-hero-card-\{\{role\}\}/);
  assert.match(styles, /\.role-hero-card-designer \.enterprise-hero-mascot\s*\{[\s\S]*width:\s*236rpx[\s\S]*height:\s*236rpx/);
  assert.match(template, /xiao-k-designer-workbench-v2\.png/);
  assert.match(template, /quick-desc quick-desc-lines[\s\S]*查看与推进[\s\S]*我的客户 →/);
  assert.match(template, /quick-desc quick-desc-lines[\s\S]*进入设计与[\s\S]*风格配方 →/);
  assert.match(styles, /\.staff-role-quick-grid \.quick-text\s*\{[\s\S]*width:\s*166rpx[\s\S]*max-width:\s*calc\(100% - 128rpx\)/);
  assert.match(styles, /\.staff-role-quick-grid \.quick-desc-lines\s*\{[\s\S]*flex-direction:\s*column[\s\S]*white-space:\s*normal/);
  const designerWorkbenchMascot = fs.readFileSync(path.join(root, 'images', 'airy-v1', 'xiao-k-designer-workbench-v2.png'));
  assert.deepEqual([...designerWorkbenchMascot.subarray(0, 8)], [137, 80, 78, 71, 13, 10, 26, 10]);
  assert.ok(designerWorkbenchMascot.length <= 300 * 1024, 'designer workbench mascot must remain package-sized');
  assert.ok(designerWorkbenchMascot.includes(Buffer.from('tRNS')) || [4, 6].includes(designerWorkbenchMascot[25]), 'designer workbench mascot must retain alpha');
