...
test('customer project consumes only the owner-only aggregate and renders archive sections', () => {
  ...
  assert.match(wxml, />家装现场顾问</);
  assert.doesNotMatch(wxml, /专属家装设计顾问|金牌家装设计顾问|资深家装现场顾问/);
  assert.match(wxml, /designerProfessionalTitle/);
  assert.match(wxml, /designerExperienceLabel/);
  assert.match(wxml, /designerServiceLabel/);
  assert.match(wxml, /measurerProfessionalTitle/);
  assert.match(page, /resolvePublicProfessionalCopy/);
  assert.match(page, /designer\.professionalProfile/);
...
test('customer-facing project surfaces hide enterprise branding', () => {
  ...
  assert.match(folio, /您的家装顾问/);
  assert.match(folio, /现场顾问与设计方案全记录/);
  assert.doesNotMatch(folio, /\{\{heroTitle|\{\{heroSubtitle/);
  assert.doesNotMatch(folio, /免费量房与设计方案全纪录/);
...
test('V4 archive keeps one compact dossier book with differentiated status colors', () => {
  ...
  assert.match(wxml, /家装设计顾问发布后可查看与分享/);
