...
test('designer-contact-sheet restores the approved Xiao K QR-first contact design', () => {
  ...
  assert.match(js, /isCustomTabBarVisible/);
  assert.match(js, /getTabBar/);
  assert.match(js, /aboveTabBar/);
  assert.match(js, /syncHostSafeArea/);
  assert.match(wxml, /长按二维码，加家装设计顾问微信/);
  assert.match(wxml, /长按二维码 识别后添加/);
  assert.match(wxml, /dcs-root \{\{aboveTabBar \? 'above-tabbar' : ''\}\}/);
  ...
  assert.match(less, /\.dcs-root\.above-tabbar/);
  assert.match(less, /--sfp-custom-tabbar-safe-height/);
