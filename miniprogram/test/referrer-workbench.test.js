...
test('referrer workbench ships the Antigravity standalone asset and preserves the Airy Minimalist 06 design contract', () => {
  ...
  assert.match(wxml, /identity-brand-stack/);
  assert.match(wxml, /wx:if="\{\{userName\}\}"[\s\S]*identity-user-name[\s\S]*\{\{userName\}\}/);
  assert.doesNotMatch(wxml, /identity-badge-pill/);
  assert.doesNotMatch(wxml, /userName \|\| '推广人'/);
  ...
  assert.match(wxml, /quick-card-title">客户/);
  assert.match(wxml, /查看客户/);
  ...
  assert.match(less, /\.identity-brand-stack\s*\{[\s\S]*flex-direction:\s*column;/);
  assert.match(less, /\.identity-user-name\s*\{[\s\S]*font-size:\s*22rpx;/);
  assert.match(less, /\.identity-user-name\s*\{[\s\S]*margin-left:\s*50rpx;/);
  assert.match(less, /\.brand-mark\s*\{[\s\S]*width:\s*40rpx;/);
  assert.doesNotMatch(less, /\.identity-badge-pill/);
  assert.doesNotMatch(less, /\.nav-left/);
