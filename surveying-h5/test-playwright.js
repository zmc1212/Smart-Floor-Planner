...
(async () => {
  ...
  const canvasShell = await page..grid-canvas;
  if (!canvasShell) {
  ...
  // Emulate pointer events instead of mouse clicks, just in case
  // But page.mouse should send pointer events in modern playwright.
  ...
  // Click 'É¾³ý'
  const delBtn = await page.getByText('É¾³ý', { exact: true });
