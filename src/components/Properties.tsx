...
export const Properties: React.FC<PropertiesProps> = ({ 
  ...
  if (selectedRooms.length === 0) {
    return (
      <div className="md:w-72 w-0 md:flex hidden bg-white border-l border-gray-200 p-6 flex-col items-center justify-center text-gray-400 text-center">
    ...
    <div className="flex flex-col gap-6 flex-1 pb-6 md:pb-0 overflow-y-auto pr-2 custom-scrollbar">
      <div className="flex items-center justify-between">
        ...
        <div className={`flex items-center gap-2 ${isCollapsed ? 'flex-col' : 'flex-row'}`}>
          ...
          >
            {isCollapsed ? <Maximize2 size={20} /> : <X size={20} className="rotate-45" />}
          </button>
          <button
            onClick={onClose}
            className="p-2 text-gray-400 hover:bg-gray-100 rounded-lg transition-colors"
  ...
  return (
    <>
      <AnimatePresence>
        {selectedRooms.length > 0 && (
          <>
            ...
            <motion.div 
              initial={{ y: '100%' }}
              animate={{ y: isCollapsed ? 'calc(100% - 80px)' : 0 }}
              exit={{ y: '100%' }}
              transition={{ type: 'spring', damping: 25, stiffness: 200 }}
              className="md:hidden block fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-6 rounded-t-3xl shadow-[0_-10px_30px_rgba(0,0,0,0.1)] z-40 max-h-[60vh] overflow-hidden flex flex-col"
            >
              <div 
      ...
      {/* Fullscreen Preview Modal - Moved outside to prevent double rendering */}
      <AnimatePresence>
